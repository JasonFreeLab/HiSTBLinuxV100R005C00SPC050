...
HI_S32 HI_MODULE_Register(HI_U32 u32ModuleID, const HI_CHAR * pszModuleName)
{
    ...
    if ((g_s32ModuleFd == -1 || NULL == pszModuleName)  || ((strlen(pszModuleName) == 0) || (strlen(pszModuleName) >= sizeof(stModule.u8ModuleName))))
    {
        MODULE_UNLOCK(&g_ModuleMutex);
        HI_ERR_PRINT(HI_ID_MODULE, "param invalid!\n");
        return HI_FAILURE;
    }
    strncpy((HI_CHAR *)stModule.u8ModuleName, pszModuleName, strlen(pszModuleName));
...
HI_S32 HI_MODULE_RegisterByName(const HI_CHAR * pszModuleName, HI_U32* pu32ModuleID)
{
    ...
    if ((NULL == pszModuleName) || (strlen(pszModuleName)==0) || (strlen(pszModuleName) >= sizeof(stModule.u8ModuleName)) || (NULL == pu32ModuleID))
    {
...
HI_S32 HI_MODULE_GetModuleID(const HI_U8* pu8ModuleName)
{
    ...
    if ((strlen((HI_CHAR *)pu8ModuleName) == 0)||(strlen((HI_CHAR *)pu8ModuleName) >= sizeof(stModule.u8ModuleName)))
    {
        MODULE_UNLOCK(&g_ModuleMutex);
        HI_ERR_PRINT(HI_ID_MODULE, "ModuleName too len !\n");
        return HI_FAILURE;
    }
    strncpy((HI_CHAR *)stModule.u8ModuleName, (HI_CHAR *)pu8ModuleName, strlen((HI_CHAR *)pu8ModuleName));
