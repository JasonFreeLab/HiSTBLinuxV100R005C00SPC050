...
extern int init_ttf_mem_info(struct ttf_mem *ttf_mem_addr);
extern int get_ttf_mem_info(struct ttf_mem *ttf_addr);
extern int unmap_ttf_mem(unsigned int addr, unsigned int size);
#endif
...
#if (TRUSTEDCORE_PLATFORM_CHOOSE == WITH_DEVCHIP_PLATFORM)
extern int swi_handler_chip(int swi_id, TSK_REGS_S *regs, UINT64 permissions);
#else
...
extern int SEC_VDEC_Init(unsigned int *, unsigned int, unsigned int *, unsigned int);
...
extern int SEC_VDEC_ReadProc(paddr_t, int);
extern int SEC_VDEC_WriteProc(unsigned int, int);
#endif
extern int SEC_VDEC_GetChanImage(int, unsigned int *);
extern int SEC_VDEC_ReleaseChanImage(int, paddr_t);
#endif

#if (TRUSTEDCORE_PLATFORM_CHOOSE != WITH_DEVCHIP_PLATFORM)
int get_target_product(char *target_product, int *buff_size)
{
	int size;

	if (NULL == target_product || NULL == buff_size)
		return -1;
...
int swi_handler_ext(int swi_id, TSK_REGS_S *regs, UINT64 permissions)
{
	...
	HANDLE_SYSCALL(swi_id) {
		...
		SYSCALL_PERMISSION(SW_SYSCALL_SCARD_SUPPORT_MODE, permissions,
				SE_GROUP_PERMISSION) /*lint !e835 !e587 !e774 */
		uwRet = (UINT32)scard_support_mode();
		regs->r0 = uwRet;
		SYSCALL_END
		...
		ACCESS_CHECK(regs->r1, regs->r2);
		ACCESS_READ_RIGHT_CHECK(regs->r1, regs->r2);
		uwRet = (UINT32)scard_send((int)regs->r0, (unsigned char *)regs->r1, (unsigned int)regs->r2);
		regs->r0 = uwRet;
		SYSCALL_END

		SYSCALL_PERMISSION(SW_SYSCALL_SCARD_RECEIVE, permissions,
				SE_GROUP_PERMISSION) /*lint !e835 !e587 !e774 */
		ACCESS_CHECK(regs->r1, sizeof(unsigned int));
		ACCESS_WRITE_RIGHT_CHECK(regs->r1, sizeof(unsigned int));
		if (regs->r1) {
			ACCESS_CHECK(regs->r0, *(unsigned int *)regs->r1);
			ACCESS_WRITE_RIGHT_CHECK(regs->r0, *(unsigned int *)regs->r1);
		}
		uwRet = (UINT32)scard_receive((unsigned char *)regs->r0, (unsigned int *)regs->r1);
		regs->r0 = uwRet;
		SYSCALL_END
		...
		uwRet = (UINT32)init_tui_sdriver((void *)regs->r0);
		regs->r0 = uwRet;
		SYSCALL_END
		...
		SYSCALL_PERMISSION(SW_SYSCALL_INIT_TTF_MEM, permissions, TUI_GROUP_PERMISSION)
		ACCESS_CHECK(regs->r0, sizeof(struct ttf_mem_k));
		ACCESS_READ_RIGHT_CHECK(regs->r0, sizeof(struct ttf_mem_k));
		uwRet = (UINT32)init_ttf_mem_info((struct ttf_mem_k *)regs->r0);
		regs->r0 = uwRet;
		SYSCALL_END

		SYSCALL_PERMISSION(SW_SYSCALL_GET_TTF_MEM, permissions, TUI_GROUP_PERMISSION)
		ACCESS_CHECK(regs->r0, sizeof(struct ttf_mem_k));
		ACCESS_WRITE_RIGHT_CHECK(regs->r0, sizeof(struct ttf_mem_k));
		uwRet = (UINT32)get_ttf_mem_info((struct ttf_mem_k *)regs->r0);
		regs->r0 = uwRet;
		SYSCALL_END

		SYSCALL_PERMISSION(SW_SYSCALL_UNMAP_TTF_MEM, permissions, TUI_GROUP_PERMISSION)
		uwRet = (UINT32)unmap_ttf_mem(regs->r0, regs->r1);
		regs->r0 = uwRet;
		SYSCALL_END

		SYSCALL_PERMISSION(SW_SYSCALL_CURSOR, permissions,
				   TUI_GROUP_PERMISSION) /*lint !e835 !e587 !e774 */
		uwRet = set_cursor_flag();
		regs->r0 = uwRet;
		SYSCALL_END
		...
		if (regs->r1) {
			...
			size = *(UINT32 *)regs->r1;
			ACCESS_CHECK(regs->r0, size);
			ACCESS_WRITE_RIGHT_CHECK(regs->r0, size);
			uwRet = (UINT32)get_target_product((char *)regs->r0, &size);
			*(UINT32 *)regs->r1 = size;
		}
		regs->r0 = uwRet;
		SYSCALL_END
		...
		ACCESS_CHECK(regs->r1, sizeof(UINT32));
		ACCESS_READ_RIGHT_CHECK(regs->r1, sizeof(UINT32));
		ACCESS_CHECK(regs->r1, sizeof(UINT32));
		ACCESS_WRITE_RIGHT_CHECK(regs->r1, sizeof(UINT32));
		uwRet = (UINT32)SEC_VDEC_GetChanImage((int)regs->r0, (unsigned int *)regs->r1);
		regs->r0 = uwRet;
		SYSCALL_END

		SYSCALL_PERMISSION(SW_SYSCALL_SEC_VDEC_RELEASECHANIMAGE, permissions, VDEC_GROUP_PERMISSION)
		uwRet = (UINT32)SEC_VDEC_ReleaseChanImage((int)regs->r0, ((paddr_t)regs->r1 << 32) | regs->r2);
		regs->r0 = uwRet;
		SYSCALL_END
#endif
#ifdef FEATURE_IRIS
