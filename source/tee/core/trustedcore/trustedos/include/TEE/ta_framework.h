...
#define RPMB_TASK_NAME "task_rpmb"
...
#define SSA_SMC_CMD		0x27
#define SSA_SMC_ACK		0x28
#define TA_LOCK_ACK		0x29
#define RPMB_SMC_CMD		0x2A
#define RPMB_SMC_ACK		0x2B
...
enum global_to_rpmb_msg {
	MSG_2_RPMB_UPDATE	= 0xAAAA0001,
	MSG_2_RPMB_REGISTER	= 0xAAAA0002,
};
...
typedef struct {
	TEE_Operation op;
	void *p_vaddr_gt_tee[4]; /* virt tee mem addr of param mems mapping for gt */
	void *p_vaddr_gt_ree[4]; /* virt ree mem addr of param mems mapping for gt */
} PAM_Node;

struct crash_remain_session {
	LIST_OBJECT_S session_list;
	TEE_UUID uuid;
	uint32_t session_id;
	uint32_t session_rnd;
};
...
TEE_Result cmd_ns_get_params(
				uint32_t task_id,
				smc_cmd_t *cmd,
				uint32_t *param_type,
				TEE_Param **params,
				TEE_Param **params_addr_ta);
TEE_Result cmd_secure_get_params(uint32_t task_id,
		smc_cmd_t *cmd,
		uint32_t *param_type,
		TEE_Param **params, TEE_Param **params_addr_ta);
...
TEE_Result kill_task(uint32_t cmd_type);
void find_reused_service(char *name, struct sessison_task_info *p_sti);
...
void dump_task(void);
void send_msg_rpmb_update(void);

TEE_Result start_internal_task(TEE_UUID uuid, bool restart);
void release_pam_node(PAM_Node *node);
void copy_pam_to_ree(uint32_t cmd_id);
#endif
