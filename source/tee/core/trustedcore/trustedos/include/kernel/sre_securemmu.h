...
/* UINT64_MAX */
#define ADDR_MAX 0xffffffffffffffff
...
extern UINT32 g_TRUSTEDCORE_VIRT_BASE;
extern UINT32 g_GT_MEMPOOL_SIZE;
...
enum
{
	LIB_TEE_SHARE_SEGMENT = 0,
	TA_MEM_PROG_SEGMENT,
	TA_HEAP_SEGMENT,
	TA_STACK_SEGMENT,
	LIB_TEE_SYMTAB_SEGMENT,
	MAX_TA_MAP_SEGMENT,
};

typedef struct {
	unsigned int rnd_offset;
	unsigned int base_addr;
	unsigned int static_size;
	unsigned int heap_viraddr;
	unsigned int heap_size;
	unsigned int stack_viraddr;
	unsigned int stack_size;
} global_map_task_mem_t;
...
pgd_t map_task_mem_for_ta(map_task_mem_t *task_map, UINT32 size);
pgd_t map_task_mem(global_map_task_mem_t *task_map);
