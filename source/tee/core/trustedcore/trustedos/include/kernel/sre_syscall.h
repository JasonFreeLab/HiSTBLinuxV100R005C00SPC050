...
extern int __handle_SecureContentMem_RelCb(paddr_t addr, unsigned int size, unsigned int type);
...
extern UINT32 __SRE_SwSleep(UINT32 uwSecs);  /*??????'?ã???????*/
...
extern UINT32 __SRE_MsgSnd(UINT32 uwMsgID, MSG_PID_T uwDstPID, void *msgp, UINT16 size);
extern UINT32 __SRE_MsgRcv(UINT32 uwTimeout, UINT32 *puwMsgID, void *msgp, UINT16 size);
extern UINT32 __SRE_MsgRcvA(UINT32 uwTimeout, UINT32 *puwMsgID, void *msgp, UINT16 size, MSG_PID_T *puwSenderPID);
...
extern INT32 __SRE_TaskMem_GetInternalTaskInfo(UINT32 *addr, UINT32 start_index, UINT32 num);
...
extern VOID *__SRE_MemAllocShared(UINT8 ucPtNo, UINT32 uwSize);
extern UINT32 __SRE_MemFreeShared(UINT8 ucPtNo, VOID *pAddr);
...
extern UINT32 __get_provision_key(DX_UTIL_OemKey_t pOemKey);
...
#define RPMB_ROOTKEY_SIZE_MAX	100
#define RPMB_CHIP_VERSION_MAGIC 0x5a5a
#define RPMB_CHIP_VERSION_01	0x5a5a0001
#define RPMB_EMMC_BITMAP_DEF	0x03
struct rpmb_atf_info {
	...
	UINT32 data_len;
	UINT32 data_addr;
	UINT32 mdt;	/* 1: EMMC 2: UFS */
	UINT32 support_bit_map;/* the device's support bit map, for example, if it support 1,2,32, then the value is 0x80000003 */
	UINT32 version; /*??16bit??0x5a5a???support_bit_map???????16bit?j?????	0x1??'*/
...
extern unsigned int __rpmb_update_capable();
extern void __rpmb_smc_switch(unsigned int phy_addr);
extern unsigned int __rpmb_partition_info_write(struct rpmb_atf_info *partition);
extern unsigned int __rpmb_partition_info_read(struct rpmb_atf_info *partition);
