...
/* 208MB virtual space for task, 104 L1 ptes */
#define TASK_L1_TABLES  104
#else
#define TASK_MEM_SHIFT 20 //1M size
/* 208MB virtual space for task, 208 L1 ptes */
#define TASK_L1_TABLES  208

#endif

/* Number of bits used for TASK ASLR */
#define TASK_ASLR_WIDTH 12
...
extern INT32 SRE_TaskMem_GetInternalTaskInfo(UINT32 *addr, UINT32 start_index, UINT32 num);
extern int phy_addr_check(paddr_t  phy_addr, UINT32 size);
extern int nomap_phy_addr_check(u64 phy_addr, UINT32 size);
extern int check_secureos_addr(paddr_t phy_addr, UINT32 size);
extern int ta2ta_sharemem_check(paddr_t phy_addr);
extern int create_rnd_offset(UINT32 *rnd_offset);
