...
#define SPACC_READ(addr)        *(volatile unsigned int *)(tee_hal_phys_to_virt((unsigned long)addr))
#define SPACC_WRITE(addr, val)  *(volatile unsigned int *)(tee_hal_phys_to_virt((unsigned long)addr)) = (val)
