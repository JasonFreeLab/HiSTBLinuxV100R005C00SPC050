...
static HI_VOID TTX_Parse_DrawSubpageSpreadNavigation(TTX_PAGE_CONTEXT_S* pstCurrentPoint,
                                                      ...
                                                      HI_U8 u8StartPosition)
{
    ...
    {
        TTX_Show_SetArea(pstPageArea, TTX_ROW_NUM + TTX_NAVIGATION_BAR, u32ColNum - 1, 1, 1);
    }
    else
    {
        TTX_Show_SetArea(pstPageArea, TTX_ROW_NUM, u32ColNum - 1, 1, 1);
    }
...
HI_VOID TTX_Parse_HideTime(TTX_PAGE_CONTEXT_S* pstContextHead)
{
    ...
    TTT_Show_FillRect(pstCurrentPoint, &stFillRectParam);
    ...
    TTT_Show_Refresh(pstCurrentPoint, &stRefrshLayerParam);
...
HI_S32 TTX_Parse_DrawTime(TTX_PAGE_CONTEXT_S* pstContextHead)
{
    ...
    for (i = 0; i < 8; i++)   /**  Draw char */
    {
        ...
        TTT_Show_DrawChar(pstCurrentPoint, &stDrawCharParam);
    }
...
HI_VOID TTX_Parse_DrawSubpageNavigation(TTX_PAGE_CONTEXT_S* pstContextHead)
{
    ...
    if (u8NumOfSubPg != 0)    /** Subpage be present*/
    {
        ...
        if (pstCurrentPoint->bNavigation)
        {
            TTX_Show_SetArea(&stPageArea, TTX_ROW_NUM + TTX_NAVIGATION_BAR, 0, TTX_SUBPAGE_BAR, u32ColNum);
        }
        else
        {
        ...
        for (j = 0; j < 4; j++)
        {
            ...
            if (pstCurrentPoint->bNavigation)
            {
                TTX_Show_SetArea(&stPageArea, TTX_ROW_NUM + TTX_NAVIGATION_BAR, j + 1, 1, 1);
            }
            else
            {
                TTX_Show_SetArea(&stPageArea, TTX_ROW_NUM, j + 1, 1, 1);
            }
        ...
        /**  Find start position */
        u8StartPosition = TTX_Parse_FindStartPosition(pstCurrentPoint, au8SubPage);
        ...
        for (i = u8StartPosition, j = 0; i < u8StartPosition + 8; i++, j++)
        {
            if (au8SubPage[i % TTX_MAX_SUBPAGENO] != 0)    /** Draw subpage */
            {
                ...
                if (pstCurrentPoint->bNavigation)
                {
                    TTX_Show_SetArea(&stPageArea, TTX_ROW_NUM + TTX_NAVIGATION_BAR, 8 + 4 * j, 1, 1);
                }
                else
                {
                    TTX_Show_SetArea(&stPageArea, TTX_ROW_NUM, 8 + 4 * j, 1, 1);
                }
        ...
        TTX_Parse_DrawSubpageSpreadNavigation(pstCurrentPoint, &stPageArea, u8NumOfSubPg, u8StartPosition);
    }
...
HI_VOID TTT_Show_DrawChar(TTX_PAGE_CONTEXT_S* pstContextHead, HI_UNF_TTX_DRAWCAHR_S* pstDrawCharParam)
{
    HI_S32 s32Ret = HI_SUCCESS;
    TTX_PAGE_CONTEXT_S* pstCurrentPoint = pstContextHead;
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_WARN_TTX("TTT_Show_DrawChar, Errors occur while call functions\n");
    }

    return;
}

HI_VOID TTT_Show_FillRect(TTX_PAGE_CONTEXT_S* pstContextHead, HI_UNF_TTX_FILLRECT_S* pstFillRectParam)
{
    HI_S32 s32Ret = HI_SUCCESS;
    TTX_PAGE_CONTEXT_S* pstCurrentPoint = pstContextHead;
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_WARN_TTX("TTT_Show_FillRect, Errors occur while call functions\n");
    }

    return;
}

HI_VOID TTT_Show_Refresh(TTX_PAGE_CONTEXT_S* pstContextHead, HI_UNF_TTX_REFRESHLAYER_S* pstRefrshLayerParam)
{
    HI_S32 s32Ret = HI_SUCCESS;
    TTX_PAGE_CONTEXT_S* pstCurrentPoint = pstContextHead;
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_WARN_TTX("TTT_Show_Refresh, Errors occur while call functions\n");
    }
