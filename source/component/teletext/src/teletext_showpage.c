...
TTX_LANGUAGE_S g_stLanguageCode[] =
{
    ...
    {"rus", HI_UNF_TTX_G0SET_CYRILLIC_2, HI_UNF_TTX_G2SET_CYRILLIC, HI_UNF_TTX_NATION_SET_BUTT}
};
...
static HI_VOID TTX_Show_RefreshPage(TTX_PAGE_CONTEXT_S* pstCurrentPoint, HI_U32 u32MaxColNum)
{
    ...
    {
        TTX_Show_SetArea(&stPageArea, 0, 0, TTX_ROW_NUM + TTX_NAVIGATION_BAR + TTX_SUBPAGE_BAR, u32MaxColNum);
    }
    else
    {
    ...
    TTX_Show_CallBack(pstCurrentPoint, HI_UNF_TTX_CB_APP_REFRESH, (HI_VOID *)&stRefrshLayerParam);
...
static HI_S32 TTX_Show_ProcControlBits(TTX_PAGE_CONTEXT_S* pstCurrentPoint, HI_BOOL* pbBoxPage, HI_U8* pu8NumOfSubPg)
{
    ...
    if (1 == C10)      /** don't display the page */
    {
        if (pstCurrentPoint->bNavigation)
        {
            TTX_Show_SetArea(&stPageArea, 0, 0, TTX_ROW_NUM + TTX_NAVIGATION_BAR + TTX_SUBPAGE_BAR,
                                      u32MaxColNum);
        }
        else
        {
                                     ...
                                     TTX_SHOW_SPACEATTR_S* pstSpaceAttr)
{
    ...
    {
        TTX_Show_SetArea(&stPageArea, u32LineNum, u32X, 1, 1);
        stFillRectParam.pstPageArea = &stPageArea;
...
static HI_S32 TTX_Show_RefreshShowArea(TTX_PAGE_CONTEXT_S* pstCurrentPoint, HI_BOOL bBoxPage, HI_U8 u8NumOfSubPg)
{
    ...
    else
    {
        ...
        if (pstCurrentPoint->bNavigation)
        {
            TTX_Show_SetArea(&stPageArea, 1, 0, TTX_ROW_NUM + TTX_NAVIGATION_BAR - 1, u32MaxColNum);
        }
        else
        {
        ...
        if (pstCurrentPoint->bNavigation)
        {
            TTX_Show_SetArea(&stPageArea, TTX_ROW_NUM + TTX_NAVIGATION_BAR, 0, TTX_SUBPAGE_BAR, u32MaxColNum);
        }
        else
        {
...
static HI_VOID TTX_Show_DecShowPage(TTX_PAGE_CONTEXT_S*  pstContextHead)
{
    ...
    HI_INFO_TTX("draw page: 0x%x, 0x%x, 0x%x\n", \
...
HI_VOID TTX_Show_DrawNavigation(TTX_PAGE_CONTEXT_S* pstCurrentPoint, HI_U32 u32MaxColNum)
{
    ...
    if (pstCurrentPoint->bNavigation)
    {
        if (!pstCurrentPoint->bTtxMix)
        {
            TTX_Show_SetArea(&stPageArea, TTX_ROW_NUM, 1, 1, u32MaxColNum / 4);
            stFillRectParam.pstPageArea = &stPageArea;
        ...
        for (i = 0; i < 4; i++)      /** Add  the page address of navigation key */
        {
            ...
            for (j = 0; j < 3; j++)
            {
                TTX_Show_SetCharAttr(&stCharAttr, u32aPageNum[j], HI_FALSE, HI_UNF_TTX_G0SET_LATIN, HI_UNF_TTX_G2SET_LATIN,
                                     HI_UNF_TTX_CHARSET_G0, 0);
                stDrawCharParam.pstCharAttr = &stCharAttr;
                TTX_Show_SetArea(&stPageArea, TTX_ROW_NUM, 4 + i * u32MaxColNum / 4 + j, 1, 1);
                stDrawCharParam.pstPageArea   = &stPageArea;
...
HI_VOID TTX_Show_DrawNavigationSubpageMode(TTX_PAGE_CONTEXT_S* pstCurrentPoint, HI_U32 u32MaxColNum)
{
    ...
    TTX_Show_SetCharAttr(&stCharAttr, '-', HI_FALSE, HI_UNF_TTX_G0SET_LATIN, HI_UNF_TTX_G2SET_LATIN,
                         HI_UNF_TTX_CHARSET_G0, 0);
    stDrawCharParam.pstCharAttr = &stCharAttr;
    TTX_Show_SetArea(&stPageArea, TTX_ROW_NUM, u32ColPos, 1, 1);  /*  col position: 1/8 */
    stDrawCharParam.pstPageArea   = &stPageArea;
    ...
    TTX_Show_SetCharAttr(&stCharAttr, '+', HI_FALSE, HI_UNF_TTX_G0SET_LATIN, HI_UNF_TTX_G2SET_LATIN,
                         HI_UNF_TTX_CHARSET_G0, 0);
    TTX_Show_SetArea(&stPageArea, TTX_ROW_NUM, u32ColPos, 1, 1);
...
HI_VOID    TTX_Show_ShowPrompt(TTX_PAGE_CONTEXT_S* pstContextHead)
{
    ...
    if (pstCurrentPoint->bNavigation)
    {
        TTX_Show_SetArea(&stPageArea, 0, 0, TTX_ROW_NUM + TTX_NAVIGATION_BAR + TTX_SUBPAGE_BAR, u32ColNum);
    }
    else
    {
