...
static HI_S32 _ARIBCC_InitDesignState4_Format (ARIBCC_CaptionType_E enCaptionType, ARIBCC_Management_S *pstManagement, HI_U32 u32Operation,
                                                                                                        HI_U8 u8LanguageIndex, HI_S32 *ps32IntervalShs, HI_S32 *ps32IntervalSvs)
{
    HI_S32  s32Ret = HI_SUCCESS;
    HI_U8   s32Format = 0xFF;
...
static HI_S32 _ARIBCC_InitDesignState4 (ARIBCC_CaptionType_E enCaptionType, ARIBCC_Management_S *pstManagement, HI_U32 u32Operation)
{
        ...
        || ((u32Operation == CAP_INIT_CSI_SWF) && (bSetFlg == HI_FALSE)) )
    {
        ...
        if (s32Ret == HI_SUCCESS)
        {
            ...
            s32Ret = _ARIBCC_InitDesignState4_Format (enCaptionType, pstManagement, u32Operation, u8LanguageIndex, &IntervalShs, &IntervalSvs);
        }
    }
...
static HI_S32 _ARIBCC_InitDesignState5_CalcWH (ARIBCC_CaptionType_E enCaptionType, ARIBCC_Management_S *pstManagement, HI_U32 u32Operation,
                                                                                                        ARIBCC_OBJECT_S *pstObject, HI_S32 *ps32Width, HI_S32 *ps32Height, HI_BOOL bSetFlg)
{
    HI_S32  s32Ret = HI_SUCCESS;
    HI_U8   u8LanguageIndex = 0;
...
static HI_S32 _ARIBCC_InitDesignState5 (ARIBCC_CaptionType_E enCaptionType, ARIBCC_Management_S *pstManagement, HI_U32 u32Operation)
{
    ...
    s32Ret = _ARIBCC_InitDesignState5_CalcWH (enCaptionType, pstManagement, u32Operation, pstObject, &Width, &Height, bSetFlg);
    if (s32Ret == HI_SUCCESS)
    {
        if (u16X == 0 || u16Y == 0)
        {
...
static HI_S32 _ARIBCC_8BitCode_SwitchControl_DRCS(ARIBCC_CaptionType_E enInCCType, HI_S32 *ps32ControlData, HI_U8 *pu8Designate)
{
    HI_U16     i  = 0;
...
static HI_S32 _ARIBCC_8BitCode_SwitchControl_DES(ARIBCC_CaptionType_E enInCCType, HI_S32 *ps32ControlData, HI_U8 *pu8Designate)
{
    HI_S32     s32Ret = HI_SUCCESS;
    HI_U16     i  = 0;
    ...
    switch( *(ps32ControlData+1) )
    {
        ...
        case ARIB_GSET_ADDITION_SIGN:
            ...
            break;
        default:
            s32Ret = _ARIBCC_8BitCode_SwitchControl_DRCS(enInCCType, ps32ControlData, pu8Designate);
            break;
    }

    gCaptionInfo[enInCCType].stSwitch.u8Designate[i] = pu8Designate[i];
...
static HI_S32 _ARIBCC_8BitCode_SwitchControl(ARIBCC_CaptionType_E enInCCType,ARIBCC_Control_S *pstControl)
{
    ...
    switch( pstControl->enControlId )
    {
        ...
        case ARIB_ESC_DES:  //desinate code
            ...
            s32Ret = _ARIBCC_8BitCode_SwitchControl_DES(enInCCType, ps32ControlData, au8Designate);
...
static HI_VOID _ARIBCC_8BitCode_Text_C0(ARIBCC_CaptionType_E enInCCType, HI_U32 *pu32TextLength, HI_U8 *pu8Text, HI_U8 *pu8Data,
                                                                                                HI_U8 *pu8i, HI_U8 *pu8Index)
{
    HI_U8      au8Designate[4];
    HI_U8      au8Invoke[2];
    HI_U8      InvokeTmp     = 0;
    ...
    if(*pu32TextLength > 0)
    {
        ARIBCC_OSD_DrawText((HI_S32)enInCCType,pu8Text,*pu32TextLength); // CC Graphic API
    }
    ...
    if(au8Invoke[ARIBCC_GL] == 0x02)
    {
        ...
        else if(au8Designate[ARIBCC_CODESET_G2] == 0x1D)
        {
            ...
            {
                *pu8Index = (HI_U8)(*pu8Text - 0xE0);
                if(*pu8Index > 15)
                {
                    HI_WARN_CC("index overflow\n");
                    *pu8Index = 15;
                }
                gCaptionInfo[enInCCType].stSwitch.u8Designate[ARIBCC_CODESET_G0] = au8Designate[ARIBCC_CODESET_G0] = g_stMacro[*pu8Index].au8Designate[ARIBCC_CODESET_G0];
                gCaptionInfo[enInCCType].stSwitch.u8Designate[ARIBCC_CODESET_G1] = au8Designate[ARIBCC_CODESET_G1] = g_stMacro[*pu8Index].au8Designate[ARIBCC_CODESET_G1];
                gCaptionInfo[enInCCType].stSwitch.u8Designate[ARIBCC_CODESET_G2] = au8Designate[ARIBCC_CODESET_G2] = g_stMacro[*pu8Index].au8Designate[ARIBCC_CODESET_G2];
                gCaptionInfo[enInCCType].stSwitch.u8Designate[ARIBCC_CODESET_G3] = au8Designate[ARIBCC_CODESET_G3] = g_stMacro[*pu8Index].au8Designate[ARIBCC_CODESET_G3];
                gCaptionInfo[enInCCType].stSwitch.u8Invoke[ARIBCC_GR] = au8Invoke[ARIBCC_GR]  = g_stMacro[*pu8Index].au8Invoke[ARIBCC_GR];
                gCaptionInfo[enInCCType].stSwitch.u8Invoke[ARIBCC_GL] = au8Invoke[ARIBCC_GL] = g_stMacro[*pu8Index].au8Invoke[ARIBCC_GL];
                is_macro_decode = 1;
            }
            pu8Data += 1;
            *pu8i += 1;
...
static HI_S32 _ARIBCC_8BitCode_Text(ARIBCC_CaptionType_E enInCCType,ARIBCC_Data_S *pstInData)
{
    ...
    for( i=0,u32TextLength=0; i < u32DataLength; i++)
    {
        ...
        {
            (HI_VOID)_ARIBCC_8BitCode_Text_C0(enInCCType, &u32TextLength, pu8Text, pu8Data, &i, &u8Index);
        }
...
static HI_S32 _ARIBCC_ParseDrcs_Font(ARIBCC_CaptionType_E enInCCType, HI_U8 *pDrcsFont, HI_U8 u8NumberOfFont,
                                                                                  HI_U8 u8EntryType, HI_S32 s32DrcsType, HI_U8 u8Code1, HI_U8 u8Code2)
{
    HI_S32     s32Ret = HI_SUCCESS;
    HI_U8      u8FontId;
    HI_U8      u8Mode;
...
static HI_S32 _ARIBCC_ParseDrcs(ARIBCC_CaptionType_E enInCCType ,HI_U32 u32InUnitId ,HI_U8 *pu8InDrcs)
{
    ...
    for(i=1 ;i <= u8NumberOfCode; i++)
    {
        ...
        s32Ret = _ARIBCC_ParseDrcs_Font(enInCCType, pDrcsFont, u8NumberOfFont, u8EntryType, s32DrcsType, u8Code1, u8Code2);
        if(s32Ret != HI_SUCCESS)
        {
            break;
        }
...
static HI_S32 _ARIBCC_ParseStatementBody_C0CS(ARIBCC_CaptionType_E enInCCType)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_BOOL   bRollupFlg     = HI_FALSE;
...
static HI_S32 _ARIBCC_ParseStatementBody_C1(ARIBCC_CaptionType_E enInCCType, ARIBCC_Control_S *pstControl)
{
    HI_S32 s32Ret = HI_SUCCESS;
...
static HI_S32 _ARIBCC_ParseStatementBody_CtrlCode(ARIBCC_CaptionType_E enInCCType, ARIBCC_Control_S *pstControl)
{
    HI_S32 s32Ret = HI_SUCCESS;
    ...
    switch( pstControl->enControlId )
    {
        ...
        case ARIB_C0_CS:
            s32Ret = _ARIBCC_ParseStatementBody_C0CS(enInCCType);
            break;
        case ARIB_C0_LS0:
        case ARIB_C0_LS1:
        case ARIB_ESC_LS2:
        ...
        case ARIB_SP_DEL:
            ...
            break;
        default:
            s32Ret = _ARIBCC_ParseStatementBody_C1(enInCCType, pstControl); /*source monitor*/
            break;
    }
    ...
    while(u32Length < u32UnitLength)
    {
        ...
        if(pstStatementBody->u32ControlCount > 0)
        {
            for(i=1 ;i <= (pstStatementBody->u32ControlCount);i++)
            {
                ...
                s32Ret = _ARIBCC_ParseStatementBody_CtrlCode(enInCCType, pstControl);
                if(s32Ret != HI_SUCCESS)
                {
