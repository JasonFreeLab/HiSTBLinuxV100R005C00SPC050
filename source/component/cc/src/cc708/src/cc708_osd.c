...
static HI_VOID _DtvCC_DrawCaptionToWindowOnScreen_Prepare(HI_U16 *pu16Buf, HI_S32 s32Row, HI_S32 s32Column,
                                                                                                     CC708_PenDef_S *pstPen, CC708_Cursor_S *pstCursor, HI_BOOL *pbIgnoreLeftBG)
{
    HI_S32 WinID = CurrentWindowID;
    HI_S32 x, y;
...
static inline HI_VOID _DtvCC_DrawCaptionToWindowOnScreen(HI_U16 *pu16Buf, HI_S32 s32Len, HI_S32 s32Row, HI_S32 s32Column)
{
    ...
    (HI_VOID)_DtvCC_DrawCaptionToWindowOnScreen_Prepare(pu16Buf, s32Row, s32Column, pstPen, pstCursor, &bIgnoreLeftBG);
...
/*source monitor*/
static HI_BOOL _DtvCC_CalcWinPosByAnchorPoint_GetPosi(HI_U8 u8WindowID, HI_S32 *ps32WinX, HI_S32 *ps32WinY, HI_S32 WinWidth, HI_S32 WinHeight)
{
...
static HI_VOID _DtvCC_CalcWinPosByAnchorPoint_ChkStaXY(HI_S32 *ps32WinX, HI_S32 *ps32WinY, HI_U16 StaX, HI_U16 StaY, HI_U16 StaWidth, HI_U16 StaHeight)
{
...
static HI_VOID _DtvCC_CalcWinPosByAnchorPoint_LimitPos (HI_S32 *ps32WinX, HI_S32 *ps32WinWidth, HI_S32 *ps32MaxStaWidth,
                                                                                                                                    CC708_OBJECT_S *pCCObj, HI_U16 StaX, HI_U16 StaWidth)
{
...
static inline HI_BOOL _DtvCC_CalcWinPosByAnchorPoint(HI_U8 u8WindowID)
{
    ...
    (HI_VOID)_DtvCC_CalcWinPosByAnchorPoint_ChkStaXY(&WinX, &WinY, StaX, StaY, StaWidth, StaHeight);

    /*--start ??*/
    (HI_VOID)_DtvCC_CalcWinPosByAnchorPoint_LimitPos (&WinX, &WinWidth, &s32MaxStaWidth, pCCObj, StaX, StaWidth);
    /*--end ??*/
...
/*source monitor*/
static HI_VOID _DtvCC_CheckWindowOverlap_FindWinMap(HI_U8 u8WindowID, HI_U8 flag, HI_U8 *pu8WindowMap)
{
    HI_S32 i;
...
static HI_VOID _DtvCC_CheckWindowOverlap_Run (HI_S32 i, HI_U8 *pu8OverlapWindowMap, HI_S32 s32CurWinStartX,
                                                                                                            HI_S32 s32CurWinStartY, HI_S32 s32CurWinEndX, HI_S32 s32CurWinEndY)
{
    HI_S32 x0,x1,y0,y1;
...
static inline HI_S32 _DtvCC_CheckWindowOverlap(HI_U8 u8WindowID, HI_U8 flag)
{
    ...
    (HI_VOID)_DtvCC_CheckWindowOverlap_FindWinMap(u8WindowID, flag, &u8WindowMap);
    if(!u8WindowMap)
    {
        return 0;
    }
    ...
    for(i = 0; i < MAX_CC_WINDOWS; i++)
    {
        if(u8WindowMap & (1<<i))
        {
            (HI_VOID)_DtvCC_CheckWindowOverlap_Run (i, &u8OverlapWindowMap, s32CurWinStartX, s32CurWinStartY, s32CurWinEndX, s32CurWinEndY);
        }
    }
...
/*source monitor*/
static HI_BOOL _DtvCC_DrawRowText_Start (HI_U8 u8WindowID, HI_U8 u8Row, HI_S32 column, HI_BOOL *pbLastUnderlineSpace)
{
    HI_BOOL bRet = HI_FALSE;
    HI_U8 u8ColumnEnd = MAX_CC_COLUMNS;
...
static HI_VOID _DtvCC_DrawRowText_SetPenAttr (HI_U8 u8WindowID, HI_U8 u8Row, HI_S32 column, CC708_PenDef_S *pstPen,
                                                                                CC708_PenDef_S *pstLastPen, HI_BOOL *pbPenColorChanged, HI_BOOL *pbLastUnderlineSpace)
{
    if(CCWindow[u8WindowID].astText[u8Row][column].u8IsPenChanged)
    {
    ...
    if((*pbLastUnderlineSpace))
    {
        pstPen = &CCWindow[u8WindowID].astText[u8Row][column].stPen;
        (HI_VOID)CC708_Pen_SetTextUnderline(pstPen,0,1);
        (*pbLastUnderlineSpace) = HI_FALSE;
    }

    return;
}

static HI_VOID _DtvCC_DrawRowText_IncCharCnt (HI_U8 u8WindowID, HI_U8 u8Row, HI_S32 column, HI_BOOL *pbLeadingSpace,
                                                                                            HI_BOOL *pbFirstColumn, HI_U8 *pu8Clear, HI_U16 *pau16Buf, HI_S32 *ps32CharCount)
{
       ...
       (CCWindow[u8WindowID].astText[u8Row][column].u16Caption != 0x1020))
    {
        ...
        {
            *pbLeadingSpace = HI_TRUE;

            if(column == 0)
            {
                *pbFirstColumn = HI_TRUE;
            }
            else
            {
                *pbFirstColumn = HI_FALSE;
            }
            ...
            if((column != 0) &&
               ...
               (CCWindow[u8WindowID].astText[u8Row][column-1].u16Caption == 0x1021)))
            {
                *pu8Clear = 1; //DrawLeadingSpace(u8WindowID,1,bFirstColumn);
            }
            else
            {
                *pu8Clear = 2; //DrawLeadingSpace(u8WindowID,2,bFirstColumn);
            }
        }

        pau16Buf[(*ps32CharCount)++] = CCWindow[u8WindowID].astText[u8Row][column].u16Caption;
    }

    return;
}

static HI_VOID _DtvCC_DrawRowText_LastPrepare(HI_BOOL bAddLeadingSpaceWidth, HI_S32 *ps32TotalWidth, HI_S32 s32SpaceCount, HI_S32 s32SpaceWidth,
                                                                                            HI_BOOL bLeadingSpace, HI_BOOL bFirstColumn, HI_BOOL *pbIgnoreLeftBG)
{
    if(bAddLeadingSpaceWidth)
    {
        *ps32TotalWidth += LEADINGSPACEWIDTH;
...
static HI_VOID _DtvCC_DrawRowText(HI_U8 u8WindowID,HI_U16 u16StartX, HI_U16 u16StartY, HI_U8 u8Row, HI_BOOL bOffScreen)
{
    ...
    for(column = u8ColumnBegin; column < u8ColumnEnd; column++)
    {
        ...
        if (bRet)
        {
            ...
            (HI_VOID)_DtvCC_DrawRowText_SetPenAttr (u8WindowID, u8Row, column, pstPen, pstLastPen, &bPenColorChanged, &bLastUnderlineSpace);
        }

        /* group characters into text buffer, increase character count */
        _DtvCC_DrawRowText_IncCharCnt(u8WindowID, u8Row, column, &bLeadingSpace, &bFirstColumn, &bClear, &au16Buf[0], &s32CharCount);
    }

    /* draw last text */
    if((s32CharCount!=0) &&(pstPen->enTextTag != CC_TT_NON_DISPLAYED_TEXT))
    {
        (HI_VOID)_DtvCC_DrawRowText_LastPrepare(bAddLeadingSpaceWidth, &s32TotalWidth, s32SpaceCount, s32SpaceWidth,
                                                            bLeadingSpace, bFirstColumn, &bIgnoreLeftBG);
...
static HI_BOOL CC708_OSD_UpdateWindow_Unchg (HI_U8 u8WindowID,HI_U8 u8Priority,HI_U8 u8AnchorPoint,HI_U8 u8RelativePositioning,
                                          HI_U8 u8AnchorVertical,HI_U8 u8AnchorHorizontal,HI_U8 u8RowCount,HI_U8 u8ColumnCount,
                                          HI_U8 u8IsVisible,HI_U8 u8WindowStyleID, HI_U8 u8PenStyleID)
{
...
static HI_BOOL CC708_OSD_UpdateWindow_Chg (HI_U8 u8WindowID, HI_U8 u8AnchorPoint,HI_U8 u8RelativePositioning,
                                          HI_U8 u8AnchorVertical,HI_U8 u8AnchorHorizontal,HI_U8 u8RowCount,HI_U8 u8ColumnCount)
{
...
static HI_S32 CC708_OSD_UpdateWindow (HI_U8 u8WindowID,HI_U8 u8Priority,HI_U8 u8AnchorPoint,HI_U8 u8RelativePositioning,
                                          HI_U8 u8AnchorVertical,HI_U8 u8AnchorHorizontal,HI_U8 u8RowCount,HI_U8 u8ColumnCount,
                                          HI_U8 u8RowLock,HI_U8 u8ColumnLock,HI_U8 u8IsVisible,HI_U8 u8WindowStyleID, HI_U8 u8PenStyleID)
{
    HI_BOOL bCmpRet = HI_FALSE;
                                          ...
                                          HI_U8 u8RowLock,HI_U8 u8ColumnLock,HI_U8 u8IsVisible,HI_U8 u8WindowStyleID, HI_U8 u8PenStyleID)
{
    ...
    {
        (HI_VOID)CC708_OSD_UpdateWindow (u8WindowID, u8Priority, u8AnchorPoint, u8RelativePositioning,
                                           u8AnchorVertical, u8AnchorHorizontal, u8RowCount, u8ColumnCount,
