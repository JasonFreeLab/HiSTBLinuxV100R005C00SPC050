...
)
{
    ...
    if ((0 == pstDocument->u32HeaderLen)
        || (u32FileSize < pstDocument->u32HeaderLen))
    {
        COM_PrintErrCode(COM_ERR_IMAGE_ILLEGAL);
        return COM_ERR_IMAGE_ILLEGAL;
    }
    ...
    if (pstDocument->u32HeaderLen >= pstDocument->u32FileLen)
    {
        COM_PrintErrCode(COM_ERR_IMAGE_ILLEGAL);
        return COM_ERR_IMAGE_ILLEGAL;
    }
    ...
    for (i = 0; i < pstDocument->u16ManuNum; ++i)
    {
        ...
        for (j = 0; j < pstManuInfo->u16FlashMapNum; ++j)
        {
            ...
            if (0 == pstFlashMap->u32ImageLength)
            {
                COM_PrintErrCode(COM_ERR_IMAGE_ILLEGAL);
                return COM_ERR_IMAGE_ILLEGAL;
            }
            ...
            if ((pstDocument->u32HeaderLen > pstFlashMap->u32ImageOffset)
                || (pstDocument->u32FileLen <= pstFlashMap->u32ImageOffset))
            {
                COM_PrintErrCode(COM_ERR_IMAGE_ILLEGAL);
                return COM_ERR_IMAGE_ILLEGAL;
            }
...
#endif
            if (pstFlashMap->u64PartitionStartAddr >= pstFlashMap->u64PartitionEndAddr)
            {
                COM_PrintErrCode(COM_ERR_IMAGE_ILLEGAL);
                return COM_ERR_IMAGE_ILLEGAL;
            }
        ...
        if (pstDocument->u32FileLen - pstDocument->u32HeaderLen < pstManuInfo->u32ImageLength)
        {
            COM_PrintErrCode(COM_ERR_IMAGE_ILLEGAL);
            return COM_ERR_IMAGE_ILLEGAL;
        }
