...
//HISILICON add begin
//tag, the memory from mmz, no from framebuffer
#ifdef CONFIG_DFB_SUPPORT_MMZ
     allocation->force_release = false;
#endif
//HISILICON add end
...
//tag, the memory from mmz, no from framebuffer
#ifdef CONFIG_DFB_SUPPORT_MMZ
     if (!locks || (locks && allocation->force_release)){
          if (locks && allocation->force_release){
         ...
         D_DEBUG_AT( Core_SurfAllocation, "%s(): allocation is lock\n", __FUNCTION__);
     }
#else
     if (!locks)
     {
