...
//HISILICON add begin
//tag, unlock the events mutex immediatly after the event pipe is released Fix a deadlock risk: while releasing event buffer,
//       but _fusion_reactor_process_message coming(such as key event...etc) deadlock may happen between lock_node and events_mutex
    direct_mutex_unlock( &data->events_mutex );
