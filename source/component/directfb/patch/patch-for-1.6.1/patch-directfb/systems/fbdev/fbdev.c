...
static void dfb_fbdev_var_to_mode( const struct fb_var_screeninfo *var, VideoMode *mode);
static const VideoMode *dfb_fbdev_find_mode(int width, int height);
static DFBResult dfb_fbdev_test_mode(const VideoMode *mode,const CoreLayerRegionConfig *config);
static DFBResult dfb_fbdev_test_mode_simple(const VideoMode *mode );
static DFBResult dfb_fbdev_set_mode(const VideoMode *mode,CoreSurface *surface,unsigned int xoffset,unsigned int yoffset);
...
//HISILICON add begin
//tag, support fence sync
static DFBResult system_fence_init(bool shutdown)
{
    int num_index = 0;
...
//tag, support fence sync
     ...
     {
         dfb_fbdev->current_fence_index = (dfb_fbdev->current_fence_index + 1) % MAX_FB_NUM;
     }
     else
     {
         dfb_fbdev->current_fence_index = (dfb_fbdev->current_fence_index + 1) % (MAX_FB_NUM - 1);
     }
//HISILICON add end
...
DFBResult dfb_fbdev_fence_sync_wait(int timeout, int buffer_id)
{
...
#else
    if (dfb_fbdev->current_fence_index > (MAX_FB_NUM - 1))
    {
...
DFBResult dfb_fbdev_quene_free_buffer_id(int *pbufferid)
{
    HI_U32 BufferNum = 0;
...
DFBResult dfb_fbdev_quene_buffer_id(unsigned int buffer, int *pbufferid)
{
...
DFBResult dfb_fbdev_quene_buffnum(int *pbuffer_num)
{
