...
HI_S32 ADP_GenerateTDEOpt(void *dev, TDE2_OPT_S* pTDEOpt, HI_BOOL bScale)
{
    ...
    {
        HI_U32 u32Alpha = (tdev->color_pixel >> 24);

        /*use colorize to realize premulti global alpha*/
        stOpt.enAluCmd |= TDE2_ALUCMD_COLORIZE;
        stOpt.u32Colorize = (u32Alpha << 24 | u32Alpha << 16 | u32Alpha << 8 | u32Alpha);
    }
    ...
    if (tdev->blittingflags & DSBLIT_COLORIZE)
    {
        ...
        if (tdev->blittingflags & DSBLIT_SRC_PREMULTCOLOR)
        {
            HI_U8 u8Alpha = (tdev->color_pixel >> 24);
            HI_U8 u8R = (tdev->color_pixel & 0xff0000) >> 16;
            HI_U8 u8G = (tdev->color_pixel & 0xff00) >> 8;
            HI_U8 u8B = (tdev->color_pixel & 0xff);

            MODULATE(u8R, u8Alpha);
            MODULATE(u8G, u8Alpha);
            MODULATE(u8B, u8Alpha);

            stOpt.u32Colorize = (((HI_U32)u8Alpha) << 24) | (((HI_U32)u8R) << 16) | (((HI_U32)u8G) << 8) | (HI_U32)u8B;
        }
    }
