                  ...
                  void       *screen_data )
{
     ...
     if (ioctl( dfb_fbdev[PRIMARYLAYER]->fd, FBIOGET_VBLANK_HIFB))
          waitretrace();
...
static DFBResult
dfb_fbdev_set_opacity(u8 opacity, int num)
{
    HIFB_ALPHA_S Alpha;
    ...
    FBDev **dfb_fbdev = dfb_system_data();
    if (ioctl(dfb_fbdev[num]->fd, FBIOPUT_ALPHA_HIFB,  &Alpha) <0 )
    {
...
static DFBResult
dfb_fbdev_set_colorkey(DFBColorKey *pKey, HI_U32 bEnable, int num)
{
    ...
    FBDev **dfb_fbdev = dfb_system_data();
    if (ioctl(dfb_fbdev[num]->fd, FBIOPUT_COLORKEY_HIFB,  &struColorKey) <0 )
    {
...
static DFBResult
dfb_fbdev_setscreensize(DFBRectangle *pDstRectangle, int num)
{
    ...
    FBDev **dfb_fbdev = dfb_system_data();
    if (ioctl(dfb_fbdev[num]->fd, FBIOPUT_SCREEN_ORIGIN_HIFB,  &struPos) <0 )
    {
    ...
    if (ioctl(dfb_fbdev[num]->fd, FBIOPUT_SCREENSIZE,  &screensize) <0 )
    {
        D_PERROR( "DirectFB/FBDev: Could not set layer size!\n" );
        return errno2result(errno);
    }
#else /*unsupport set screen size*/
    if((pDstRectangle->w != dfb_fbdev[num]->shared->modes->xres)||
        (pDstRectangle->h != dfb_fbdev[num]->shared->modes->yres))
    {
                  ...
                  CoreSurfaceBufferLock      *right_lock)
{
    ...
    {
        ret = dfb_fbdev_set_opacity(config->opacity, num);
        if(ret != DFB_OK)
        {
    ...
    {
        ret = dfb_fbdev_set_colorkey(&(config->src_key), HI_TRUE, num);
        if(ret != DFB_OK)
        {
    ...
    {
        ret = dfb_fbdev_set_colorkey(&(config->dst_key), HI_TRUE, num);
        if(ret != DFB_OK)
        {
    ...
    {
        ret = dfb_fbdev_setscreensize(&(config->dest), num);
        if(ret != DFB_OK)
        {
...
static DFBResult
hi_dfb_fbdev_pan( int xoffset, int yoffset, bool onsync, int num )
{
     int result;
    ...
    if(first)
    {
     ...
     if (ioctl( dfb_fbdev[num]->fd, FBIOPAN_DISPLAY, var ) < 0) {
          result = errno;
                     ...
                     CoreSurfaceBufferLock *right_lock )
{
    ...
    if (layer)
    {
     ...
     FBDevShared  *shared = dfb_fbdev[num]->shared;
     var = &shared->current_var;
     hi_dfb_fbdev_pan( var->xoffset, var->yoffset, 0, num);
