...
DFBResult
dfb_vt_initialize( int nlayernum )
{
     DFBResult ret;
     struct vt_stat vs;
...
dfb_vt_shutdown( bool emergency )
{
     ...
     D_FREE( dfb_vt );
     dfb_vt = dfb_fbdev[PRIMARYLAYER]->vt = NULL;
     ...
     while (true) {
          ...
          switch (dfb_vt->vt_sig) {
               ...
               case SIG_SWITCH_FROM:
                    if (dfb_core_suspend( dfb_fbdev[PRIMARYLAYER]->core ) == DFB_OK) {
               ...
               case SIG_SWITCH_TO:
                    if (dfb_core_resume( dfb_fbdev[PRIMARYLAYER]->core ) == DFB_OK) {
                         if (ioctl( dfb_vt->fd, VT_RELDISP, VT_ACKACQ ) < 0)
                              D_PERROR( "DirectFB/fbdev/vt: VT_RELDISP failed\n" );
...
vt_get_fb( int vt )
{
     ...
     if (ioctl( dfb_fbdev[PRIMARYLAYER]->fd, FBIOGET_CON2FBMAP, &c2m )) {
...
vt_set_fb( int vt, int fb )
{
     ...
     if (fstat( dfb_fbdev[PRIMARYLAYER]->fd, &sbf )) {
     ...
     if (ioctl( dfb_fbdev[PRIMARYLAYER]->fd, FBIOPUT_CON2FBMAP, &c2m ) < 0) {
          D_PERROR( "DirectFB/FBDev/vt: "
