...
//HISILICON add begin
//[TAG]:add Add header file of log fun and use mmz  log
#include "fbdev.h"

extern FBDevMmzInfo dfb_mmz_info;
...
*/
      ...
      */
//HISILICON add begin
//[TAG]: if we use mmz memory pool don't Adjust the offset of the heap
#ifndef  CFG_MEMPOOL_SIZE     
...
//HISILICON add begin
//[TAG]:  if we use mmz memory pool don't need to update pool length 
#ifndef CFG_MEMPOOL_SIZE 
     if (!c->next) {
...
//HISILICON add begin
//[TAG]: output log to file
#ifdef CFG_DEBUG_MMZ 
  #ifndef CFG_MEMPOOL_FB
      direct_log_printf(dfb_mmz_info.mmz_log, "[mmz-pool] free mem, available: %d, free length: %d\n", manager->avail, chunk->length );
  #else
      direct_log_printf(dfb_mmz_info.mmz_log, "[fb-pool] free mem, available: %d, free length: %d\n", manager->avail, chunk->length );
  #endif
#endif
//HISILICON add end
...
//[TAG]:  output log to file
   ...
   #ifndef  CFG_MEMPOOL_FB
          if(allocation != NULL){
            direct_log_printf(dfb_mmz_info.mmz_log, "[mmz-pool]  allocate mem available: %d, allocate length: %d\n", manager->avail, chunk->length);
          }
    #else
          if(allocation != NULL){
           direct_log_printf(dfb_mmz_info.mmz_log, "[fb-pool]  allocate mem available: %d, allocate length: %d\n", manager->avail, chunk->length);
          }
      #endif
//HISILICON add end
#endif
