...
DFBResult
dfb_agp_initialize( int nlayernum )
{
     AGPShared     *shared;
     ...
     if (!dfb_agp_capable( dfb_fbdev[nlayernum]->shared->pci.bus,
                           dfb_fbdev[nlayernum]->shared->pci.dev,
                           dfb_fbdev[nlayernum]->shared->pci.func ))
     ...
     shared = SHCALLOC( dfb_fbdev[nlayernum]->shared->shmpool, 1, sizeof(AGPShared) );
     if (!shared) {
     ...
     dfb_fbdev[nlayernum]->agp = dfb_agp;
     dfb_fbdev[nlayernum]->shared->agp = shared;
...
error2:
     close( dfb_agp->fd );
error1:
     SHFREE( dfb_fbdev[nlayernum]->shared->shmpool, shared );
error0:
     D_FREE( dfb_agp );
     dfb_agp = NULL;
...
dfb_agp_join( void )
{
     ...
     shared = dfb_fbdev[PRIMARYLAYER]->shared->agp;
     if (!shared)
          return DFB_OK;
     ...
     dfb_fbdev[PRIMARYLAYER]->agp = dfb_agp;
...
dfb_agp_shutdown( void )
{
     ...
     SHFREE( dfb_fbdev[PRIMARYLAYER]->shared->shmpool, shared );
     D_FREE( dfb_agp );

     dfb_fbdev[PRIMARYLAYER]->shared->agp = NULL;
     dfb_fbdev[PRIMARYLAYER]->agp = dfb_agp = NULL;
