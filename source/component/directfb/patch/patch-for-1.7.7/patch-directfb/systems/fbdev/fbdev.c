...
static DFBResult system_fence_init(bool shutdown, int layer_id)
{
    ...
    if (false == shutdown)
    {
       for (num_index = 0; num_index < MAX_FB_NUM; num_index++)
       {
...
//HISILICON add end
     ...
     {
          D_PERROR("DirectFB/FBDev: " "Could not get fixed screen information!\n" );
          goto error;
     }
     ...
     /* Map the framebuffer */
     dfb_fbdev[layer_id]->framebuffer_base = mmap( NULL, shared->fix.smem_len,PROT_READ | PROT_WRITE, MAP_SHARED,dfb_fbdev[layer_id]->fd, 0 );
     if (dfb_fbdev[layer_id]->framebuffer_base == MAP_FAILED)
     {
          D_PERROR("DirectFB/FBDev: " "Could not mmap the framebuffer!\n");
     ...
     {
          D_PERROR( "DirectFB/FBDev: ""Could not get variable screen information!\n" );
          goto error;
     }
     ...
     {
          D_PERROR( "DirectFB/FBDev: ""Could not disable console acceleration!\n" );
          goto error;
     }
     ...
     {
          D_DEBUG( "DirectFB/FBDev: ""Could not retrieve palette for backup!\n" );
     ...
     fusion_call_init(&shared->fbdev_ioctl,fbdev_ioctl_call_handler, NULL, dfb_core_world(core) );
...
//HISILICON add end
     ...
     /* Map the framebuffer */
     dfb_fbdev[layer_id]->framebuffer_base = mmap( NULL, dfb_fbdev[layer_id]->shared->fix.smem_len,PROT_READ | PROT_WRITE, MAP_SHARED,dfb_fbdev[layer_id]->fd, 0 );
     if (dfb_fbdev[layer_id]->framebuffer_base == MAP_FAILED)
     {
          D_PERROR( "DirectFB/FBDev: ""Could not mmap the framebuffer!\n");
          close( dfb_fbdev[layer_id]->fd);
...
//HISILICON add end
     ...
     {
          D_PERROR("DirectFB/FBDev: " "Could not restore variable screen information!\n" );
     }
     ...
     if (shared->orig_cmap.len)
     {
          ...
          {
               D_DEBUG("DirectFB/FBDev: " "Could not restore palette!\n");
          }
     }
                   ...
                   DFBScreenDescription *description )
{
     ...
     snprintf(description->name,DFB_SCREEN_DESC_NAME_LENGTH, "FBDev Primary Screen" );
...
dfb_fbdev_pan( int xoffset, int yoffset, bool onsync,int numlayer )
{
     ...
     FBDevShared  *shared = dfb_fbdev[numlayer]->shared;
     CoreLayerRegionConfig *config = &dfb_fbdev[numlayer]->shared->config;
...
//tag, support fence sync
     ...
     dfb_fbdev_quene_buffer_id(numlayer,layer_info.u32LayerAddr,&(dfb_fbdev[numlayer]->current_fence_index));
     ...
     {
         dfb_fbdev[numlayer]->current_fence_index = (dfb_fbdev[numlayer]->current_fence_index + 1) % MAX_FB_NUM;
     }
     else
     {
         dfb_fbdev[numlayer]->current_fence_index = (dfb_fbdev[numlayer]->current_fence_index + 1) % (MAX_FB_NUM - 1);
     }
#endif
//HISILICON add end
...
DFBResult dfb_fbdev_fence_sync_wait(int timeout, int layer_num, int buffer_id)
{
#ifndef CONFIG_DFB_POLL_BUFFER
	int to = -1;
    int ret = -1;
#endif
...
#ifdef CONFIG_DFB_POLL_BUFFER
    if (dfb_fbdev[layer_num]->fence_fd[buffer_id] < 0)
    {
       return DFB_OK;
    }
    close(dfb_fbdev[layer_num]->fence_fd[buffer_id]);
    dfb_fbdev[layer_num]->fence_fd[buffer_id] = -1;
#else
    if (dfb_fbdev[layer_num]->current_fence_index > (MAX_FB_NUM - 1))
    {
        D_WARN("+++++++++++++++ the buffer num is large to 3\n");
        return DFB_OK;
    }

	if (dfb_fbdev[layer_num]->fence_fd[dfb_fbdev[layer_num]->current_fence_index] < 0)
    {
		return DFB_OK;
	}

    ret = ioctl(dfb_fbdev[layer_num]->fence_fd[dfb_fbdev[layer_num]->current_fence_index], SYNC_IOC_WAIT, &to);
	if (ret < 0)
    {
		D_PERROR("dfb_fbdev_fence_sync_wait error!, wait: 0x%x\n", ret);
	}

	close(dfb_fbdev[layer_num]->fence_fd[dfb_fbdev[layer_num]->current_fence_index]);
    dfb_fbdev[layer_num]->fence_fd[dfb_fbdev[layer_num]->current_fence_index] = -1;
#endif
...
DFBResult dfb_fbdev_quene_free_buffer_id(int layernum,int *pbufferid)
{
    ...
    HIFB_HWC_LAYERINFO_S layer_info;
    CoreLayerRegionConfig *config = &dfb_fbdev[layernum]->shared->config;
    ...
    layer_info.eFmt = HIFB_FMT_ARGB8888;
    if (ioctl(dfb_fbdev[layernum]->fd, FBIO_HWC_GETLAYERINFO, &layer_info) < 0)
    {
    	D_PERROR("FBIO_HWC_GETLAYERINFO ERROR==%d==\n", __LINE__);
    	return DFB_OK;
    }

    if (DLBM_TRIPLE == config->buffermode)
    {
       BufferNum = MAX_FB_NUM;
    }
    else
    {
       BufferNum = MAX_FB_NUM - 1;
    }
...
DFBResult dfb_fbdev_quene_buffer_id(int layernum,unsigned int buffer, int *pbufferid)
{
    ...
    FBDevShared  *shared = dfb_fbdev[layernum]->shared;
                    ...
                    unsigned int             yoffset, int num )
{
     ...
     FBDevShared               *shared     = dfb_fbdev[num]->shared;
...
{
     int             i;
     struct fb_cmap *cmap = &dfb_fbdev[layerindex]->shared->current_cmap;
...
dfb_fbdev_set_rgb332_palette( int num )
{
     ...
     int blue_val;
     int i = 0;
     FusionSHMPoolShared *pool = dfb_fbdev[num]->shared->shmpool_data;
