...
{
    QString tmpPath = QT_LIBDIR_EGL;
    if(!qEnvironmentVariableIsEmpty("QT_LIBDIR_EGL")){
        tmpPath = QString::fromLatin1(qgetenv("QT_LIBDIR_EGL"));
    }
    base::FilePath libEGLPath = QtWebEngineCore::toFilePath(tmpPath);
    libEGLPath = libEGLPath.Append("libEGL.so.1");
    base::NativeLibrary eglLibrary = LoadLibrary(libEGLPath);
    if (!eglLibrary)
        return false;

    tmpPath = QT_LIBDIR_GLES2;
    if(!qEnvironmentVariableIsEmpty("QT_LIBDIR_GLES2")){
        tmpPath = QString::fromLatin1(qgetenv("QT_LIBDIR_GLES2"));
    }
    base::FilePath libGLES2Path = QtWebEngineCore::toFilePath(tmpPath);
