	  ...
	  unsigned char ch, unsigned char ch_offset, unsigned short bw_mode)
{
	...
	struct mlme_ext_priv *pmlmeext = &padapter->mlmeextpriv;
	/* according to channel plan */
	u8 max_chan_nums = pmlmeext->max_chan_nums;
	int i;

	scan_info->channel_num = max_chan_nums;
	...
	for (i = 0 ; i < scan_info->channel_num ; i++) {
		if (pmlmeext->channel_set[i].ScanType == SCAN_ACTIVE)
			scan_info->ssid_channel_info[i].active = 1;
		else if (pmlmeext->channel_set[i].ScanType == SCAN_PASSIVE)
			scan_info->ssid_channel_info[i].active = 0;

		scan_info->ssid_channel_info[i].timeout = 100;

		scan_info->ssid_channel_info[i].channel = pmlmeext->channel_set[i].ChannelNum;

		/* OFDM rate for 5G */
		if (scan_info->ssid_channel_info[i].channel >= 36)
			scan_info->ssid_channel_info[i].tx_power =
				PHY_GetTxPowerIndex(padapter, RF_PATH_A, MGN_6M, bw_mode, scan_info->ssid_channel_info[i].channel);
		/* CCK rate for 5G */
		else
			scan_info->ssid_channel_info[i].tx_power =
				PHY_GetTxPowerIndex(padapter, RF_PATH_A, MGN_1M, bw_mode, scan_info->ssid_channel_info[i].channel);
	}
...
void rtw_dev_pno_debug(struct net_device *net)
{
	...
	for (i = 0 ; i < pwrctl->pscan_info->channel_num ; i++) {
