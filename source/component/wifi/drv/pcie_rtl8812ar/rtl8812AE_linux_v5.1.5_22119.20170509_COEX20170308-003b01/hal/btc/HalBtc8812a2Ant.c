...
u32	glcoex_ver_date_8812a_2ant = 20170308;
u32	glcoex_ver_8812a_2ant = 0x3b01;
/* linux coding style
...
VOID
halbtc8812a2ant_set_gpio_for_mailbox(
	...
	)
{
	...
	h2c_parameter[0] = num << 2;

	BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE, "[BTCoex], Set mailbox GPIO = 0x%x\n", num);
	BTC_TRACE(trace_buf);
	BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE, "[BTCoex], FW write 0x70=0x%x\n", h2c_parameter[0]);
	BTC_TRACE(trace_buf);
		...
		IN boolean force_exec, IN u8 type)
{
	...
	case 9:
		halbtc8812a2ant_coex_table(btcoexist, force_exec,
				   0xaaaa5a5a, 0x55555555, 0xf0ffffff, 0xb);
		break;
	case 10:
		halbtc8812a2ant_coex_table(btcoexist, force_exec,
	...
	*/
	if (turn_on) {
		...
		case 23:
			...
			break;
		case 29:
			halbtc8812a2ant_set_fw_pstdma(btcoexist, 0x61, 0x15, 0x03, 0x11, 0x11);
			break;
		case 30: /*d3, wb,TXpause*/
			halbtc8812a2ant_set_fw_pstdma(btcoexist, 0xe3, 0x14, 0x03, 0x10, 0x94);
			break;
		case 31: /*d3, pb,TXpause*/
			halbtc8812a2ant_set_fw_pstdma(btcoexist, 0x63, 0x14, 0x03, 0x10, 0x94);
			break;
		case 32:
			halbtc8812a2ant_set_fw_pstdma(btcoexist, 0x61, 0x35, 0x03, 0x11, 0x11);
			break;
		case 71:
			halbtc8812a2ant_set_fw_pstdma(btcoexist, 0xe3,
						      0x1a, 0x1a, 0xe1, 0x90);
		...
		case 86:
			...
			break;
		case 87:
			halbtc8812a2ant_set_fw_pstdma(btcoexist, 0x51, 0x14, 0x03, 0x10, 0x54);
			break;
		case 88:
			halbtc8812a2ant_set_fw_pstdma(btcoexist, 0x53, 0x14, 0x03, 0x10, 0x54);
			break;		
		}
	} else {
		/* disable PS tdma */
...
void halbtc8812a2ant_action_bt_inquiry(IN struct btc_coexist *btcoexist)
{
	...
	if (coex_sta->wifi_is_high_pri_task && (!bt_link_info->a2dp_exist)) {
		halbtc8812a2ant_coex_table_with_type(btcoexist, NORMAL_EXEC, 10);
		halbtc8812a2ant_ps_tdma(btcoexist, NORMAL_EXEC, true, 32);
	} else {
		halbtc8812a2ant_coex_table_with_type(btcoexist, NORMAL_EXEC, 0);
		halbtc8812a2ant_ps_tdma(btcoexist, NORMAL_EXEC, true, 29);
	}
...
void halbtc8812a2ant_action_bt_relink(IN struct btc_coexist *btcoexist)
{
	...
	if (anttype == 0) { /* ANTTYPE = 0   92E 2ant with SPDT */
		if ((coex_sta->scan_ap_num > NOISY_AP_NUM_THRESH_8812A)) {
			...
			if (BTC_RSSI_HIGH(wifi_rssi_state) &&
			    (!BTC_RSSI_LOW(bt_rssi_state))) {
				/* WIFI RSSI = high & BT RSSI = high & shielding room */
				halbtc8812a2ant_ps_tdma(btcoexist, NORMAL_EXEC, true, 88);
				halbtc8812a2ant_coex_table_with_type(btcoexist,
							     NORMAL_EXEC, 7);
			} else {	/* WIFI RSSI || BT RSSI == low */
				halbtc8812a2ant_ps_tdma(btcoexist, NORMAL_EXEC, true, 88);
...
void halbtc8812a2ant_action_wifi_multi_port(IN struct btc_coexist *btcoexist)
{		
	halbtc8812a2ant_ps_tdma(btcoexist, NORMAL_EXEC, FALSE, 0);
	halbtc8812a2ant_coex_table_with_type(btcoexist, NORMAL_EXEC, 9);
}
/* **************************************************** */
void halbtc8812a2ant_run_coexist_mechanism(IN struct btc_coexist *btcoexist)
{
	...
	if ((num_of_wifi_link >= 2) ||
		...
		btcoexist->btc_set(btcoexist, BTC_SET_BL_MIRACAST_PLUS_BT,
				   &miracast_plus_bt);
		halbtc8812a2ant_action_wifi_multi_port(btcoexist);
	...
	if (coex_sta->is_setup_link) {
		...
		halbtc8812a2ant_action_bt_relink(btcoexist);
		return;
	}
				    ...
				    IN boolean back_up)
{
	...
	//set GPIO2 for HW mailbox
	halbtc8812a2ant_set_gpio_for_mailbox(btcoexist, 0x1);
}
...
void ex_halbtc8812a2ant_periodical(IN struct btc_coexist *btcoexist)
{
	...
	if (firstrun == true) {
		/*enable PTA*/
		u32tmp = btcoexist->btc_read_4byte(btcoexist, 0x40);
		u32tmp |= BIT(5);
		btcoexist->btc_write_4byte(btcoexist, 0x40, u32tmp);
...
/*If wifi is connecting, the update of wifi channel mask may fail caused by wifi FW*/
	...
	if (coex_sta->cnt_wifi_high_pri > 0) {
		coex_sta->cnt_wifi_high_pri--;
		if ((coex_sta->cnt_wifi_high_pri == 0) && (coex_sta->wifi_is_high_pri_task))
			coex_sta->wifi_is_high_pri_task = false;
	}
