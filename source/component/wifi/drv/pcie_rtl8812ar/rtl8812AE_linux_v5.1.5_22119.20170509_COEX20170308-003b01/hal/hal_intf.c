...
void rtw_hal_pci_dbi_write(_adapter *padapter, u16 addr, u8 data)
{
	...
	padapter->HalFunc.SetHwRegHandler(padapter, HW_VAR_DBI, (u8 *) cmd);
}

u8 rtw_hal_pci_dbi_read(_adapter *padapter, u16 addr)
{
	padapter->HalFunc.GetHwRegHandler(padapter, HW_VAR_DBI, (u8 *)(&addr));
...
void rtw_hal_pci_mdio_write(_adapter *padapter, u8 addr, u16 data)
{
	...
	padapter->HalFunc.SetHwRegHandler(padapter, HW_VAR_MDIO, (u8 *) cmd);
}

u16 rtw_hal_pci_mdio_read(_adapter *padapter, u8 addr)
{
	padapter->HalFunc.GetHwRegHandler(padapter, HW_VAR_MDIO, &addr);
...
u8 rtw_hal_pci_l1off_nic_support(_adapter *padapter)
{
	...
	padapter->HalFunc.GetHwRegHandler(padapter, HW_VAR_L1OFF_NIC_SUPPORT, &l1off);
	return l1off;
}
...
u8 rtw_hal_pci_l1off_capability(_adapter *padapter)
{
	...
	padapter->HalFunc.GetHwRegHandler(padapter, HW_VAR_L1OFF_CAPABILITY, &l1off);
	return l1off;
}
