...
void mpt_SetRFPath_8812A(PADAPTER pAdapter)
{
	...
	case ANTENNA_B:
		...
		/*Tim patch 20170216*/
		if (pHalData->RFEType == 1 && IS_HARDWARE_TYPE_8812(pAdapter)) {
			RTW_INFO("TX: RFEType == 1: ant B switch to WiFi\n");
	...
	case ANTENNA_B:
		...
		/*Tim patch 20170216*/
		if (pHalData->RFEType == 1 && IS_HARDWARE_TYPE_8812(pAdapter)) {
			RTW_INFO("RX: RFEType == 1: ant B switch to WiFi\n");
		...
		if (IS_HARDWARE_TYPE_8812(pAdapter)) {
			if (channel <= 14) {
				if (bandwidth == CHANNEL_WIDTH_20
				    && eLNA_2g == 0) {
					/* 0x830[3:1]=3'b010 */
					//201723 patch by Haus for sensitivity
					PHY_SetBBReg(pAdapter, rPwed_TH_Jaguar, BIT1 | BIT2 | BIT3, 0x03);
				} else
					/* 0x830[3:1]=3'b100 */
					PHY_SetBBReg(pAdapter, rPwed_TH_Jaguar, BIT1 | BIT2 | BIT3, 0x04);
			} else
