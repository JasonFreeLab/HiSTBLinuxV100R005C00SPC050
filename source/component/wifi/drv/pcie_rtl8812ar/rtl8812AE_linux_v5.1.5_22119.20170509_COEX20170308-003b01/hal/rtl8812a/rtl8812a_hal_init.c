...
)
{
	...
	#ifdef RTW_HUAWEI_CUSTOMIZATION
                log_err_code = WIFI_EXCE_FW_DL_SZ_EXCEED_ERR;
	#endif
		goto exit;
	}
...
#ifdef RTW_HUAWEI_CUSTOMIZATION
                if (rtStatus != _SUCCESS)
                        log_err_code = WIFI_EXCE_FW_DL_CHKSUM_ERR;
#endif
	}
	...
	if (_SUCCESS != rtStatus){
		goto fwdl_stat;
	#ifdef RTW_HUAWEI_CUSTOMIZATION
                log_err_code = WIFI_EXCE_FW_DL_POLLING_ERR;
	#endif
	}
fwdl_stat:
...
#ifdef RTW_HUAWEI_CUSTOMIZATION
        if (rtStatus != _SUCCESS)
                rtw_drv_log_evt_report(Adapter, WIFI_EXCEPTION_FW_DL_ERR, log_err_code);
#endif
	if (pFirmware)
