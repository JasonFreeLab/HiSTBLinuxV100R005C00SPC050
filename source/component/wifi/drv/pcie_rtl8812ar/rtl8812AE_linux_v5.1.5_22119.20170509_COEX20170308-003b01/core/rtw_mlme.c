...
#define RTW_ROAM_SCAN_INTERVAL_MS (10*1000)
	...
	_rtw_memset(&pmlmepriv->evt_tab, 0, sizeof(struct mlme_log_evt_tab));
	pmlmepriv->evt_tab.log_evt_en = RTW_MLME_DRV_LOG_EN;
#endif
...
#ifdef RTW_HUAWEI_CUSTOMIZATION
	if (mlmepriv->evt_tab.evt_num > 0)
		report_drv_log_redirect_evt(adapter);		
#endif
...
{
	rtw_drv_log_redirect_to_file(padapter);
}
#endif
