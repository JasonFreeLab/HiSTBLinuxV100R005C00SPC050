...
#ifdef RTW_HUAWEI_CUSTOMIZATION
extern unsigned char ARTHEROS_OUI1[];
extern unsigned char ARTHEROS_OUI2[];

extern unsigned char BROADCOM_OUI1[];
extern unsigned char BROADCOM_OUI2[];
extern unsigned char BROADCOM_OUI3[];

extern unsigned char CISCO_OUI[];
extern unsigned char MARVELL_OUI[];
extern unsigned char RALINK_OUI[];
extern unsigned char REALTEK_OUI[];
extern unsigned char AIRGOCAP_OUI[];

#define CONFIG_RTW_DRV_LOG_REDIRECT	1
#define SECURITY_TYPE_WPA_TKIP 1
#define SECURITY_TYPE_WPA_AES 2
#define SECURITY_TYPE_WPA2_TKIP 3
#define SECURITY_TYPE_WPA2_AES 4
#define SECURITY_TYPE_WEP 5
#define SECURITY_TYPE_OPEN 6
void rtw_drv_log_exception_info(_adapter *padapter, u32 offset)
{
	...
	bool wpa_exist = _FALSE;
	bool rsn_exist = _FALSE;
	u8 WPA_TKIP[] = {0x00, 0x50, 0xf2, 0x02};
	u8 WPA_AES[] = {0x00, 0x50, 0xf2, 0x04};
	u8 WPA2_TKIP[] = {0x00, 0x0f, 0xac, 0x02};
	u8 WPA2_AES[] = {0x00, 0x0f, 0xac, 0x04};
	u8 ap_num_per_ch[13][4];
	u8 ch_cnt = 1;
	u8 same_ssid_cnt = 0;
	u8 output_redirect = 0;//CONFIG_RTW_DRV_LOG_REDIRECT;
	u8 *addr = NULL;
        NDIS_802_11_SSID *ssid = NULL;

	if(output_redirect) {
		addr = pmlmepriv->evt_tab.evt_entries[offset].bssid;
		ssid = &pmlmepriv->evt_tab.evt_entries[offset].ssid;
	} else {
		addr = pmlmeinfo->network.MacAddress;
		ssid = &pmlmeinfo->network.Ssid;
	}

	memset((void *)ap_num_per_ch, 0, sizeof(u8)*13*4);
	//RTW_PRINT"!!!!!!!!enter rtw_drv_log_exception_info!!!!!!!!!!\n");
	//RTW_PRINT"ssid = %s\n", ssid->Ssid);
	//RTW_PRINT"%d:bssid %02x %02x %02x %02x %02x %02x\n",__LINE__,addr[0],addr[1],addr[2],addr[3],addr[4],addr[5]);
	phead = get_list_head(&pmlmepriv->scanned_queue);
	plist = get_next(phead);

	//RTW_PRINT"!!!!!!!!!!!!!!!!!phead = %p, plist = %p!!!!!!!!!!!!!!!!!!\n", phead, plist);

	while (plist != phead) {
		pnetwork = LIST_CONTAINOR(plist, struct wlan_network ,list);
		if ((s32)pnetwork->network.Rssi > -55)
			ap_num_per_ch[pnetwork->network.Configuration.DSConfig-1][3]++;
		else if ((s32)pnetwork->network.Rssi < -55 && ((s32)pnetwork->network.Rssi > -70))
			ap_num_per_ch[pnetwork->network.Configuration.DSConfig-1][2]++;
		else if ((s32)pnetwork->network.Rssi < -70 && ((s32)pnetwork->network.Rssi > -100))
			ap_num_per_ch[pnetwork->network.Configuration.DSConfig-1][1]++;
		else
			ap_num_per_ch[pnetwork->network.Configuration.DSConfig-1][0]++;

		if (_rtw_memcmp(addr, pnetwork->network.MacAddress, ETH_ALEN) == _TRUE) {
			ap_ch = pnetwork->network.Configuration.DSConfig;
			if (!output_redirect) {
				RTW_PRINT("AP INFO\n");
				RTW_PRINT("bssid %02x:%02x:%02x:%02x:%02x:%02x\n",addr[0],addr[1],addr[2],addr[3],addr[4],addr[5]);
				RTW_PRINT("ssid %s\n", pnetwork->network.Ssid.Ssid);
				RTW_PRINT("ap channel = %d\n", ap_ch);
				RTW_PRINT("rssi = %d\n", (s32)pnetwork->network.Rssi);
			} else {
				pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].ap_channel = ap_ch;
				pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].rssi = (s32)pnetwork->network.Rssi;
			}

			for (i = sizeof(NDIS_802_11_FIXED_IEs); i < pnetwork->network.IELength;) {
				pIE = (PNDIS_802_11_VARIABLE_IEs)(pnetwork->network.IEs+ i);
				...
				switch (pIE->ElementID)
				{
					case _VENDOR_SPECIFIC_IE_:
						if ((_rtw_memcmp(pIE->data, ARTHEROS_OUI1, 3)) || (_rtw_memcmp(pIE->data, ARTHEROS_OUI2, 3))) {
							if (!output_redirect)
								RTW_PRINT("vendor: Artheros AP\n");
							else
								pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].ap_vendor = HT_IOT_PEER_ATHEROS;
						} else if (	(_rtw_memcmp(pIE->data, BROADCOM_OUI1, 3))
								|| (_rtw_memcmp(pIE->data, BROADCOM_OUI2, 3))
								|| (_rtw_memcmp(pIE->data, BROADCOM_OUI3, 3))) {
							if (!output_redirect)
								RTW_PRINT("vendor: Broadcom AP\n");
							else
								pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].ap_vendor = HT_IOT_PEER_BROADCOM;
						} else if (_rtw_memcmp(pIE->data, MARVELL_OUI, 3)) {
							if (!output_redirect)
								RTW_PRINT("vendor: Marvell AP\n");
							else
								pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].ap_vendor = HT_IOT_PEER_MARVELL;
						} else if (_rtw_memcmp(pIE->data, RALINK_OUI, 3)) {
							if (!output_redirect)
								RTW_PRINT("vendor: Ralink AP\n");
							else
								pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].ap_vendor = HT_IOT_PEER_RALINK;
						} else if (_rtw_memcmp(pIE->data, CISCO_OUI, 3)) {
							if (!output_redirect)
								RTW_PRINT("vendor: Cisco AP\n");
							else
								pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].ap_vendor = HT_IOT_PEER_CISCO;
						} else if (_rtw_memcmp(pIE->data, REALTEK_OUI, 3)) {
							if (!output_redirect)
								RTW_PRINT("vendor: Realtek AP\n");
							else
								pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].ap_vendor = HT_IOT_PEER_REALTEK;
						} else if (_rtw_memcmp(pIE->data, AIRGOCAP_OUI,3)) {
							if (!output_redirect)
								RTW_PRINT("vendor: Airgo Cap\n");
							else
								pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].ap_vendor = HT_IOT_PEER_AIRGO;
						} else if (_rtw_memcmp(pIE->data, RTW_WPA_OUI, 4)) {
							wpa_exist = _TRUE;
							if (_rtw_memcmp((pIE->data+12), WPA_TKIP, 4)) {
								if (!output_redirect)
									RTW_PRINT("security: WPA TKIP\n");
								else
									pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].security = SECURITY_TYPE_WPA_TKIP;
							} else if (_rtw_memcmp((pIE->data+12), WPA_AES, 4)) {
								if (!output_redirect)
									RTW_PRINT("security: WPA AES\n");
								else
									pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].security = SECURITY_TYPE_WPA_AES;
							}
						}
						break;
					case _RSN_IE_2_:
						rsn_exist = _TRUE;
						if (_rtw_memcmp((pIE->data+8), WPA2_TKIP, 4)) {
							if (!output_redirect)
								RTW_PRINT("security: WPA2 TKIP\n");
							else
								pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].security = SECURITY_TYPE_WPA2_TKIP;
						} else if (_rtw_memcmp((pIE->data+8), WPA2_AES, 4)) {
							if (!output_redirect)
								RTW_PRINT("security: WPA2 AES\n");
							else
								pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].security = SECURITY_TYPE_WPA2_AES;
						}
						break;
			...
			if (wpa_exist == _FALSE && rsn_exist == _FALSE) {
				if (pnetwork->network.Privacy) {
					if (!output_redirect)
						RTW_PRINT("security: WEP\n");
					else
						pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].security = SECURITY_TYPE_WEP;
				} else {
					if (!output_redirect)
						RTW_PRINT("security: OPEN\n");
					else
						pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].security = SECURITY_TYPE_OPEN;
				}
			}
			...
			if ( _rtw_memcmp(ssid, &pnetwork->network.Ssid, sizeof(NDIS_802_11_SSID)) == _TRUE) {
                        	same_ssid_cnt++;
				//RTW_PRINT"same ssid = %s\n", pnetwork->network.Ssid.Ssid);
	                }
		}
	...
	if (!output_redirect)
		RTW_PRINT("same_ssid_cnt = %d\n", same_ssid_cnt);
	else
		pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].same_ssid_cnt = same_ssid_cnt;

	for(;ch_cnt <= 13;ch_cnt++) {
		if (!output_redirect)
			RTW_PRINT("%sch %d: [4] %d [3] %d [2] %d [1] %d\n",(ch_cnt == ap_ch)? "*":" ", ch_cnt, ap_num_per_ch[ch_cnt-1][3],
			ap_num_per_ch[ch_cnt-1][2], ap_num_per_ch[ch_cnt-1][1], ap_num_per_ch[ch_cnt-1][0]);
		else {
			pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].ap_num_per_ch[ch_cnt-1][3] = ap_num_per_ch[ch_cnt-1][3];
			pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].ap_num_per_ch[ch_cnt-1][2] = ap_num_per_ch[ch_cnt-1][2];
			pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].ap_num_per_ch[ch_cnt-1][1] = ap_num_per_ch[ch_cnt-1][1];
			pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].ap_num_per_ch[ch_cnt-1][0] = ap_num_per_ch[ch_cnt-1][0];
		}
	}

}
void rtw_drv_log_evt_report(_adapter *padapter, u8 evt_id, int err_code)
{
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
	struct mlme_ext_priv	*pmlmeext = &(padapter->mlmeextpriv);
	struct mlme_ext_info	*pmlmeinfo = &(pmlmeext->mlmext_info);
	...
	u8 is_p2p_connected = _FALSE;
#endif
	u8 output_redirect = 0;//CONFIG_RTW_DRV_LOG_REDIRECT;

	if (!padapter || !pmlmepriv)
		return;

	if (!pmlmepriv->evt_tab.log_evt_en)
		return;

#if 0//def CONFIG_P2P  //todo
		if (is_primary_adapter(padapter)) {
			//_adapter *pbuddy = padapter->pbuddy_adapter;
			pwdinfo = &(pbuddy->wdinfo);

			p2p_role = rtw_p2p_role(pwdinfo);
			if ((( rtw_p2p_role(pwdinfo) == P2P_ROLE_CLIENT ) || ( rtw_p2p_role(pwdinfo) == P2P_ROLE_GO))
				&& (rtw_mi_buddy_check_fwstate(padapter, _FW_LINKED) == _TRUE))
				is_p2p_connected = _TRUE;
		}
#endif

	if (output_redirect) {
		if (pmlmepriv->evt_tab.evt_num < RTW_MLME_MAX_LOG_ENTRIES ) {
			pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].evt_id= evt_id;
			pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].err_code = err_code;
			_rtw_memcpy(&(pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].ssid), &(pmlmeinfo->network.Ssid), sizeof( NDIS_802_11_SSID));
			_rtw_memcpy(pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].bssid, get_my_bssid(&pmlmeinfo->network), ETH_ALEN);
			rtw_drv_log_exception_info(padapter, pmlmepriv->evt_tab.evt_num);
#ifdef CONFIG_P2P
			if (is_primary_adapter(padapter)) {
				pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].p2p_role = p2p_role;
				pmlmepriv->evt_tab.evt_entries[pmlmepriv->evt_tab.evt_num].p2p_connected = is_p2p_connected;
			}
#endif
			pmlmepriv->evt_tab.evt_num++;
		} else
			RTW_PRINT("%s event entry is full, do not record\n", __func__);

	} else {
		pmlmepriv->evt_tab.evt_num = 0;
		switch (evt_id) {
			case WIFI_EXCEPTION_ASSOC:
			case WIFI_EXCEPTION_AUTH:
				RTW_PRINT("exception during %s %s(err=%d)\n",
					(is_primary_adapter(padapter)) ? "WiFi" : "P2P",
					(evt_id == WIFI_EXCEPTION_ASSOC) ? "association" : "authentication" ,err_code);
				RTW_PRINT("BSSID: "MAC_FMT"\n", MAC_ARG(get_my_bssid(&pmlmeinfo->network)));
				rtw_drv_log_exception_info(padapter, 0);
#ifdef CONFIG_P2P
			if (is_primary_adapter(padapter)) {
				if (p2p_role ==  P2P_ROLE_DEVICE)
					RTW_PRINT("P2P device is not connected\n");
				else if ((p2p_role == P2P_ROLE_CLIENT ) || (p2p_role == P2P_ROLE_GO))
					RTW_PRINT("P2P %s is %s\n",
						(p2p_role == P2P_ROLE_GO) ? "GO" : "GC",
						(is_p2p_connected == _TRUE) ? "connected" : "connecting");
				else
					RTW_PRINT("P2P inactive\n");
			}
#endif
				break;
			case WIFI_EXCEPTION_USB_BUS_ERR:
				RTW_PRINT("DSM_WIFI_SDIO_USB_BUS_ERROR(err=%d)\n", err_code);
				break;

            case WIFI_EXCEPTION_PCI_BUS_ERR:
                RTW_PRINT("DSM_WIFI_PCI_BUS_ERROR: ");
                switch (err_code) {
                    case WIFI_EXCE_PCI_TXDMA_ERR:
                        RTW_PRINT("TXDMA_ERROR\n");
                        break;
                    case WIFI_EXCE_PCI_RXDMA_ERR:
                        RTW_PRINT("RXDMA_ERROR\n");
                        break;
                    default:
                        break;
                }
			...
			case WIFI_EXCEPTION_FW_DL_ERR:
				RTW_PRINT("DSM_WIFI_FIRMWARE_DL_ERROR: ");
				switch (err_code) {
					case WIFI_EXCE_FW_DL_SZ_EXCEED_ERR:
						RTW_PRINT("firmware size exceed\n");
						break;
					case WIFI_EXCE_FW_DL_CHKSUM_ERR:
						RTW_PRINT("firmware checksum error\n");
						break;
					case WIFI_EXCE_FW_DL_POLLING_ERR:
						RTW_PRINT("firmware polling ready\n");
						break;
					default:
						break;
				}
				break;
			case WIFI_EXCEPTION_DRV_CMD_ERR:
				RTW_PRINT("DSM_WIFI_DRV_CMD_ERROR: ");
				switch (err_code) {
						case WIFI_NL80211_CMD_NEW_KEY_ERR:
							RTW_PRINT("WIFI_EXCEPTION_DRV_NL80211_CMD_NEW_KEY_ERR\n");
							break;
						case WIFI_NL80211_CMD_GET_STA_ERR:
							RTW_PRINT("WIFI_EXCEPTION_DRV_NL80211_CMD_GET_STATION_ERR\n");
							break;
						case WIFI_NL80211_CMD_SET_INTF_ERR:
							RTW_PRINT("WIFI_EXCEPTION_DRV_NL80211_CMD_SET_INTERFACE_ERR\n");
							break;
						case WIFI_NL80211_CMD_TRIGGER_SCAN_ERR:
							RTW_PRINT("WIFI_EXCEPTION_DRV_NL80211_CMD_TRIGGER_SCAN_ERR\n");
							break;
						case WIFI_NL80211_CMD_CONNECT_ERR:
							RTW_PRINT("WIFI_EXCEPTION_DRV_NL80211_CMD_CONNECT_ERR\n");
							break;
						case WIFI_NL80211_CMD_SET_PMKSA_ERR:
							RTW_PRINT("WIFI_EXCEPTION_DRV_NL80211_CMD_SET_PMKSA_ERR\n");
							break;
						default:
							break;
				}
...
#define CONFIG_DRV_LOG_FILE_PATH	"/data/log/wifi/wifi_abnormal.log"
void rtw_drv_log_redirect_to_file(_adapter *padapter)
{
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
	struct file *fp = NULL;
	mm_segment_t oldfs;
	...
	if (!pmlmepriv->evt_tab.evt_num)
		return;

	if (!pmlmepriv->evt_tab.log_evt_en)
		return;

	fp = filp_open(CONFIG_DRV_LOG_FILE_PATH, O_CREAT|O_WRONLY|O_APPEND, 0666);
				if (IS_ERR(fp)) {
		RTW_PRINT("file %s open fail\n", CONFIG_DRV_LOG_FILE_PATH);
		return ;
				}
	...
	for (offset = 0; offset < pmlmepriv->evt_tab.evt_num; offset++) {
		_rtw_memset(line, 0, sizeof(line));
		switch (pmlmepriv->evt_tab.evt_entries[offset].evt_id) {
			case WIFI_EXCEPTION_ASSOC:
			case WIFI_EXCEPTION_AUTH:
				RTW_PRINT("1st str_len = %d\n",str_len);
				str_len = sprintf(line, "exception during %s %s(err=%d)\n",
					(is_primary_adapter(padapter)) ? "WiFi" : "P2P",
					(pmlmepriv->evt_tab.evt_entries[offset].evt_id == WIFI_EXCEPTION_ASSOC) ? "association" : "authentication",
					pmlmepriv->evt_tab.evt_entries[offset].err_code);

				str_len += sprintf(line + str_len, "BSSID %02x:%02x:%02x:%02x:%02x:%02x, ", pmlmepriv->evt_tab.evt_entries[offset].bssid[0],pmlmepriv->evt_tab.evt_entries[offset].bssid[1],pmlmepriv->evt_tab.evt_entries[offset].bssid[2], pmlmepriv->evt_tab.evt_entries[offset].bssid[3], pmlmepriv->evt_tab.evt_entries[offset].bssid[4], pmlmepriv->evt_tab.evt_entries[offset].bssid[5]);
				str_len += sprintf(line + str_len, "SSID %s\n",pmlmepriv->evt_tab.evt_entries[offset].ssid.Ssid);
				str_len += sprintf(line + str_len, "ap channel ch%d, ", pmlmepriv->evt_tab.evt_entries[offset].ap_channel);
				str_len += sprintf(line + str_len, "RSSI %ddBm\n", pmlmepriv->evt_tab.evt_entries[offset].rssi);
				if (pmlmepriv->evt_tab.evt_entries[offset].ap_vendor == HT_IOT_PEER_ATHEROS)
					str_len += sprintf(line + str_len, "ap vendor Atheros\n");
				else if (pmlmepriv->evt_tab.evt_entries[offset].ap_vendor == HT_IOT_PEER_BROADCOM)
					str_len += sprintf(line + str_len, "ap vendor Broadcom\n");
				else if (pmlmepriv->evt_tab.evt_entries[offset].ap_vendor == HT_IOT_PEER_MARVELL)
					str_len += sprintf(line + str_len, "ap vendor Marvell\n");
				else if (pmlmepriv->evt_tab.evt_entries[offset].ap_vendor == HT_IOT_PEER_RALINK)
					str_len += sprintf(line + str_len, "ap vendor Ralink\n");
				else if (pmlmepriv->evt_tab.evt_entries[offset].ap_vendor == HT_IOT_PEER_CISCO)
					str_len += sprintf(line + str_len, "ap vendor Cisco\n");
				else if (pmlmepriv->evt_tab.evt_entries[offset].ap_vendor == HT_IOT_PEER_REALTEK)
					str_len += sprintf(line + str_len, "ap vendor Realtek\n");
				else if (pmlmepriv->evt_tab.evt_entries[offset].ap_vendor == HT_IOT_PEER_AIRGO)
					str_len += sprintf(line + str_len, "ap vendor Airgo Cap\n");
				else
					str_len += sprintf(line + str_len, "ap vendor Unknown\n");

				if (pmlmepriv->evt_tab.evt_entries[offset].security == SECURITY_TYPE_WPA_TKIP)
					str_len += sprintf(line + str_len, "security WPA TKIP\n");
				else if (pmlmepriv->evt_tab.evt_entries[offset].security == SECURITY_TYPE_WPA_AES)
					str_len += sprintf(line + str_len, "security WPA AES\n");
				else if (pmlmepriv->evt_tab.evt_entries[offset].security == SECURITY_TYPE_WPA2_TKIP)
					str_len += sprintf(line + str_len, "security WPA2 TKIP\n");
				else if (pmlmepriv->evt_tab.evt_entries[offset].security == SECURITY_TYPE_WPA2_AES)
					str_len += sprintf(line + str_len, "security WPA2 AES\n");
				else if (pmlmepriv->evt_tab.evt_entries[offset].security == SECURITY_TYPE_WEP)
					str_len += sprintf(line + str_len, "security WEP\n");
				else if (pmlmepriv->evt_tab.evt_entries[offset].security == SECURITY_TYPE_OPEN)
					str_len += sprintf(line + str_len, "security OPEN\n");
				else
					str_len += sprintf(line + str_len, "security UNKNOWN\n");

				str_len += sprintf(line + str_len, "same ssid cnt %d\n", pmlmepriv->evt_tab.evt_entries[offset].same_ssid_cnt);

				for(;ch_cnt <= 13;ch_cnt++) {
					str_len += sprintf(line + str_len, "%sch %d: [4] %d [3] %d [2] %d [1] %d\n",
					(ch_cnt == pmlmepriv->evt_tab.evt_entries[offset].ap_channel)? "*":" ", ch_cnt,
					pmlmepriv->evt_tab.evt_entries[offset].ap_num_per_ch[ch_cnt-1][3],
					pmlmepriv->evt_tab.evt_entries[offset].ap_num_per_ch[ch_cnt-1][2],
					pmlmepriv->evt_tab.evt_entries[offset].ap_num_per_ch[ch_cnt-1][1],
					pmlmepriv->evt_tab.evt_entries[offset].ap_num_per_ch[ch_cnt-1][0]);
				}
				ch_cnt = 1;
#ifdef CONFIG_P2P
				if (is_primary_adapter(padapter)) {
					if (pmlmepriv->evt_tab.evt_entries[offset].p2p_role ==  P2P_ROLE_DEVICE)
						str_len += sprintf(line + str_len, "P2P device is not connected\n");
					else if ((pmlmepriv->evt_tab.evt_entries[offset].p2p_role == P2P_ROLE_CLIENT ) ||
						(pmlmepriv->evt_tab.evt_entries[offset].p2p_role == P2P_ROLE_GO))
						str_len += sprintf(line + str_len, "P2P %s is %s\n",
							(pmlmepriv->evt_tab.evt_entries[offset].p2p_role == P2P_ROLE_GO) ? "GO" : "GC",
							(pmlmepriv->evt_tab.evt_entries[offset].p2p_connected == _TRUE) ? "connected" : "connecting");
					else
						str_len += sprintf(line + str_len, "P2P inactive\n");
				}
#endif
				RTW_PRINT("2nd str_len = %d\n",str_len);
				/*debug only*/
				if (1)
					RTW_PRINT("====\n log:%s =====\n", line);
				break;
			case WIFI_EXCEPTION_USB_BUS_ERR:
				str_len = sprintf(line, "DSM_WIFI_SDIO_USB_BUS_ERROR(err=%d)\n", pmlmepriv->evt_tab.evt_entries[offset].err_code);
				break;
			case WIFI_EXCEPTION_FW_DL_ERR:
				switch (pmlmepriv->evt_tab.evt_entries[offset].err_code) {
					case WIFI_EXCE_FW_DL_SZ_EXCEED_ERR:
						str_len = sprintf(line, "DSM_WIFI_FIRMWARE_DL_ERROR: firmware size exceed\n");
						break;
					case WIFI_EXCE_FW_DL_CHKSUM_ERR:
						str_len = sprintf(line, "DSM_WIFI_FIRMWARE_DL_ERROR: firmware checksum error\n");
						break;
					case WIFI_EXCE_FW_DL_POLLING_ERR:
						str_len = sprintf(line, "DSM_WIFI_FIRMWARE_DL_ERROR: firmware polling ready\n");
						break;
					default:
						break;
				}
			...
			case WIFI_EXCEPTION_DRV_CMD_ERR:
				switch (pmlmepriv->evt_tab.evt_entries[offset].err_code) {
						case WIFI_NL80211_CMD_NEW_KEY_ERR:
							str_len = sprintf(line, "DSM_WIFI_DRV_CMD_ERROR: WIFI_EXCEPTION_DRV_NL80211_CMD_NEW_KEY_ERR\n");
							break;
						case WIFI_NL80211_CMD_GET_STA_ERR:
							str_len = sprintf(line, "DSM_WIFI_DRV_CMD_ERROR: WIFI_EXCEPTION_DRV_NL80211_CMD_GET_STATION_ERR\n");
							break;
						case WIFI_NL80211_CMD_SET_INTF_ERR:
							str_len = sprintf(line, "DSM_WIFI_DRV_CMD_ERROR: WIFI_EXCEPTION_DRV_NL80211_CMD_SET_INTERFACE_ERR\n");
							break;
						case WIFI_NL80211_CMD_TRIGGER_SCAN_ERR:
							str_len = sprintf(line, "DSM_WIFI_DRV_CMD_ERROR: WIFI_EXCEPTION_DRV_NL80211_CMD_TRIGGER_SCAN_ERR\n");
							break;
						case WIFI_NL80211_CMD_CONNECT_ERR:
							str_len = sprintf(line, "DSM_WIFI_DRV_CMD_ERROR: WIFI_EXCEPTION_DRV_NL80211_CMD_CONNECT_ERR\n");
							break;
						case WIFI_NL80211_CMD_SET_PMKSA_ERR:
							str_len = sprintf(line, "DSM_WIFI_DRV_CMD_ERROR: WIFI_EXCEPTION_DRV_NL80211_CMD_SET_PMKSA_ERR\n");
							break;
						default:
							break;
				}
		...
		if (fp->f_op->write) {
			RTW_PRINT("3rd str_len = %d\n",str_len);
			fp->f_op->write(fp, line, str_len, &fp->f_pos);
		}
}

	set_fs(oldfs);
	filp_close(fp, NULL);
	_rtw_memset(&pmlmepriv->evt_tab.evt_entries, 0x0, sizeof(struct mlme_log_evt_entry));
	pmlmepriv->evt_tab.evt_num = 0;
}
#endif	/*RTW_HUAWEI_CUSTOMIZATION*/
...
int proc_get_ap_info(struct seq_file *m, void *v)
{
...
#ifdef RTW_HUAWEI_CUSTOMIZATION
	if (!psta || (check_fwstate(pmlmepriv, _FW_LINKED) == _FALSE)) {
		RTW_PRINT_SEL(m, "under disconecting\n");
		return 0;
	}
	...
	if (psta->wireless_mode & WIRELESS_11_24N)
		_RTW_PRINT_SEL(m, "11n,"); //no prefix
	RTW_PRINT_SEL(m, "\n");

	/*bandwidth*/
	RTW_PRINT_SEL(m, "B:%s\n", (!psta->bw_mode)? "20M" : "40M");

	/*chip vendor*/
	RTW_PRINT_SEL(m, "T:");
	switch (pmlmeinfo->assoc_AP_vendor) {
		case HT_IOT_PEER_ATHEROS:
			RTW_PRINT_SEL(m, "ATHEROS\n");
			break;
		case HT_IOT_PEER_BROADCOM:
			RTW_PRINT_SEL(m, "BROADCOM\n");
			break;
		case HT_IOT_PEER_RALINK:
			RTW_PRINT_SEL(m, "RALINK\n");
			break;
		case HT_IOT_PEER_CISCO:
			RTW_PRINT_SEL(m, "CISCO\n");
			break;
		case HT_IOT_PEER_MARVELL:
			RTW_PRINT_SEL(m, "MARVELL\n");
			break;
		case HT_IOT_PEER_INTEL:
			RTW_PRINT_SEL(m, "INTEL\n");
			break;
		...
		case HT_IOT_PEER_REALTEK_JAGUAR_CCUTAP:
			RTW_PRINT_SEL(m, "REALTEK\n");
			break;
		default:
			RTW_PRINT_SEL(m, "UNKNOWN\n");
			break;
	}
