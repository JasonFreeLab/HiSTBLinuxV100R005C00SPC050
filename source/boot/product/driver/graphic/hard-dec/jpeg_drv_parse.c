...
static HI_BOOL parse_sofn_marker(IMAG_INFO_S *pstImgInfo)
{
    ...
    if (pstImgInfo->s32NumComponent > PIXEL_COMPONENT_NUM)
    {
       return HI_FALSE;
    }
...
static HI_BOOL parse_sos_marker(IMAG_INFO_S *pstImgInfo)
{
    ...
    if (s32NumComponent > PIXEL_COMPONENT_NUM)
    {
...
static inline HI_BOOL DRV_JPEG_CheckIsSupportFactor(IMAG_INFO_S *pstImgInfo)
{
     ...
     if (pstImgInfo->s32ComInScan > PIXEL_COMPONENT_NUM)
     {
        return HI_FALSE;
     }
...
#ifndef HI_BUILD_IN_BOOT
    if ((0 == pstInMsg->u32AllPhy) || (0 == pstInMsg->u32AllLen))
    {
        return HI_FAILURE;
    }
