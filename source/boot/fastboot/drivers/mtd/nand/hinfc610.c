...
static void hinfc610_dma_transfer(struct hinfc_host *host, int todev)
{
	...
	if (DMA_TO_DEVICE == todev) {
		flush_dcache_range((uint32_t)host->dma_buffer, ALIGN((uint32_t)host->dma_buffer + host->pagesize, ARCH_DMA_MINALIGN));
		flush_dcache_range((uint32_t)host->dma_oob, ALIGN((uint32_t)host->dma_oob + host->oobsize, ARCH_DMA_MINALIGN));
	}
	...
	if (DMA_FROM_DEVICE == todev) {
		invalidate_dcache_range((uint32_t)host->dma_buffer, ALIGN((uint32_t)host->dma_buffer + host->pagesize, ARCH_DMA_MINALIGN));
		invalidate_dcache_range((uint32_t)host->dma_oob, ALIGN((uint32_t)host->dma_oob + host->oobsize, ARCH_DMA_MINALIGN));
	}
}
/*****************************************************************************/
...
static int hinfc610_send_cmd_pageprog(struct hinfc_host *host)
{
	...
	*host->epm = 0x0000;
	...
	hinfc610_dma_transfer(host, DMA_TO_DEVICE);
...
static int hinfc610_do_read_retry(struct hinfc_host *host)
{
	...
	for (ix = 1; IS_PS_UN_ECC(host) && ix < host->read_retry->count; ix++) {
		...
		hinfc610_dma_transfer(host, DMA_FROM_DEVICE);
