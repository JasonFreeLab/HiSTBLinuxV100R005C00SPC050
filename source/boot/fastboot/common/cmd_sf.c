...
static int do_spi_flash_probe(int argc, char *argv[])
{
	...
	printf("OK:%u KiB %s at %u:%u is now current device\n",
...
static int do_spi_flash_read_write(int argc, char *argv[])
{
	...
	if (!buf) {
		puts("ERROR:Failed to map physical memory\n");
		return 1;
	}
	...
	if (strncmp(argv[0], "read", sizeof("read")) == 0) {
		if ((offset + len) > spiflash_info->chipsize) {
			...
			printf("ERROR: read area is out of range, "
			       "length is truncated to 0x%08lx.\n", len);
		}
		ret = spi_flash_read(flash, offset, len, buf);

		printf(" 0x%x bytes %s: %s\n", len,
		       "read", ret ? "ERROR" : "OK");
	} else {
	...
	if (ret) {
		printf("ERROR:SPI flash %s failed\n", argv[0]);
		return 1;
	}
