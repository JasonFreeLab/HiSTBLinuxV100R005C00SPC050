				 ...
				 struct nand_ctrl_info_t *best, int is_hw_auto)
{
	...
	if (pagesize != best->pagesize) {
		...
		if (is_hw_auto) {
			PR_BUG("!!! warning: "
				"Hardware configure pagesize 0x%x, "
...
static void hinfc610_sanely_check(struct nand_dev_t *nand_dev, struct nand_ctrl_info_t *best)
{
	int regval;
...
static void hinfc610_buffer_init(struct hinfc_host *host, struct nand_ctrl_info_t *best)
{
...
static int hinfc610_params_adjust(struct nand_dev_t *nand_dev)
{
	...
	hinfc610_buffer_init(host, best);
	...
	hinfc610_sanely_check(nand_dev, best);
