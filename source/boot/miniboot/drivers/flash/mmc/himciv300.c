...
static void hi_mci_start_tuning(struct mmc_dev_t *mmc_dev, unsigned int min, 
							   unsigned int max, unsigned int mask, unsigned int offset,
							   unsigned int reg_addr,
							   unsigned int *found, unsigned int *tuning_count,
							   unsigned int *raise_point, unsigned int *fall_point,
							   unsigned int datastrobe)
{
...
{
	unsigned int min = 0,max = 0,mask = 0,offset = 0,reg_addr = 0;
...
tuning_start:
	hi_mci_start_tuning(mmc_dev, min, max, mask, offset, reg_addr, &found, &tuning_count, &raise_point, &fall_point, datastrobe);
	if (found) {
		if (-1 == raise_point)
			raise_point = min;
		if (-1 == fall_point)
