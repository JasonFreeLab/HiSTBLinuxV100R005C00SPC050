...
HI_S32 DRV_PQ_SetHDHue(HI_U32 u32Hue)
{
    ...
    if (PQ_FUNC_CALL(HI_PQ_MODULE_HDR,SetHdrCSCVideoSetting))
    {
        GET_ALG_FUN(HI_PQ_MODULE_HDR)->SetHdrCSCVideoSetting(&stPictureSetting);
    }
...
HI_S32 DRV_PQ_GetHdrSaturation(HI_U32 *pu32Saturation)
{
    ...
    if (PQ_FUNC_CALL(HI_PQ_MODULE_HDR, GetHdrSaturation))
    {
...
HI_S32 DRV_PQ_SetHdrSaturation(HI_U32 u32Saturation)
{
    ...
    if (PQ_FUNC_CALL(HI_PQ_MODULE_HDR, SetHdrSaturation))
    {
...
HI_S32 DRV_PQ_SetHdrOffsetPara(HI_PQ_SETHDROFFSET_S *pstHdrOffsetPara)
{
    ...
    if ((pstHdrOffsetPara->u32HdrMode > 1)
        || (pstHdrOffsetPara->u32Bright > 100)
        || (pstHdrOffsetPara->u32Contrast > 100)
        || (pstHdrOffsetPara->u32Satu > 100)
        || (pstHdrOffsetPara->u32Hue > 100)
        || (pstHdrOffsetPara->u32R > 100)
        || (pstHdrOffsetPara->u32G > 100)
        || (pstHdrOffsetPara->u32B > 100)
        || (pstHdrOffsetPara->u32darkCv > 100)
        || (pstHdrOffsetPara->u32brightCv > 100)
        || (pstHdrOffsetPara->u32ACCdark > 100)
        || (pstHdrOffsetPara->u32ACCbrigt > 100))
    {
        HI_ERR_PQ("out of range!\n");
        return HI_FAILURE;
    }
...
HI_S32 DRV_PQ_GetVPPreviewMode(HI_DRV_PQ_VP_MODE_E *penVPSceneMode)
{
    ...
    PQ_CHECK_NULL_PTR(penVPSceneMode);
...
HI_S32 DRV_PQ_UpdateVdpPQ(HI_PQ_DISPLAY_E enDisplayId, HI_VDP_PQ_INFO_S *pstTimingInfo, S_VDP_REGS_TYPE *pstVDPReg)
{
    ...
    if (HI_TRUE == pstTimingInfo->bHdrMode)
    {
        ...
        drv_pq_SetModuleEn(HI_PQ_MODULE_DCI, HI_TRUE);
...
{
    HI_S32 s32Ret = HI_FAILURE;
    if (GET_ALG(HI_PQ_MODULE_DCI) && GET_ALG_FUN(HI_PQ_MODULE_DCI)->GetACCHistGram)
    {
...
HI_S32 DRV_PQ_UpdateACCHistGram(HI_PQ_ACC_HISTGRAM_S *pstACCHistGram)
{
    ...
    if (GET_ALG(HI_PQ_MODULE_HDR) && GET_ALG_FUN(HI_PQ_MODULE_HDR)->SetHdrACCHistGram)
    {
...
{
    if (HI_PQ_SCENE_MODE_BUTT <= enSceneMode)
    {
        HI_ERR_PQ("[%d]hi_mpi_SwitchDrvToUnfVPSceneMode error!\n", enSceneMode);
        return HI_FAILURE;
    }
                                    ...
                                    HI_PQ_HDR_UNIFIED_METADATA_OUT_S *pstHDRInfoOut)
{
    ...
    if (PQ_FUNC_CALL(HI_PQ_MODULE_HDR, GetHdrUnifiedMetadata))
    {
