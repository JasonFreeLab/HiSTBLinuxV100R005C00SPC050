...
enum
{
    VEDU_CAP_LEVEL_QCIF = 0, /**<The resolution of the picture to be decoded is less than or equal to 176x144.*/ /**<CNcomment: �����ͼ���С������176*144 */
    VEDU_CAP_LEVEL_CIF  = 1,      /**<The resolution of the picture to be decoded less than or equal to 352x288.*/ /**<CNcomment: �����ͼ���С������352*288 */
    VEDU_CAP_LEVEL_D1   = 2,       /**<The resolution of the picture to be decoded less than or equal to 720x576.*/ /**<CNcomment: �����ͼ���С������720*576 */
    VEDU_CAP_LEVEL_720P = 3,     /**<The resolution of the picture to be decoded is less than or equal to 1280x720.*/ /**<CNcomment: �����ͼ���С������1280*720 */
    VEDU_CAP_LEVEL_1080P = 4,   /**<The resolution of the picture to be decoded is less than or equal to 1920x1080.*/ /**<CNcomment: �����ͼ���С������1920*1080 */
    VEDU_CAP_LEVEL_BUTT
} ;
...
static HI_U32 H265e_MakeVPS(HI_U8 *pVPSBuf, const VeduEfl_H265e_VPS_S *pVPS)
{
    ...
    VENC_DRV_BsPutBits31(&BS, 0, 3);           //vps_max_sub_layers_minus1 = 0��1  u(3)   ����СP��ȡֵΪ1������ȡֵΪ0
...
static HI_S32 QuickEncode_Process(VeduEfl_EncPara_S* EncHandle, HI_HANDLE GetImgHhd)         //�ɹ�ȡ֡���� HI_SUCCESS,��һ�ζ�ȡ���ɹ�����HI_FAILURE
{
...
static HI_VOID GetFrmRate_Attach( VeduEfl_EncPara_S *pEncPara,HI_UNF_VENC_CHN_ATTR_S stSrcUserCfg)
{
    ...
    {
        if (pEncPara->LastFrmRate[5] == 0)  /*ǰ6 ֡��û������һֱ���������Ǹ�֡��*/
        {
        ...
        else
        {
            ...
            else if (pEncPara->LastSecInputFrmRate[1] != 0) /*ǰ2s֡�ʶ�ͳ�Ƴ�����*/
            {
            ...
            else if (pEncPara->LastSecInputFrmRate[0] != 0)  /*ǰ1s֡���Ѿ�ͳ�Ƴ���*/
            {
                newViFrmRate = pEncPara->LastSecInputFrmRate[0];
            }
            else  /*��һs��֡�ʻ�û��ͳ�Ƴ���*/
...
static HI_VOID VENC_DRV_EflQueryChn_X_Stride(VeduEfl_EncIn_S* pEncIn, VeduEfl_EncPara_S* pEncPara)
{
    ...
    {
       pEncIn->BusViV = pEncPara->stImage.stBufAddr[0].u32PhyAddr_Cr;//pEncPara->stImage.u32CAddr;      //Ŀǰ֡��Ϣ�ṹ��ȱ�ٸýṹ,planer��ʽʱ��Ҫ��
    }

    if ((pEncPara->stHal.YuvStoreType == VENC_STORE_SEMIPLANNAR) && (VENC_YUV_422 == pEncPara->YuvSampleType))  /*==ǿ�ư�SEMIPLANAR_422 ��semiplaner 420����*/
    {
...
{
    if (pEncPara->stImage.u32TunnelPhyAddr)      //����ȷ!!
    {
        pEncPara->stRc.LowDlyMod = HI_TRUE;
        pEncIn->TunlCellAddr = pEncPara->stImage.u32TunnelPhyAddr;
    }
    ...
    if ((pEncPara->stHal.YuvStoreType == VENC_STORE_SEMIPLANNAR) && (pEncPara->YuvSampleType == VENC_YUV_422))  /*==ǿ�ư�SEMIPLANAR_422 ��semiplaner 420����*/
    {
        pEncIn->ViYStride = pEncPara->stImage.stBufAddr[0].u32Stride_Y;
        pEncIn->ViCStride = pEncPara->stImage.stBufAddr[0].u32Stride_C * 2;
    }
...
static HI_VOID VENC_DRV_GetLastFrmInfo_OMX(VeduEfl_EncPara_S* pEncPara, VeduEfl_EncIn_S* pEncIn)
{
    ...
    if (pEncPara->stHal.YuvStoreType == VENC_STORE_PLANNAR)
    {
        pEncIn->BusViV = pEncIn->BusViY + pEncPara->stImage_OMX.offset_YCr;//pEncPara->stImage_OMX.strideC*pEncPara->stImage_OMX.picHeight /2;//pEncPara->stImage.u32CAddr;      //Ŀǰ֡��Ϣ�ṹ��ȱ�ٸýṹ,planer��ʽʱ��Ҫ��
    }
