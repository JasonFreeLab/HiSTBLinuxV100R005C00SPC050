        ...
        for (i = 0; i < pstBufVpssInst->u32BufNum; i++)
        {
            ...
            if (HI_SUCCESS != s32Ret)
            {
                ...
                list_for_each_safe(pos, n, &(pstBufVpssInst->stVpssBufAvailableList))
                {
                    pstTarget = list_entry(pos, BUFMNG_VPSS_NODE_S, node);
                    HI_DRV_VDEC_UnmapAndRelease(&(pstTarget->VDECBuf_frm));
                    HI_DRV_VDEC_UnmapAndRelease(&(pstTarget->VDECBuf_meta));//DRM: meta�ڴ�Ϊ�ǰ��ڴ� sdk
                    list_del_init(pos);
                    HI_VFREE_BUFMNG(pstTarget);
                }
...
static HI_S32 VDEC_Chan_SetExtBuffer(HI_HANDLE hVpss, VDEC_BUFFER_ATTR_PRI_S* pstBufferAttr)
{
    ...
    /*���粥�ų�����bufferType == HI_DRV_VPSS_BUF_USER_ALLOC_MANAGE ʱ,start vpss ��ʱ����Ҫ������������bypass����֮�� add by sdk */
...
static HI_S32 VDEC_Chan_InitParam(HI_HANDLE hHandle, HI_UNF_AVPLAY_OPEN_OPT_S* pstCapParam)
{
    ...
    pstChan->stOption.u32DynamicFrameStoreAllocEn = ((MaskCtrlWord & 0x1) == 1) ? 0 : s_stVdecDrv.astChanEntity[hHandle].u32DynamicFsEn; //sdk ʹ�ܶ�̬���书��
    pstChan->stOption.s32ExtraFrameStoreNum = PLUS_FS_NUM + 1;//sdk PLUS_FS_NUM + 1:�������ŷ�ֹ����
    pstChan->stOption.s32DelayTime = 0;//sdk ��̬����֡��ʱ�������䲻���ڴ�vfmw�ȴ���ʱ��
    pstChan->stOption.u32CfgFrameNum = 0;//sdk ����֡��
    pstChan->stOption.u32MaxMemUse = -1;//sdk������֡��
...
{
#ifdef HI_TEE_SUPPORT
#if ((defined(CHIP_TYPE_hi3798mv200) || defined(CHIP_TYPE_hi3798mv200_a)) && defined(HI_ULTRA_VMX))
    HI_S32 s32Ret;
    HI_U32 u32Arg;
#endif
#endif
    ...
    if (pstChan != HI_NULL)
    {
        ...
        {
            u32Arg = (HI_ID_VDEC << 16 | hHandle);
            s32Ret = (s_stVdecDrv.pVfmwFunc->pfnVfmwControl)(HI_INVALID_HANDLE, VDEC_CID_FREE_CHAN_NOTIFY, &u32Arg, sizeof(HI_HANDLE));
        }
    }
#endif
#endif
...
HI_S32 VDEC_Chan_AllocVFMWMem(HI_HANDLE hHandle)
{
...
#endif
    {
        ...
        if (u32VDHSize > 0)
        {
            ...
            if (1 != pstChan->stOption.u32DynamicFrameStoreAllocEn) //sdk ���ö�̬����ķ�����Ԥ�������������ڲ�֪��������Ҫ��֡���С�����Բ���ȥԤ�ֽڵ㡣
            {
            ...
            if (HI_SUCCESS == s32Ret)
            {
                /* CNcomment:�������Ԥ�����ڴ�ķ�ʽ�����Խ�Ԥ�����ڴ�Ĵ�С��������룬�����ǰ�����Ҫ�Ĵ�С��ֵ*/
...
#endif
            {
                if (1 == pstChan->stOption.u32DynamicFrameStoreAllocEn) //������ö�̬֡��ķ���,�������ﲻ��Ҫ������������ sdk//
                {
                    pstChan->stVDHMMZBuf.u32Size = 0;
                    pstChan->stVDHMMZBuf.u32StartPhyAddr = HI_NULL;
                    pstChan->stVDHMMZBuf.pu8StartVirAddr = HI_NULL;
...
HI_S32 HI_DRV_VDEC_SetUVMOS(HI_HANDLE hHandle, HI_BOOL bUVMOSEnable)
{
    ...
    if (hHandle >= HI_VDEC_MAX_INSTANCE_NEW)
    {
        HI_ERR_VDEC("HI_DRV_VDEC_SetUVMOS err hvdec:%d too large!\n", hHandle);
        return HI_FAILURE;
    }
...
static HI_S32 VDEC_Ioctl_ChanAlloc(struct file*  filp, unsigned int cmd, void* arg)
{
    ...
    if (((VDEC_CMD_ALLOC_S*)arg)->u32DFSEnable > 1)  //sdk
    {
    ...
    s_stVdecDrv.astChanEntity[hHandle].u32DynamicFsEn = ((VDEC_CMD_ALLOC_S*)arg)->u32DFSEnable;  //sdk
