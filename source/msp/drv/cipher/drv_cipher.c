...
HI_S32 DRV_CIPHER_CloseChn(HI_U32 softChnId)
{
    ...
    memset(pSoftChan->stCtrl.u32Key, 0, sizeof(pSoftChan->stCtrl.u32Key));
    ret = HAL_Cipher_SetKey(pSoftChan->u32HardWareChn, &(pSoftChan->stCtrl));
    if (HI_SUCCESS != ret)
    {
        return ret;
    }
...
HI_S32 DRV_CIPHER_ConfigChn(HI_U32 softChnId,  HI_UNF_CIPHER_CTRL_S *pConfig)
{
    ...
    if((pConfig->enAlg == HI_UNF_CIPHER_ALG_3DES)
         && (pConfig->enKeyLen != HI_UNF_CIPHER_KEY_DES_2KEY)
         && (pConfig->enKeyLen != HI_UNF_CIPHER_KEY_DES_3KEY))
    {
         HI_ERR_CIPHER("Cipher config failed!3DES with invalid key len\n");
         return HI_ERR_CIPHER_INVALID_PARA;
    }

    if((pConfig->enAlg == HI_UNF_CIPHER_ALG_AES)
        && (pConfig->enKeyLen > HI_UNF_CIPHER_KEY_AES_256BIT))
    {
         HI_ERR_CIPHER("Cipher config failed!AES with invalid key len\n");
         return HI_ERR_CIPHER_INVALID_PARA;
    }
...
/**********************************************************************
    DCP����Ķ�����HDCP keyԭʼ��ʽ����308Bytes��
    0~4    KSV  �̶�Ϊ "02000000"    Key Selection Vector use to communicate between HDMI Transmitter and Receiver.
    5~7         �̶�Ϊ "000000"
    8~287     Device Private Key    Secret data
    288~307    Reserved    No use

    HISI HDCP Key���߽����뵽��Key���·�װ�������µ�key�ļ���СΪ384���ֽڣ����ֶ��������£�
    ��0~7(8)�ֽ�      ����˼���λ��HISI_xxx
    ��8~15(8)�ֽ�     �����߰汾�ţ�V0000001
    ��16~47(32)�ֽ�   ���ͻ���ID�ţ�xxxxxxxx
    ��48~367(320)�ֽ� �����ܺ��320bytes HDCP key(16�ֽڶ���)��
    ��368~384(16)�ֽ� ������ʹ�ã��ݶ�Ϊ�������

    оƬ�ϼ��ܴ�ŵ��ļ��е�HDCP Key��ʽ
    head              HDMIIP_HDCPKey                 CRC32_0 CRC32_1
    |-------|-----------------------------------------|------|------|
    |4bytes-|-----------------Encrypt(320bytes)-------|-4byte|-4byte|
    ...
    HDCPKey ��ʽ��320 byte
    B0:      �̶�Ϊ0x00;
    B1~B5:   KSV (DCP��֯�����HDCP Key����ȡ��
    B6~B7:   HDCPKey��CRC16��������B6~B8.
    B8��     �̶�Ϊ0xa8;
    B9~B288: Device Private Key (DCP��֯�����HDCP Key����ȡ��
    B289~B319: ���Ϊ 0x14, 0xf7, 0x61, 0x03, 0xb7, 0x59, 0x45, 0xe3, 0x0c, 0x7d, 0xb4, 0x45, 0x19, 0xea, 0x8f, 0xd2,0x89, 0xee, 0xbd, 0x90, 0x21, 0x8b, 0x05, 0xe0,0x4e, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00

    CRC32_0��  ����ǰHDCPKey��CRC32
    CRC32_1��  Head+HDCPKey+CRC32_0��CRC32
*/
static HI_S32 s_DRV_CIPHER_DecryptHDCPKeyBeforeFormat(HI_UNF_HDCP_HDCPKEY_S *pSrcKey,  HI_UNF_HDCP_DECRYPT_S *pDstkey)
{
    ...
    pu8EncryptKey+=8; //������˼���λ��HISI_xxx
    pu8EncryptKey+=32; //�����ͻ���ID�ţ�xxxxxxxx
...
HI_S32 DRV_RSA_RandMaskEnable(CIPHER_RSA_DATA_S *pCipherRsaData, HI_U32 u32KeyLen, HI_U32 u32Rnadom[2], HI_U32 u32EnableCrcCheck)
{
    ...
    if(u32EnableCrcCheck)
    {
        ...
        u32Rnadom[0] = stRNG.u32RNG;

        DRV_CIPHER_GetRandomNumber(&stRNG);
        u32Rnadom[1] = stRNG.u32RNG;
...
//    HI_PRINT_HEX("K", pCipherRsaData->pu8K, pCipherRsaData->u16NLen);
    ...
    ret = DRV_RSA_RandMaskEnable(pCipherRsaData, u32KeyLen, u32Rnadom, u32EnableCrcCheck);
    if( HI_SUCCESS != ret )
    {
    ...
    /*Write N,E,M*/
    HAL_RSA_WriteData(CIPHER_RSA_DATA_TYPE_MODULE, pCipherRsaData->pu8N, pCipherRsaData->u16NLen, u32KeyLen, u32Rnadom);
    HAL_RSA_WriteData(CIPHER_RSA_DATA_TYPE_KEY, pCipherRsaData->pu8K, pCipherRsaData->u16NLen, u32KeyLen, u32Rnadom);
    HAL_RSA_WriteData(CIPHER_RSA_DATA_TYPE_CONTEXT, pCipherRsaData->pu8Input, pCipherRsaData->u16NLen, u32KeyLen, u32Rnadom);
