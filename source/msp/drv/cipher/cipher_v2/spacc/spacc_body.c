...
void SM4_Enable(void)
{
    HI_U32 Value;

    HAL_CIPHER_ReadReg(g_u32Sm4RegBase, &Value);
    HAL_SET_BIT(Value, 8); /* clock opened and cancel reset */
    HAL_CIPHER_WriteReg(g_u32Sm4RegBase, Value);
}

void SM4_Disable(void)
{
    HI_U32 Value;

    HAL_CIPHER_ReadReg(g_u32Sm4RegBase, &Value);
    HAL_CLEAR_BIT(Value, 8); /* reset and clock closed*/
    HAL_CIPHER_WriteReg(g_u32Sm4RegBase, Value);
}
