                    ...
                    int stride, int x, int y)
{
    ...
    /*此处对mv所指向参考区范围作了限制,限回有效区域sdk 20100903*/
...
static int vp56_size_changed(AVCodecContext *avctx)
{
    ...
    /*sdk 20100701 采用av_malloc_hw分配内存 并且不支持size变化*/
                      ...
                      const uint8_t *buf, int buf_size)
{
    ...
    /*sdk 20100901 此处增加指针记录码流起始地址*/
    const uint8_t *uiStartBuf;
    int key_frame;

    avctx->iChromaFormat = 1;

    /*sdk 2011 0331 buf的首地址是cropsize*/
    ...
    //   for (is_alpha=0; is_alpha < 1+s->has_alpha; is_alpha++)
    {
        ...
        /*如果图像宽高变了，则修正为以前的宽高 sdk 20100903*/
        res = s->parse_header(s, buf, buf_size, &golden_frame, &key_frame);

        /*如果图像宽高非法，则返回，重新寻找头 sdk 20100913*/
        if (-1 == res)
        {
            return AVCODEC_RET_DECODE_FAILURE;
        }

        /*如果图像宽高大于通道规格，则无法继续解码 sdk 20110328*/
        if (-2 == res)
        {
        ...
        /*sdk 20100906修改 误码测试，当前一关键帧帧头有错误导致未分配数据时，如下修改：不管是否有错误都分配内存*/
        if (!is_alpha)
        {
        ...
        /*sdk 20100901 此处增加代码，检查码流解析是否越界*/
...
av_cold int vp56_free(AVCodecContext *avctx)
{
    ...
    /*sdk 20100701修改 将av_free 改为av_freep*/
    av_freep(&s->above_blocks);
    av_freep(&s->macroblocks);
    av_freep(&s->edge_emu_buffer_alloc);

    /*释放vlc表内存 sdk 20100928*/
    for (pt = 0; pt < 2; pt++)
    {
