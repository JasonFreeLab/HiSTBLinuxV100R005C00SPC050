...
#define MPEG2_MAX_HEADER_LENGTH 800
        ...
        /*根据c68632检视意见进行修改,当uiNumBytes<3时*puiBytesConsumed会反转 sdk 20100418*/
                            ...
                            switch (ext_type)
                            {
                                ...
                                case 0x3:
                                    mpeg_decode_extension(avctx, pucBufPos, iInputSize);
                                    {
                                        ...
                                        /*根据k68699代码检视意见进行修改，解码扩展可能会失败导致无法获得色散模式 sdk 20100518*/
                                        if (s->chroma_format != 1 && s->chroma_format != 2)
                                        {
                                            iFindVosFlag = 0;
                                            /*sdk 20100607 如果chroma错误 则将指针置空*/
...
#define HEADER_BUF_SIZE 1024
    ...
    /*找到帧头以及成功解码标志位 sdk 20100714*/
    INT32 DecodeSuccess;
    INT32 FindHeader;
    INT32 NumBytes = (INT32)uiNumBytes;
    *puiBytesConsumed = 0; //初始化

    /*初始化各标志位 sdk 20100714*/
    DecodeSuccess = 0;
    FindHeader = 0;
#define FIND_HEADER 1
#define DECODE_SUCCESS 1

    /*sdk 20100714 若找不到正确frame header，则一直找下去，直到码流结束*/
    while (DECODE_SUCCESS != DecodeSuccess)
    {
        ...
        /*调用函数 解析码流头 sdk 20100626*/
        /*vp6 帧头I帧最大为81个比特，P帧最大为43个比特，所以最小要求为11个字节 这里设为20 sdk 20100706*/
#define MIN_VP6_HEADER 20
        ...
        /*最少读900字节进去 去掉0x03 sdk 20100707*/
#define VP6_MAX_HEADER_LENGTH 900
        ...
        if ( RETURN_OK == iRet)
        {
            ...
            {
                /*如果解码头部失败，则将buffer中的起始码去掉，重新开始寻找起始码 sdk 20100716*/
                FindHeader = 0;
                pucBuffer += 3;
                /*问题单号：AZ1D02284
                修改人：熊飞 +00141957
...
/*这里需要考虑到如果是p frame，则没有宽高信息，需要退出重新寻找 sdk 20100714*/
static INT32 VP6_Decodeheader(UINT8 *pucSrcBuf, INT32 iLength, STRU_IVIDEO_STREAM_INFO *pstParams)
{
    INT32 key_frame = 0;
    ...
    /*添加：如果解析成功 则将信息写入pstParams sdk 20100629*/
    pstParams->stAspectRatio.usSarHeight = 1;
    pstParams->stAspectRatio.usSarWidth = 1;
    /*问题单号：AZ1D02316
    修改人：熊飞 +00141957
