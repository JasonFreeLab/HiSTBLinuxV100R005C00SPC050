...
static HI_VOID HIFB_ParseProcEchoCmd(HI_U32 u32LayerId, HI_CHAR *pCmd)
{
    ...
    {
     #ifdef CFG_HIFB_FENCE_SUPPORT
       pstPar->bFenceRefreshWorkQueue = HI_TRUE;
     #endif
       return;
    }
    ...
    {
     #ifdef CFG_HIFB_FENCE_SUPPORT
       pstPar->bFenceRefreshWorkQueue = HI_FALSE;
     #endif
       return;
    }
    ...
    if (par->hifb_sync)
    {
       for (SyncIndex = 0; SyncIndex < HIFB_DISP_BUFFER_NUM_MAX; SyncIndex++)
       {
          PROC_PRINT(p,  "fence information                          \t : [%d][%d][%d,%d]\n",\
                                                                          SyncIndex,         \
                                                                          gs_SyncInfo.ReleaseFenceFd[SyncIndex],\
                                                                          gs_SyncInfo.FenceValue[SyncIndex],    \
                                                                          gs_SyncInfo.TimeLine[SyncIndex]);
       }
    }
#endif
