...
HI_S32 VDP_DRV_SetHdrMode(VDP_HDR_CFG_S *stHdrCfg, rpu_ext_config_fixpt_main_t *pstCompCfg, DmKsFxp_t *pstDmKs)
{
    ...
    if (el_en)
    {
        el_en = VDP_DRV_JudgeWinIsAllScreen(&stOutReso, &stHdrCfg->stVdpDispInfo.stFmtResolution);
    }
    //----------------------------------------------------------------------
    //gfx setting
    //----------------------------------------------------------------------
    ...
    stZmeInfo.stZme2InRect   = stOutReso;//add by h226871
...
HI_BOOL VDP_DRV_JudgeWinIsAllScreen(VDP_RECT_S *pstWinOutRect, VDP_RECT_S *pstScreenRect)
{
    ...
    if ((pstWinOutRect->u32Wth != pstScreenRect->u32Wth)
        || (pstWinOutRect->u32Hgt != pstScreenRect->u32Hgt))
    {
        return HI_FALSE;
    }
