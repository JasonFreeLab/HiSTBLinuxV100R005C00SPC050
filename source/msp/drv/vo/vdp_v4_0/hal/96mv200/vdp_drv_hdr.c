...
HI_S32 VDP_DRV_SetHdrMode(VDP_HDR_CFG_S *stHdrCfg, rpu_ext_config_fixpt_main_t *pstCompCfg, DmKsFxp_t *pstDmKs)
{
	...
	VDP_DRAW_PIXEL_MODE_E penHScalerMode;
	VDP_DRAW_MODE_E   penVScalerMode;
	VDP_PREZME_CONFIG_S pstPreZmeConfig;
	VDP_RECT_S	stInreso = stHdrCfg->stVidReso;
	
	HI_U32 u32ZmeInResoWidth  = stHdrCfg->stVidReso.u32Wth;
	HI_U32 u32ZmeInResoHeight = stHdrCfg->stVidReso.u32Hgt;
    ...
    VDP_VID_SetTileStride(VDP_LAYER_VID0, stHdrCfg->stBlAddr_2LowBit.u32LumStr/32, stHdrCfg->stBlAddr_2LowBit.u32ChmStr/32);
    ...
    if(stHdrCfg->bDcmpEn == HI_TRUE)
    {
        ...
        VDP_VID_SetHeadSize        (VDP_LAYER_VID0, stHdrCfg->stVidReso.u32Hgt * stHdrCfg->stHeadAddr.u32LumStr);
    }
	...
	//on sdk  can't open!!!!
	//stHdrCfg->enDrawMode=VDP_DRAW_MODE_1;
	stHdrCfg->bIoP=1;

	VDP_DRV_GetPreZmeConfig(stInreso,stHdrCfg->stBlReso, &penHScalerMode,&penVScalerMode,&pstPreZmeConfig);

	pstPreZmeConfig.bPreZme=0;


	if(pstPreZmeConfig.bPreZme == HI_TRUE)
	{
		u32ZmeInResoWidth /=2;
	}

	if(penVScalerMode == VDP_DRAW_MODE_2)
	{
		u32ZmeInResoHeight /=2;
	}
	else if(penVScalerMode== VDP_DRAW_MODE_4)
	{
		u32ZmeInResoHeight /=4;
	}
	else if(penVScalerMode== VDP_DRAW_MODE_8)
	{
	...
	VDP_PRINT("penHScalerMode000==%d\n",penHScalerMode);
	
	VDP_PRINT("u32ZmeInResoWidth==%d\n",u32ZmeInResoWidth);

	if(penHScalerMode == VDP_DRAW_PIXEL_MODE_2)
	{
		u32ZmeInResoWidth /=2;
	}
	else if(penHScalerMode == VDP_DRAW_PIXEL_MODE_4)
	{
		u32ZmeInResoWidth /=4;
	}
	else if(penHScalerMode == VDP_DRAW_PIXEL_MODE_8)
	{
		u32ZmeInResoWidth /=8;
	}
	else //no pixel draw
	...
	stInreso.u32Wth = u32ZmeInResoWidth;
	stInreso.u32Hgt = u32ZmeInResoHeight;
	...
	//抽行抽点水平与缩放
	VDP_VID_SetHpzmeHpzmeEn(VDP_LAYER_VID0  ,pstPreZmeConfig.bPreZme);
	VDP_VID_SetHpzmeHpzmeMode(VDP_LAYER_VID0,pstPreZmeConfig.bPreZmeFir);
	
	VDP_VID_SetHpzmeCoef0(VDP_LAYER_VID0,6);
	VDP_VID_SetHpzmeCoef1(VDP_LAYER_VID0,52);
	VDP_VID_SetHpzmeCoef2(VDP_LAYER_VID0,6);
	VDP_VID_SetDrawPixelMode(VDP_LAYER_VID0,penHScalerMode);
    VDP_VID_SetDrawMode (VDP_LAYER_VID0, penVScalerMode, penVScalerMode);
//#if EDA_TEST
	...
	//if((stHdrCfg->stBlReso.u32Wth != stHdrCfg->stBlReso.u32Wth) || (stHdrCfg->stBlReso.u32Hgt != stHdrCfg->stBlReso.u32Hgt))
	{
		...
		VDP_VID_SetZmeHorRatio		   (VDP_LAYER_VID0, (stInreso.u32Wth*ZME_HPREC)/stHdrCfg->stBlReso.u32Wth);
		VDP_VID_SetZmeVerRatio		   (VDP_LAYER_VID0, (stInreso.u32Hgt*ZME_VPREC)/stHdrCfg->stBlReso.u32Hgt);
...
//#ifdef VDP_PQ_STRATEGY
		...
		pstZmeInfo.pstZmeDrvPara.u32ZmeFrmWIn	= stHdrCfg->stElReso.u32Wth;
		pstZmeInfo.pstZmeDrvPara.u32ZmeFrmWOut = stHdrCfg->stBlReso.u32Wth;
		pstZmeInfo.pstZmeDrvPara.u32ZmeFrmHIn	= stHdrCfg->stElReso.u32Hgt;
		pstZmeInfo.pstZmeDrvPara.u32ZmeFrmHOut = stHdrCfg->stBlReso.u32Hgt;
		...
		pstZmeInfo.pstZmeDrvPara.bZmeFrmFmtIn = 1;
		if(stHdrCfg->bIoP==0)
		{
			pstZmeInfo.pstZmeDrvPara.bZmeFrmFmtOut = 0;
		}
		else
		{
			pstZmeInfo.pstZmeDrvPara.bZmeFrmFmtOut = 1;
		}
		...
		{
			 static HI_PQ_ZME_COEF_RATIO_E enHiPqZmeCoefRatio=HI_PQ_ZME_COEF_E1;
		...
		GetVdpZmeCoef(enRationHC,HI_PQ_ZME_COEF_TYPE_CV,HI_PQ_ZME_TAP_4T32P, (HI_S16*)(gstVdpCoefBufAddr.pu8CoefVirAddr[VDP_COEF_BUF_ZME_TMP]));
		...
		GetVdpZmeCoef(HI_PQ_ZME_COEF_TYPE_LV,HI_PQ_ZME_COEF_TYPE_CV,HI_PQ_ZME_TAP_4T32P, (HI_S16*)(gstVdpCoefBufAddr.pu8CoefVirAddr[VDP_COEF_BUF_ZME_TMP]));
		...
		GetVdpZmeCoef(HI_PQ_ZME_COEF_TYPE_CV,HI_PQ_ZME_COEF_TYPE_CV,HI_PQ_ZME_TAP_4T32P, (HI_S16*)(gstVdpCoefBufAddr.pu8CoefVirAddr[VDP_COEF_BUF_ZME_TMP]));
		...
		VDP_VID_SetZmeCoefAddr(VDP_LAYER_VID1, VDP_VID_PARA_ZME_HOR , (gstVdpCoefBufAddr.u32CoefPhyAddr[VDP_COEF_BUF_ZME_V1_H]));
		VDP_VID_SetZmeCoefAddr(VDP_LAYER_VID1, VDP_VID_PARA_ZME_VER , (gstVdpCoefBufAddr.u32CoefPhyAddr[VDP_COEF_BUF_ZME_V1_V]));
		
		VDP_VID_SetParaUpd(VDP_LAYER_VID1,VDP_VID_PARA_ZME_HOR);
		VDP_VID_SetParaUpd(VDP_LAYER_VID1,VDP_VID_PARA_ZME_VER);
	#endif
		...
		if((stHdrCfg->stElReso.u32Wth != stHdrCfg->stBlReso.u32Wth) || (stHdrCfg->stElReso.u32Hgt != stHdrCfg->stBlReso.u32Hgt))
		{
			...
			if(stHdrCfg->bIoP==0)
			{
				...
				VDP_VID_SetZmePhase 			(VDP_LAYER_VID1, VDP_ZME_MODE_VERL, pstZmeRtlPara.s32ZmeOffsetVL);
				VDP_VID_SetZmePhase 			(VDP_LAYER_VID1, VDP_ZME_MODE_VERC, pstZmeRtlPara.s32ZmeOffsetVC);
				VDP_VID_SetZmePhase 			(VDP_LAYER_VID1, VDP_ZME_MODE_VERBL, pstZmeRtlPara.s32ZmeOffsetVLBtm);
				VDP_VID_SetZmePhase 			(VDP_LAYER_VID1, VDP_ZME_MODE_VERBC, pstZmeRtlPara.s32ZmeOffsetVCBtm);

			
			}
			else if((stHdrCfg->stElReso.u32Wth*2==stHdrCfg->stBlReso.u32Wth) ||(stHdrCfg->stElReso.u32Hgt*2==stHdrCfg->stBlReso.u32Hgt))
			{
			...
			VDP_VID_SetZmeHorRatio		   (VDP_LAYER_VID1, (stHdrCfg->stElReso.u32Wth*ZME_HPREC)/stHdrCfg->stBlReso.u32Wth);
			VDP_VID_SetZmeVerRatio		   (VDP_LAYER_VID1, (stHdrCfg->stElReso.u32Hgt*ZME_VPREC)/stHdrCfg->stBlReso.u32Hgt);
...
#define DHD0_OUT_RESOLUTION_H_THD 2160
 ...
 HI_VOID VDP_DRV_GetPreZmeConfig(VDP_RECT_S  stZmeIn, VDP_RECT_S  stZmeOut, VDP_DRAW_PIXEL_MODE_E* penHScalerMode,
VDP_DRAW_MODE_E* penVScalerMode, VDP_PREZME_CONFIG_S* pstPreZmeConfig)
{
	...
	*penHScalerMode = VDP_DRAW_PIXEL_MODE_1;
	*penVScalerMode = VDP_DRAW_MODE_1;

	if (stZmeIn.u32Hgt<= V0_READ_FMT_ZME_THD * stZmeOut.u32Hgt) /* (in <= out) or (> 1 ~ <= 2); read frame and close prescaler*/
	{
	bHPZmeEnable = HI_FALSE;
	*penVScalerMode = VDP_DRAW_MODE_1;
	}
	else if ((stZmeIn.u32Hgt > V0_READ_FMT_ZME_THD * stZmeOut.u32Hgt)
	&& (stZmeIn.u32Hgt <= V0_HPZME_ENABLE_THD * stZmeOut.u32Hgt)) /* > 2 ~ <= 3; read frame and Open HPzme */
	{
	/* Todo: 1. 假如不是按比例缩放;例如: 垂直在2~3之间;但水平不在，这样开启水平预缩放可能会导致水平方向上先缩小再放大;
	2. 720x576i 传递过来的高度是576还是288 ?
	3. 如何保证缩放中的配置:先垂直后水平 */
	bHPZmeEnable = HI_TRUE;
	*penVScalerMode = VDP_DRAW_MODE_1;
	}
	else if ((stZmeIn.u32Hgt > V0_READ_FMT_ZME_THD * stZmeOut.u32Hgt)
	&& (stZmeIn.u32Hgt <= V0_PREZME_ENABLE_2_THD * stZmeOut.u32Hgt)) /* > 3 ~ <= 6; read frame and 1/2 prescaler*/
	{
	bHPZmeEnable = HI_TRUE;
	*penVScalerMode = VDP_DRAW_MODE_2;
	}
	else if ((stZmeIn.u32Hgt > V0_PREZME_ENABLE_2_THD * stZmeOut.u32Hgt)
	&& (stZmeIn.u32Hgt <= V0_PREZME_ENABLE_4_THD * stZmeOut.u32Hgt)) /* > 6 ~ <= 12; read frame and 1/4 prescaler */
	{
	bHPZmeEnable = HI_TRUE;
	*penVScalerMode = VDP_DRAW_MODE_4;
	}
	else if ((stZmeIn.u32Hgt > V0_PREZME_ENABLE_4_THD * stZmeOut.u32Hgt)
	&& (stZmeIn.u32Hgt <= V0_PREZME_ENABLE_8_THD * stZmeOut.u32Hgt)) /* > 12 ~ <= 24; read frame and 1/8 prescaler */
	{
	bHPZmeEnable = HI_TRUE;
	*penVScalerMode = VDP_DRAW_MODE_8;
	}
	else if (stZmeIn.u32Hgt > V0_PREZME_ENABLE_8_THD * stZmeOut.u32Hgt) /* > 24; read field and 1/8 prescaler */
	{
	/* 该倍数只在4K源(Vpss ByPass)时才会进入, 但4K没有场的概念, 理论上只能读帧; read field可认为是读基数行, 偶数行 */
	bHPZmeEnable = HI_TRUE;
	*penHScalerMode = VDP_DRAW_PIXEL_MODE_2;
	*penVScalerMode = VDP_DRAW_MODE_8;
	}
