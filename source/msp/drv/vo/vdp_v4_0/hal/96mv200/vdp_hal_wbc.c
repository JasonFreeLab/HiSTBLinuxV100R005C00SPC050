...
HI_S32 VDP_WBC_ReadTunlCellAddr(VDP_LAYER_WBC_E enLayer)
{
    U_WBC_DHD0_TUNLADDR WBC_DHD0_TUNLADDR;

    WBC_DHD0_TUNLADDR.u32 = VDP_RegRead(((HI_ULONG) & (pVdpReg->WBC_DHD0_TUNLADDR.u32) + enLayer * WBC_OFFSET));

    return WBC_DHD0_TUNLADDR.u32;
}
...
HI_S32 VDP_WBC_ReadEnable(VDP_LAYER_WBC_E enLayer)
{
    U_WBC_DHD0_CTRL WBC_DHD0_CTRL;

    WBC_DHD0_CTRL.u32 = VDP_RegRead(((HI_ULONG) & (pVdpReg->WBC_DHD0_CTRL.u32) + enLayer * WBC_OFFSET));

    return WBC_DHD0_CTRL.bits.wbc_en;
}
...
HI_U32  VDP_WBC_SetWbcDhdReadCheckSumY  (VDP_LAYER_WBC_E enLayer)
{
    HI_U32 u32Checksum = 0;
    //U_WBC_DHD0_CHECKSUM_Y WBC_DHD0_CHECKSUM_Y;

    u32Checksum = VDP_RegRead(((HI_ULONG) & (pVdpReg->WBC_DHD0_CHECKSUM_Y.u32) + enLayer * WBC_OFFSET));

    return u32Checksum;
}
HI_U32  VDP_WBC_SetWbcDhdReadCheckSumC  (VDP_LAYER_WBC_E enLayer)
{
    HI_U32 u32Checksum = 0;
    //U_WBC_DHD0_CHECKSUM_C WBC_DHD0_CHECKSUM_C;

    u32Checksum = VDP_RegRead(((HI_ULONG) & (pVdpReg->WBC_DHD0_CHECKSUM_C.u32) + enLayer * WBC_OFFSET));
    return u32Checksum;
}

HI_U32  VDP_WBC_SetWbcGpReadCheckSumY  (VDP_LAYER_WBC_E enLayer)
{
    HI_U32 u32Checksum = 0;
    //U_WBC_GP0_CHECKSUM_Y   WBC_GP0_CHECKSUM_Y;
    if (enLayer != VDP_LAYER_WBC_GP0)
    {
        VDP_PRINT("Error, VDP_WBC_SetWbcGpReadCheckSumY() select wrong dhd layer id\n");
        return HI_FALSE;
    }

    u32Checksum = VDP_RegRead(((HI_ULONG) & (pVdpReg->WBC_GP0_CHECKSUM_Y.u32) + enLayer * WBC_OFFSET));
    VDP_PRINT("tc : u32Checksum yyy == 0x%x\n", u32Checksum );

    return u32Checksum;
}
HI_U32  VDP_WBC_SetWbcGpReadCheckSumC  (VDP_LAYER_WBC_E enLayer)
{
    HI_U32 u32Checksum = 0;
    //U_WBC_GP0_CHECKSUM_C   WBC_GP0_CHECKSUM_C;
    if (enLayer != VDP_LAYER_WBC_GP0)
    {
        VDP_PRINT("Error, VDP_WBC_SetWbcGpReadCheckSumC() select wrong dhd layer id\n");
        return HI_FALSE;
    }

    u32Checksum = VDP_RegRead(((HI_ULONG) & (pVdpReg->WBC_GP0_CHECKSUM_C.u32) + enLayer * WBC_OFFSET));
    VDP_PRINT("tc : u32Checksum ccc == 0x%x\n", u32Checksum );
