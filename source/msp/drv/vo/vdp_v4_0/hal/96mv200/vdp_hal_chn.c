...
HI_VOID  VDP_DISP_TRUSTZONE_CTRL(HI_U32 u32Num)
{
    U_TRUSTZONE_CTRL_DEBUG   TRUSTZONE_CTRL_DEBUG;
    //TRUSTZONE_CTRL_DEBUG.u32        = VDP_RegRead((&(pVdpReg->TRUSTZONE_CTRL_DEBUG.u32)));
    TRUSTZONE_CTRL_DEBUG.u32 = u32Num;
    VDP_RegWrite((HI_ULONG)&(pVdpReg->TRUSTZONE_CTRL_DEBUG.u32), TRUSTZONE_CTRL_DEBUG.u32);
    return ;
}
    ...
    switch(enDigFmt)
    {
        ...
        case VDP_DISP_DIGFMT_1080I50:
        {
           ...
           stSyncInfo.u32Vbb   = 20;
            // stSyncInfo.u32Vfb   = 2000;
             //stSyncInfo.u32Vbb   = 2000;

            stSyncInfo.u32Hact  = 1920;

            //stSyncInfo.u32Hact  = debug_width;
            stSyncInfo.u32Hfb   = 528;
            stSyncInfo.u32Hbb   = 192;
            //stSyncInfo.u32Hfb   = 5280;
            //stSyncInfo.u32Hbb   = 1920;
...
#if FPGA_DOLBY_DEBUG_STEP
HI_VOID VDP_SetVdpFpgaDebugWrite(HI_U32 u32Data)
{

VDP_RegWrite(((HI_ULONG)&(pVdpReg->VO_DAC_CTRL0.u32)), u32Data);
}
HI_S32 VDP_SetVdpFpgaDebugRead(HI_VOID)
{

return (VDP_RegRead(((HI_ULONG)&(pVdpReg->VO_DAC_CTRL0.u32))))&0x1;
}



HI_VOID VDP_SetVdpFpgaDebugWrite1(HI_U32 u32Data)
{

VDP_RegWrite(((HI_ULONG)&(pVdpReg->VO_DAC_C_CTRL.u32)), u32Data);
}
HI_S32 VDP_SetVdpFpgaDebugReadOver(HI_VOID)
{

return (VDP_RegRead(((HI_ULONG)&(pVdpReg->VO_DAC_C_CTRL.u32))))&0x1;
}
#endif
...
HI_VOID  VDP_DISP_ClearIntSta(HI_U32 u32intmask )
{
	...
	VDP_SetVdpFpgaDebugWrite(0);
	VDP_SetVdpFpgaDebugWrite1(0);
	while(u32Data==0)
	{
		msleep(1);
		u32Data= VDP_SetVdpFpgaDebugRead();
	}
#endif
	...
	if((u32intmask & VDP_INTMSK_DHD1_VTTHD)  == VDP_INTMSK_DHD1_VTTHD)
	{
		...
		if(DHD0_CTRL.bits.iop == 0)
		{
			...
			{
			DHD0_STATE.u32 = VDP_RegRead((&(pVdpReg->DHD0_STATE.u32)+VDP_CHN_DHD1*CHN_OFFSET));
			}
	...
	//wbc vp
	if((u32intmask & VDP_INTMSK_WBC_VP0_INT)  == VDP_INTMSK_WBC_VP0_INT)
	{
	...
	if((u32intmask & VDP_INTMSK_WBC_GP0_INT)  == VDP_INTMSK_WBC_GP0_INT)
	{
		...
		u32_dut_data_wbc_y 	 =	 *sum_data_wbc ;
		u32_dut_data_wbc_c    =   *(sum_data_wbc+1);
		VDP_PRINT(" ONE_CHK_CHN == %d\n",ONE_CHK_CHN);
		VDP_PRINT(" MAX_CHK_FRM == %d\n",MAX_CHK_FRM);
		VDP_PRINT(" gu32FrmCnt == %d\n",gu32FrmCnt);
		VDP_PRINT(" 2*ONE_CHK_CHN*MAX_CHK_FRM + 0*MAX_CHK_FRM + gu32FrmCnt-1 == %d\n",2*ONE_CHK_CHN*MAX_CHK_FRM + 0*MAX_CHK_FRM + gu32FrmCnt-1);
...
//		  VDP_PRINT("cSrcTmp = %s\n\n",cSrcTmp); 
	...
	VOINTSTA.u32 = VDP_RegRead(((HI_ULONG)&(pVdpReg->VOINTSTA.u32)));
	if(((u32intmask & VDP_INTMSK_WBC_HD0_INT)  != VDP_INTMSK_WBC_HD0_INT)||
	((u32intmask & VDP_INTMSK_WBC_VP0_INT)  != VDP_INTMSK_WBC_VP0_INT)||
	((u32intmask & VDP_INTMSK_WBC_GP0_INT)  != VDP_INTMSK_WBC_GP0_INT))
	{
		HI_U32	intf0_en;
		HI_U32	intf1_en;
	DHD0_CTRL.u32 = VDP_RegRead(((HI_ULONG)&(pVdpReg->DHD0_CTRL.u32)+VDP_CHN_DHD0*CHN_OFFSET));
...
HI_S32 VDP_DISP_GetDhdRowNum(HI_U32 u32Data)
{
    U_DHD0_STATE  DHD0_STATE;

    DHD0_STATE.u32 = VDP_RegRead(((HI_ULONG)&(pVdpReg->DHD0_STATE.u32)+u32Data * CHN_OFFSET));
...
{
    U_DHD0_CTRL DHD0_CTRL;
    if(u32Data != VDP_CHN_DHD1)
    {
        VDP_PRINT("Error,VDP_DISP_SetTriggerEnable Select Wrong CHANNEL ID\n");
        return ;
    }

    DHD0_CTRL.u32 = VDP_RegRead(((HI_ULONG)&(pVdpReg->DHD0_CTRL.u32)+u32Data * CHN_OFFSET));
    DHD0_CTRL.bits.trigger_en = u32Enable;
    VDP_RegWrite(((HI_ULONG)&(pVdpReg->DHD0_CTRL.u32)+u32Data * CHN_OFFSET), DHD0_CTRL.u32);
}
...
HI_VOID VDP_DISP_SetSdSyncEnable         (HI_U32 u32Data, HI_U32 u32Enable)
{
    ...
    DHD0_SYNC_CFG.u32 = VDP_RegRead(((HI_ULONG)&(pVdpReg->DHD0_SYNC_CFG.u32)+u32Data * CHN_OFFSET));
    DHD0_SYNC_CFG.bits.sd_sync_en = u32Enable;
    VDP_RegWrite(((HI_ULONG)&(pVdpReg->DHD0_SYNC_CFG.u32)+u32Data * CHN_OFFSET), DHD0_SYNC_CFG.u32);
}


HI_VOID VDP_DISP_SetSdSyncFrmNum         (HI_U32 u32Data, HI_U32 u32FrmCnt)
{
    U_DHD0_SYNC_CFG DHD0_SYNC_CFG;
    if(u32Data != VDP_CHN_DHD1)
    {
    ...
    DHD0_SYNC_CFG.u32 = VDP_RegRead(((HI_ULONG)&(pVdpReg->DHD0_SYNC_CFG.u32)+u32Data * CHN_OFFSET));
    DHD0_SYNC_CFG.bits.sync_frm_num = u32FrmCnt;
    VDP_RegWrite(((HI_ULONG)&(pVdpReg->DHD0_SYNC_CFG.u32)+u32Data * CHN_OFFSET), DHD0_SYNC_CFG.u32);
}
...
{
    U_DHD0_VTTHD3 DHD0_VTTHD3;
    if(u32Data != VDP_CHN_DHD0)
    {
        VDP_PRINT("Error,VDP_DISP_SetTriggerEnable Select Wrong CHANNEL ID\n");
        return ;
    }

    DHD0_VTTHD3.u32 = VDP_RegRead(((HI_ULONG)&(pVdpReg->DHD0_VTTHD3.u32)+u32Data * CHN_OFFSET));
    DHD0_VTTHD3.bits.vtmgthd4 = trigger_row;
    VDP_RegWrite(((HI_ULONG)&(pVdpReg->DHD0_VTTHD3.u32)+u32Data * CHN_OFFSET), DHD0_VTTHD3.u32);
}
