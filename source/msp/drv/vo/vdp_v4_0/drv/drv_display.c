...
static HI_BOOL DispInCsMatchSupportOutCs(HI_DRV_COLOR_SPACE_E enInCs, HI_DRV_COLOR_SPACE_E enSupportCs)
{
    HI_BOOL bMatch = HI_FALSE;
    HI_DRV_COLOR_SPACE_E enInTmpCs = enInCs;

    /* invalid input color space. */
    if ((HI_DRV_CS_DEFAULT >= enInTmpCs)
        || (HI_DRV_CS_BUTT <= enInTmpCs))
    {
        return HI_FALSE;
    }
    ...
    switch (enInTmpCs)
    {
        ...
        case HI_DRV_CS_BT601_YUV_FULL:
            if (HI_DRV_CS_BT601_YUV_LIMITED == enSupportCs)
            {
                bMatch = HI_TRUE;
            }
            break;
        ...
        case HI_DRV_CS_BT709_YUV_FULL:
            if (HI_DRV_CS_BT709_YUV_LIMITED == enSupportCs)
            {
                bMatch = HI_TRUE;
            }
            break;
        ...
        case HI_DRV_CS_BT2020_YUV_FULL:
            if (HI_DRV_CS_BT2020_YUV_LIMITED == enSupportCs)
            {
                bMatch = HI_TRUE;
            }
            break;
...
static HI_S32 DispGenerateOutputColorSpace(DISP_S *pstDisp,
                                           ...
                                           HI_DRV_COLOR_SPACE_E *penOutColorSpace)
{
   ...
   *penOutColorSpace = enSupportYuvOutCsArray[0];
   ...
   for(u32Index = 0; u32Index < DISP_SUPPORT_YUV_OUT_CS_MAX_NUM; u32Index++)
   {
      if (DispInCsMatchSupportOutCs(enInColorSpace, enSupportYuvOutCsArray[u32Index]))
      {
        /* update the output color space when the inCs match outCs. */
        *penOutColorSpace = enSupportYuvOutCsArray[u32Index];
        break;
      }
   }
...
static HI_S32 Disp_GenerateDispOutputColorSpace(DISP_S* pstDisp,
                                                DISP_HAL_ENCFMT_PARAM_S *pstFmt,
                                                HI_DRV_DISP_COLOR_SPACE_E enPreUserColorSpace,
                                                HI_DRV_DISP_COLOR_SPACE_E enCurUserColorSpace,
                                                HI_DRV_COLOR_SPACE_E *penOutputColorSpace)
{
    if ((HI_DRV_DISP_FMT_861D_640X480_60 <= pstFmt->eFmt)
        && (HI_DRV_DISP_FMT_VESA_2560X1600_60_RB >= pstFmt->eFmt))
    {
        *penOutputColorSpace = pstFmt->enColorSpace;
    }
    else
    {
        /* generate output color space when init sys or user change output colorspace mode. */
        if ((HI_DRV_CS_DEFAULT >= *penOutputColorSpace)
            || (HI_DRV_CS_BUTT <= *penOutputColorSpace)
            || (enPreUserColorSpace != enCurUserColorSpace))
        {
            /* input color is unknow cause receive no frames. */
            (HI_VOID)DispGenerateOutputColorSpace(pstDisp, HI_DRV_CS_UNKNOWN, penOutputColorSpace);
        }
    }
...
HI_S32 DispProduceDisplayInfo(DISP_S* pstDisp, HI_DISP_DISPLAY_INFO_S* pstInfo)
{
    ...
    if (eFmt < HI_DRV_DISP_FMT_CUSTOM)
    {
        ...
        Disp_GenerateDispOutputColorSpace(pstDisp,
                                          &stFmt,
                                          pstInfo->enUserSetColorSpace,
                                          pstDisp->stSetting.enUserSetColorSpace,
                                          &pstDisp->stSetting.enOutputColorSpace);
...
HI_S32 DISP_GetDispOutColorSpace(HI_DRV_DISPLAY_E enDisp, HI_DRV_COLOR_SPACE_E *penDispOutColorSpace)
{
    DISP_S* pstDisp;

    DispCheckDeviceState();
    DispCheckID(enDisp);
    DispCheckNullPointer(penDispOutColorSpace);
    DispGetPointerByID(enDisp, pstDisp);

    *penDispOutColorSpace = pstDisp->stSetting.enOutputColorSpace;
                                                 ...
                                                 HI_DRV_COLOR_SPACE_E *penFrameOutColorSpace)
{
    ...
    (HI_VOID)DispGenerateOutputColorSpace(pstDisp, enFrameInColorSpace, penFrameOutColorSpace);
...
HI_S32 DISP_SetOutputColorSpace(HI_DRV_DISPLAY_E enDisp, HI_DRV_DISP_COLOR_SPACE_E enColorSpace)
{
    ...
    if ((HI_DRV_DISP_FMT_PAL <= pstDisp->stDispInfo.eFmt)
        && (HI_DRV_DISP_FMT_NTSC_443 >= pstDisp->stDispInfo.eFmt))
    {
...
{
    HI_DRV_COLOR_SPACE_E enDispOutColorSpace =HI_DRV_CS_UNKNOWN;
    HI_S32 s32Ret = HI_SUCCESS;

    DispCheckNullPointer(penColorSpace);

    s32Ret = DISP_GetDispOutColorSpace(enDisp, &enDispOutColorSpace);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_DISP("DISP_GetDispOutColorSpace Err.\n");
        return s32Ret;
    }

    if ((HI_DRV_CS_BT709_YUV_LIMITED <= enDispOutColorSpace)
        && (HI_DRV_CS_BT709_RGB_FULL >= enDispOutColorSpace))
    {
        *penColorSpace = HI_DRV_DISP_COLOR_SPACE_BT709;
    }
    else if ((HI_DRV_CS_BT2020_YUV_LIMITED <= enDispOutColorSpace)
        && (HI_DRV_CS_BT2020_RGB_FULL >= enDispOutColorSpace))
    {
        *penColorSpace = HI_DRV_DISP_COLOR_SPACE_BT2020;
    }
    else
    {
