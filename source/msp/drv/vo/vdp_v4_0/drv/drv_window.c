                                                            ...
                                                            WIN_INFO_S     *pstWinInfo)
{
    ...
    else
    {
        ...
        pstWinInfo->enOutType = pstWinPara->pstDispInfo->stDispHDRAttr.enDispType;
    }
...
{
    HI_U32 u32ChnHd_time = 0,  u32CircleTime = 0;
    ...
    if (HI_NULL != pstWin)
    {
        ...
        (HI_VOID)DISP_GetVbiTimeInfo(pstWin->enDisp, &u32CircleTime, &u32ChnHd_time);
    }

    spin_unlock_irqrestore(&g_threadIsr_Lock, irqflag);

    msleep((u32ChnHd_time + u32CircleTime));
...
HI_S32 WinDestroyDisplayWindow(WINDOW_S *pstWin)
{
    ...
    /*pay attention please, here release layer is wrong. when in v1, layer shoulde not be released.*/
...
HI_S32 WinBufferReset(WIN_BUFFER_S *pstBuffer)
{
    ...
    DISP_MEMSET(&pstBuffer->stUselessBuf, 0,
                                           ...
                                           HI_U32  u32PlayCnt)
{
    ...
    {
        WIN_HdrDolbyProcessPolicy(pstFrame->enSrcFrameType,
...
HI_VOID ISR_WinReleaseUSLFrame(WINDOW_S *pstWin)
{
	...
	memset(&stRlsFrm, 0, sizeof(HI_DRV_VIDEO_FRAME_S));
                               ...
                               WIN_HAL_PARA_S *pstLayerPara)
{
    ...
    if (HI_FALSE == pstPriv->bAspectProcessed)
    {
        /*window phy OutRect*/
    ...
    else
    {
        ...
        if (VDP_LAYER_VID3 == pstWin->u32VideoLayer)
        {
            ...
            stOutputRect = pstFramePrivate->stWBC2SDDispRect;
        }
        else
        {
            stOutputRect = pstWin->stUsingAttr.stOutRect;
        }

        if (VDP_LAYER_VID0 == pstWin->u32VideoLayer)
        {
            stFinalVideoPosition = stOutputRect;
        }
        else
        {
            stFinalVideoPosition = pstFrame->stDispRect;
            stFinalVideoPosition.s32X = stOutputRect.s32X;
            stFinalVideoPosition.s32Y = stOutputRect.s32Y;
        }
        ...
        || (pstFrame->stDispRect.s32Height != pstLayerPara->stVideo.s32Height))
    {
        ...
        {
            pstLayerPara->bRegionMute = 1;
            pstLayerPara->stIn = pstWin->stUsingAttr.stOutRect ;
            pstLayerPara->stVideo = pstWin->stUsingAttr.stOutRect ;
            pstLayerPara->stDisp = pstWin->stUsingAttr.stOutRect ;
        }
    }
...
HI_S32 WinCapture420Frame10bitTransforTo8bit(HI_DRV_VIDEO_FRAME_S* pstFrame, DISP_MMZ_BUF_S* pstSrcmem)
{
    ...
    u32Datalen =  u32Width + 8 * u32Ystride;
...
HI_S32 WinGetOutputCaptureFrame(HI_DRV_VIDEO_FRAME_S* pstFrame, DISP_MMZ_BUF_S* pstDstMem)
{
        ...
        ( (HI_DRV_PIX_FMT_NV21 == pstFrame->ePixFormat) || (HI_DRV_PIX_FMT_NV12 == pstFrame->ePixFormat)))
    {
        ...
        WinCapture420Frame10bitTransforTo8bit(pstFrame, pstDstMem);
    ...
    else
    {
        ...
        WinProcessFrameToSP420((HI_DRV_VIDEO_FRAME_S*)pu8TempMem, pstDstMem->u32StartPhyAddr, (WIN_EXTERN_S*)(pu8TempMem + sizeof(HI_DRV_VIDEO_FRAME_S)));
...
HI_S32 WinCaptureFrame(HI_HANDLE hWin, HI_DRV_VIDEO_FRAME_S *pstFrame, HI_U32 *stMMZPhyAddr, HI_U32 *stMMZlen)
{
    ...
    HI_U32 u32Datalen = 0, u32Ystride = 0, u32Cstride = 0, u32Height = 0;
    ...
    *pstFrame = *(pstFrm);
    ...
    {
        u32Datalen = u32Height * u32Ystride * 3 / 2 + u32Height * 4;
    }
    else if ( HI_DRV_PIX_FMT_NV21_CMP == pstFrame->ePixFormat)
    {
        u32Datalen = u32Height * u32Ystride * 3 / 2 + 16 * u32Height * 3 / 2;
    }
    else if ( HI_DRV_PIX_FMT_NV61_2X1 == pstFrame->ePixFormat)
    {
        u32Datalen = u32Height * u32Ystride * 2 + u32Height * 4;
    }
    else
    {
