...
static HI_S32 DmxParserFilterSCData(HI_U32 Pid, HI_U64 ParserOffset, HI_U8 *TsRecBuf, HI_U32 TsRecBufSize, HI_U8 *SCDataBuf, HI_U32 SCDataBufLen, HI_U32 *MonoParseLen, HI_U32 *RealDataLen)
{
    HI_U64 FrameGlobalOffset;
    HI_U8 *pData, *pDataSC;
    HI_U32 DestIdx, SrcIdx;
...
static HI_S32 DmxFixupHevcEsIndex(Dmx_Set_S *DmxSet, DMX_RecInfo_S *RecInfo, FINDEX_SCD_S *pstFidx)
{
    ...
    do
    {
        ...
        ret = DmxParserFilterSCData(RecInfo->IndexPid, pstFidx->s64GlobalOffset, pBuf, BufLen, pDupDataBuf, HEVC_DUP_DATA_TOTAL_LEN, &MonoParseLen, &RealParseLen);
        if (HI_SUCCESS != ret)
        {
            goto out;
        }
