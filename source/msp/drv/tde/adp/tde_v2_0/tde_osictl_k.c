                                     ...
                                     TDE2_MBOPT_S* pMbOpt)
{
    ...
    TDE2_OPT_S stOpt = {0};
    //TDE_MB_FMT_CHECK_ODD(pstMB->enMbFmt,pstMbRect->u32Width,pstMbRect->u32Height);
...
STATIC HI_VOID TdeOsiFinalizeSliceCalculation(TDE_SLICE_DATA_S* stSliceData , TDE_HWNode_S* pChildNode)
{
    ...
    stSliceData->des_crop_en = (!((stSliceData->crop_start_x >  stSliceData->des_xed_pos_blk) ||  (stSliceData->crop_end_x < stSliceData->des_xst_pos_blk))) && stSliceData->des_crop_en;

    stSliceData->des_crop_start_x = stSliceData->des_crop_en ?  (MAX2(stSliceData->des_xst_pos_blk, stSliceData->crop_start_x) - stSliceData->des_xst_pos_blk) : 0;
    stSliceData->des_crop_end_x = stSliceData->des_crop_en ?  (MIN2(stSliceData->des_xed_pos_blk, stSliceData->crop_end_x) -  stSliceData->des_xst_pos_blk)  : 0;
