...
{
	return (tc9012_last_key[idx].lower || tc9012_last_key[idx].upper)
...
enum IR_MATCH_RESULT tc9012_Check_Header(struct ir_buffer *buf, struct ir_protocol *ip, struct key_attr *key)
{
...
enum IR_MATCH_RESULT tc9012_Check_Frame(struct ir_buffer *buf, struct ir_protocol *ip,struct key_attr *key, int idx)
{
...
void tc9012_process_timeout(struct ir_priv *ir,struct ir_protocol *ip,struct key_attr *last_key,struct ir_buffer *wr,uint idx,int *pcnt)
{
        ...
        struct ir_buffer *rd, struct ir_buffer *wr)
{
    ...
    mm_ps(&ip->attr.burst, minp, maxp, mins, maxs);
    if (data_margin_bigger((u32)symbol2->lower, (u32)symbol2->upper, minp, maxp, mins, maxs)) {
        goto repeat_key;
    }
    ...
    while (flsg) {
		...
		if (data_both_margin((u32)symbol->lower, (u32)symbol->upper, minp, maxp, mins, maxs)) {
            key.lower |= (unsigned long long)(((unsigned long long)1) << i);
            symbol = ir_next_reader_clr_inc(rd);
            i ++;
			flsg = symbol->upper && symbol->lower && i < ip->attr.wanna_bits;
            continue;
