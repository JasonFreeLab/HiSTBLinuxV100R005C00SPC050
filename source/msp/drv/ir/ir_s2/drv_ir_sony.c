...
static int data_margin_burst(int fail, u32 lower, u32 upper,
	                          struct ir_protocol* ip, 
							  unsigned long long mins, unsigned long long maxs)
{
	return !fail && ((!data_fallin(lower, ip->attr.b0.minp, ip->attr.b0.maxp)
...
enum IR_MATCH_RESULT sony_check_header(struct ir_buffer *buf, struct ir_protocol *ip,struct key_attr *key)
{
...
enum IR_MATCH_RESULT sony_check_frame(struct ir_buffer *buf, struct ir_protocol *ip,struct key_attr *key, int idx)
{
        ...
        struct ir_buffer *rd, struct ir_buffer *wr)
{
    ...
    /* frame start? */
    if (data_neither_margin((u32)symbol->lower, (u32)symbol->upper, 
		                                      minp, maxp, mins, maxs))
    {
    ...
    if (data_margin_burst(fail, (u32)symbol->lower, 
		                (u32)symbol->upper,ip, mins, maxs)) {
        hiir_info("SONY : unkown symbol[l, u]: [%d, %d],"\
            " burst[p, s, f]: [%d, %d, %d].\n",
            (u32)symbol->lower, (u32)symbol->upper,
