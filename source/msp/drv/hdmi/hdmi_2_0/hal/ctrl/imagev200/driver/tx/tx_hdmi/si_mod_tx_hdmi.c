...
static int sHdcpCapabilityGet(HdmiTxObj_t* pObj)
{
    ...
    /* read BKSV */
    pObj->stHdcpCaps.enHdcp14 = Sii_HDMI_HDCP_UNDO;
    ddcErr = sBksvRead(pObj, u8Bksv);
    if(SI_TX_DDC_ERROR_CODE_NO_ERROR == ddcErr)
    {
        for(i = 0,u8Num = 0; i < SII_BKSV_LIST_BYTES; i++)
        {
            for(j = 0; j < 8; j++)
            {
                if((u8Bksv[i] >> j) & 0x01)
        ...
        if (u8Num == 20)
        {
            SII_LIB_LOG_DEBUG1(pObj, "HDCP14 ddc read success,hdcp14supprot=1!\n");
            pObj->stHdcpCaps.enHdcp14 = Sii_HDMI_HDCP_TRUE;
        }
        else
        {
			SII_LIB_LOG_WARN(pObj, "Bksv error,not 20 zero!\n");
            pObj->stHdcpCaps.enHdcp14 = Sii_HDMI_HDCP_FALSE;
        }
    }
    else
    {
            SII_LIB_LOG_WARN(pObj, "HDCP1.4 ddc read fail,ddcErr=%d!\n",ddcErr);
            pObj->stHdcpCaps.enHdcp14 = Sii_HDMI_HDCP_ERROR;
            s32Ret |= -1;
    }
