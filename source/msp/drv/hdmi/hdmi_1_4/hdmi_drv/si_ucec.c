...
static void PrintCommand( SiI_CEC_t * SiI_CEC )
{
    ...
    SI_CHECK_POINTER_NO_RET(SiI_CEC);
...
void store_cec_msg(SiI_CEC_t *rx_cmd)
{
    ...
    SI_CHECK_POINTER_NO_RET(rx_cmd);
...
HI_U32 SI_CEC_SendCommand(HI_UNF_HDMI_CEC_CMD_S  *pCECCmd)
{
    ...
    cec_frame.bCount          = (pCECCmd->unOperand.stRawData.u8Length&0x0F);
    memcpy((cec_frame.bOperand), (pCECCmd->unOperand.stRawData.u8Data), (pCECCmd->unOperand.stRawData.u8Length&0x0F));
