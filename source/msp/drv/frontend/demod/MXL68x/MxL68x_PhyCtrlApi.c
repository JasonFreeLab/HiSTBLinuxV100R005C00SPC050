...
const UINT8 MxLWareDrvVersion[] = {1, 2, 4, 6};
...
/* New EAD Spec requirement for loop through behavior */
//#define NEW_EAD_SPEC
...
/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxLWare68x_API_CfgDevIntrMask
--| 
--| AUTHOR        :  JW Chang
--|
--| DATE CREATED  : 6/24/2016
--|
--| DESCRIPTION   :This API configures MxL68x Host interrupt mask.
--|                 HOST_INT_ACDATA|HOST_INT_EWS|HOST_INT_TUNEDONE|HOST_INT_GPI|HOST_INT_FRMSYNC_CHANGED|HOST_INT_REBOOT
--|
--| RETURN VALUE  : MXL_SUCCESS, MXL_NOT_INITIALIZED, MXL_INVALID_PARAMETER or
--|                 MXL_FAILURE
--|---------------------------------------------------------------------------*/
MXL_STATUS_E MxLWare68x_API_CfgDevIntrMask(   
                UINT8 devId,			
                UINT16 intrMask)
{
  MXL_STATUS_E status = MXL_SUCCESS;
  ...
  if((intrMask&(~HOST_INT_ALL))==0) // checking if the param is valid
  {
    status = MxL68x_Ctrl_ReadRegister(devId, DMD_INT_MASK_MAILBOX_REG, &regData);
    if(status == MXL_SUCCESS) 
    {
      regData &= HOST_INTR_MODE;
      intrMask |= regData;	//masking the interrupt mode

      status = MxL68x_Ctrl_WriteRegister(devId, DMD_INT_MASK_MAILBOX_REG, intrMask); //setting the interrupt mask   
    }

    MxL_DLL_DEBUG0("MxLWare68x_API_CfgDevIntrMask - write intrMask : 0x%x \n", intrMask);
  }
  else
  {
    MxL_DLL_DEBUG0(" MxLWare68x_API_CfgDevIntrMask - return with MXL_INVALID_PARAMETER \n");
    status = MXL_INVALID_PARAMETER;
  }
...
/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxLWare68x_API_CfgDevIntrMode
--| 
--| AUTHOR        : JW Chang
--|
--| DATE CREATED  : 6/24/2016
--|
--| DESCRIPTION   :This API configures MxL68x Host interrupt mode.
--|                Valid modes are the edge triggered interrupt(Falling/Rising) and the level triggered interrupt (Low/High)
--|
--| RETURN VALUE  : MXL_SUCCESS, MXL_NOT_INITIALIZED, MXL_INVALID_PARAMETER or
--|                 MXL_FAILURE
--|---------------------------------------------------------------------------*/
MXL_STATUS_E MxLWare68x_API_CfgDevIntrMode(   
                UINT8 devId,			
                MXL68X_INTR_MODE_E intrMode)
{
  MXL_STATUS_E status = MXL_SUCCESS;
  UINT16 tempData = 0;
  UINT16 regData = 0;
  ...
  if(intrMode<MXL68x_INTR_MODE_MAX) // checking if the param is valid
  {	
    status = MxL68x_Ctrl_ReadRegister(devId, DMD_INT_MASK_MAILBOX_REG, &regData);
    if(status == MXL_SUCCESS) 
    {
      regData &= ~HOST_INTR_MODE;
      tempData = intrMode ;
      regData |= (tempData<<INTR_MODE_BIT_SHIFT);

      status = MxL68x_Ctrl_WriteRegister(devId, DMD_INT_MASK_MAILBOX_REG, regData); //setting Host interrupt mode 
	
      if((intrMode == MXL68x_INTR_LEVEL_LOW )|| (intrMode == MXL68x_INTR_FALLING_EDGE ) ) 
      {
        status = MxL68x_Ctrl_ReadRegister(devId, DMD_GPIO_FUNC_MODE_REG, &regData); 
        if(status == MXL_SUCCESS) 
        {
          regData |= 0x1000; // set high (1)
          status = MxL68x_Ctrl_WriteRegister(devId, DMD_GPIO_FUNC_MODE_REG, regData);   //de-active the interrupt line
        }
      }
      else 
      {
        ...
        if(status == MXL_SUCCESS) 
        {
          regData &= ~(0x1000);  //set low (0)
          status = MxL68x_Ctrl_WriteRegister(devId, DMD_GPIO_FUNC_MODE_REG, regData);   //de-active the interrupt line
        }
      }
    }

    MxL_DLL_DEBUG0(" MxLWare68x_API_CfgDevIntrMode - IntrMode : %d, INT pin status : 0x%x \n" , intrMode, regData);
  }
  else
  {
    MxL_DLL_DEBUG0(" MxLWare68x_API_CfgDevIntrMode - return with MXL_INVALID_PARAMETER \n");
    status = MXL_INVALID_PARAMETER;
  }
                ...
                UINT8* intrStatusPtr)
{
  ...
  if ((devContextPtr) && (intrStatusPtr != NULL))
  {
    if (devContextPtr->driverStatus)
    {
	
      status = MxL68x_Ctrl_ReadRegister(devId, DMD_INTERRUPT_MAILBOX_REG, &readBack);  //read the interrupt status register
      if(status == MXL_SUCCESS) 
      {
        *intrStatusPtr = (UINT8)readBack;
 
        status = MxL68x_Ctrl_WriteRegister(devId, DMD_INTERRUPT_MAILBOX_REG, 0x0);  //clear the interrupt status register 
	 if(status != MXL_SUCCESS)  MxL_DLL_DEBUG0("MxLWare68x_API_ReqDevIntrStatus [%d] - fail to clear the interrupt status register\n",devId);

        status |= MxL68x_Ctrl_ReadRegister(devId, DMD_INT_MASK_MAILBOX_REG, &readBack);
        if(status == MXL_SUCCESS) 
        {
          if(((readBack&HOST_INTR_MODE)>>INTR_MODE_BIT_SHIFT) == MXL68x_INTR_LEVEL_LOW) //if low level triggered interrupt
          {
            status = MxL68x_Ctrl_ReadRegister(devId, DMD_GPIO_FUNC_MODE_REG, &regData); 
             if(status == MXL_SUCCESS) 
            {
          ...
          else if(((readBack&HOST_INTR_MODE)>>INTR_MODE_BIT_SHIFT) == MXL68x_INTR_LEVEL_HIGH) //if high level triggered interrupt
          {
            status = MxL68x_Ctrl_ReadRegister(devId, DMD_GPIO_FUNC_MODE_REG, &regData); 
            if(status == MXL_SUCCESS) 
            {
  ...
  MxL_DLL_DEBUG0("MxLWare68x_API_ReqDevIntrStatus - RETURN: %d [%d]\n",status, devId);
  return(MXL_STATUS_E)status;
}
  ...
  if (devContextPtr) 
  {
    ...
    { 
      ...
      if ((layerABerPtr) || (layerBBerPtr) || (layerCBerPtr))
      {
        ...
        MxL_DLL_DEBUG0(" Check Demod Lock = %d ,status:%d\n", tmccLock,status); 
            ...
            if (status == MXL_SUCCESS)
            {
              ...
              for (i = 0; i < MXL68x_ISDBT_LAYER_ALL; i++)
              {
                if (berInfoPtr[i])
                {
                  ...
                  MxL_DLL_DEBUG0("MxLWare68x_API_ReqDemodBER:%d: index %d, mode %d, seg %d, mod %d, numCR %d, denCR %d, frm %d,status:%d\n",__LINE__, i, mode, seg, mod, numCR, denCR, (regData & 0x0F),status);
