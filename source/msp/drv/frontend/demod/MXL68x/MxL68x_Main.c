...
*/
  ...
  // config the interrupt mask and mode	
  status = MxLWare68x_API_CfgDevIntrMask(MxL68xDeviceId, HOST_INT_ALL);
  if (status != MXL_SUCCESS)
    printf ("Err : MxLWare68x_API_CfgDevIntrMask %d\n", status);
	
  status = MxLWare68x_API_CfgDevIntrMode(MxL68xDeviceId, MXL68x_INTR_RISING_EDGE);
  if (status != MXL_SUCCESS) 
    printf ("Err : MxLWare68x_API_CfgDevIntrMode %d\n", status);
