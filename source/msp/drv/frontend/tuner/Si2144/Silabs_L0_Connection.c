...
HI_U32  si2144_tda_write(HI_U8 uAddress, HI_U32 uNbData, HI_U8 * pDataBuff)
{
	...
	s32Ret = tuner_i2c_send_data (g_stTunerOps[s_u32Si2144TunerPort].enI2cChannel, uAddress, &stI2cDataStr);
...
int     Si2144_L0_ReadBytes         (L0_Context* i2c, unsigned int iI2CIndex, int iNbBytes, unsigned char *pucDataBuffer) {
  ...
  s_u32Si2144TunerPort = i2c->u32TunerId;
  switch (i2c->connectionType) {
#ifdef    USB_Capability
    case USB:
      ...
      */
      ...
      for(i = 0;i < 1;i++)
      {
          ...
          HI_UNF_DEMOD_DEV_TYPE_3130I == g_stTunerOps[i2c->u32TunerId].enDemodDevType)
          {
			  ...
			  si2144_tda_read ((HI_U8)g_stTunerOps[i2c->u32TunerId].u32TunerAddress, iNbBytes, pucDataBuffer);
          }
          else
          {
        ...
        */
	    ...
	    //{
	        for(i=0;i<1/*5*/;i++)
	        {
                ...
                {
                    qam_config_i2c_out (i2c->u32TunerId, 1);
		            si2144_tda_write ((HI_U8)g_stTunerOps[i2c->u32TunerId].u32TunerAddress,  iNbBytes, pucDataBuffer);
                }
                else
                {
    	            hi3137_config_i2c_out(i2c->u32TunerId);
