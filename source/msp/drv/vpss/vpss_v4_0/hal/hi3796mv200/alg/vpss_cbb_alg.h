...
#if 1
#define VPSS_ALG_IS_MCNR_ENABLE(width, height) \
    (((width) <= 1920) && ((width) >= 64) && ((height) <= 2044))
#else
#define VPSS_ALG_IS_MCNR_ENABLE(width, height) \
    (((width) <= VPSS_ALG_MCNR_W_MAX) && ((width) >= VPSS_ALG_MCNR_W_MIN) && ((height) <= VPSS_PAL_HEIGHT))
#endif
...
typedef enum
{
    ...
    CBB_ALG_SNR,
    CBB_ALG_FMTC,
    CBB_ALG_DB,  //no use in 96mv200
    CBB_ALG_DB_DET,
    CBB_ALG_DM, //no use in 96mv200
    CBB_ALG_NR,//no use in 96mv200
    CBB_ALG_DS,//no use in 96mv200
    CBB_ALG_IGBM,
    CBB_ALG_IFMD,
    CBB_ALG_MCDEI,
    CBB_ALG_MEDS,
