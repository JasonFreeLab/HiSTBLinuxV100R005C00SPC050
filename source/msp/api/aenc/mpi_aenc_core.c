...
static pthread_mutex_t g_AencMutex[AENC_INSTANCE_MAXNUM] = {[0 ... (AENC_INSTANCE_MAXNUM-1)] = PTHREAD_MUTEX_INITIALIZER};

#define AENC_LOCK(AencId) \
    (void)pthread_mutex_lock(&g_AencMutex[AencId])

#define AENC_TRY_LOCK(AencId) \
    do \
    { \
        if (0 != pthread_mutex_trylock(&g_AencMutex[AencId])) \
        { \
            return HI_FAILURE; \
        } \
    } while (0)

#define AENC_UNLOCK(AencId) \
    (void)pthread_mutex_unlock(&g_AencMutex[AencId])
...
static HI_S32 AENC_SendInnerBuffer(AENC_CHAN_S* pstAencChan)
{
    ...
    AENC_TRY_LOCK(pstAencChan->u32ChID);
    ...
    AENC_UNLOCK(pstAencChan->u32ChID);
    return nRet;
}
