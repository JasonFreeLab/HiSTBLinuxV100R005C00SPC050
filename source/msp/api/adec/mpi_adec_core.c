...
static HI_S32 ADECOpenDevice(ADEC_CHAN_S* pstAdecChan)
{
    ...
    {
        HI_FATAL_ADEC("Adec Device Open failed(0x%x)\n", s32Ret);
        return HI_FAILURE;
    }
...
static HI_S32 ADECSwDecode(ADEC_CHAN_S* pstAdecChan)
{
    ...
    while (1)
    {
        ...
        else if (HA_ErrorNone == sRet)    /* Success */
        {
            ...
            ADECConvertHaOut2MpiOut(pstAdecChan, &pstOutBuf->outBuf[pstOutBuf->u32BufWriteIdx], &sOut, u32StreamReadPos);
...
HI_S32 ADEC_SetAttr(HI_HANDLE hAdec, HI_U32 u32Command, HI_VOID* pstParam)
{
    ...
    //------------1 . first  get attr -------------------
    s32Ret = ADEC_GetAttr(hAdec, ADEC_ATTR_ALLATTR, &stAllAttr);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_ADEC("ADEC_GetAttr ADEC_ATTR_ALLATTR failed(0x%x)\n", s32Ret);
        return s32Ret;
    }
