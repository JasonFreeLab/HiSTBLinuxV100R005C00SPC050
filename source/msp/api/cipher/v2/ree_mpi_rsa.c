...
HI_VOID *s_u32RngBase = HI_NULL;
HI_VOID *s_u32SicRsaBase = HI_NULL;
                                   ...
                                   HI_U8 *pu8Output, HI_U32 *pu32OutLen)
{
    ...
    cipher_memset_s(u8EM, CIPHER_MAX_RSA_KEY_LEN, 0, CIPHER_MAX_RSA_KEY_LEN);
...
static HI_S32 CIPHER_RsaRandomNumber(HI_VOID *param, HI_U8 *rng, size_t size)
{
    ...
    for(i=0; i<size; i+=4)
    {
        ...
        byte = size - i;
        byte = byte > 4 ? 4: byte;
        cipher_memcpy_s(rng+i, 4, &u32Randnum, byte);
    }
    return HI_SUCCESS;
}
                               ...
                               HI_UNF_CIPHER_RSA_PRI_KEY_S *pstRsaPriKey)
{
    ...
    INLET_PARAM_CHECK_POINT_NULL(pstRsaPriKey);
    INLET_PARAM_CHECK_U32_ZERO(u32NumBits);
    INLET_PARAM_CHECK_U32_ZERO(u32Exponent);
    INLET_PARAM_CHECK_POINT_NULL(pstRsaPriKey->pu8N);
    INLET_PARAM_CHECK_POINT_NULL(pstRsaPriKey->pu8E);
    INLET_PARAM_CHECK_POINT_NULL(pstRsaPriKey->pu8D);
    INLET_PARAM_CHECK_POINT_NULL(pstRsaPriKey->pu8P);
    INLET_PARAM_CHECK_POINT_NULL(pstRsaPriKey->pu8Q);
    INLET_PARAM_CHECK_POINT_NULL(pstRsaPriKey->pu8DP);
    INLET_PARAM_CHECK_POINT_NULL(pstRsaPriKey->pu8DQ);
    INLET_PARAM_CHECK_POINT_NULL(pstRsaPriKey->pu8QP);
    ...
    ret = (HI_U32)HI_SYS_MapRegister(TRNG_BASE, 0x10, &pu32RngBase);
    if (ret != HI_SUCCESS)
    {
                               ...
                               HI_U8 *pu8Q, HI_U8 *pu8DP, HI_U8 *pu8DQ, HI_U8 *pu8QP)
{
    ...
    INLET_PARAM_CHECK_POINT_NULL(pu8Q);
    INLET_PARAM_CHECK_POINT_NULL(pu8DP);
    INLET_PARAM_CHECK_POINT_NULL(pu8DQ);
    INLET_PARAM_CHECK_POINT_NULL(pu8QP);
    INLET_PARAM_CHECK_U32_ZERO(u32NumBits);
    INLET_PARAM_CHECK_U32_ZERO(u32Exponent);
