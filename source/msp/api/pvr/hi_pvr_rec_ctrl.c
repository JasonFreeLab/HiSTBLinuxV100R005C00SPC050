...
static HI_S32 PVRRecShowProc(HI_PROC_SHOW_BUFFER_S* pstBuf, HI_VOID* pPrivData)
{
    ...
    for (i = 0; i < PVR_REC_MAX_CHN_NUM; i++)
    {
            ...
            (pChnAttr[i].enState != HI_UNF_PVR_REC_STATE_BUTT))
        {
            ...
            HI_PROC_Printf(pstBuf, "\tAppend rec        :%d/%d (User/Actually)\n", pChnAttr[i].stUserCfg.bAppendRec, pChnAttr[i].IndexHandle->stAppendRecLastRecordInfo.bAppend);
            if(HI_TRUE == pChnAttr[i].IndexHandle->stAppendRecLastRecordInfo.bAppend)
            {
                HI_PROC_Printf(pstBuf, "\tPrev rec S/E/L    :%d/%d/%d\n", pChnAttr[i].IndexHandle->stAppendRecLastRecordInfo.u32StartFrame, pChnAttr[i].IndexHandle->stAppendRecLastRecordInfo.u32EndFrame,
                                                                            pChnAttr[i].IndexHandle->stAppendRecLastRecordInfo.u32LastFrame);
                HI_PROC_Printf(pstBuf, "\tPrev rec Time     :%d\n", pChnAttr[i].IndexHandle->stAppendRecLastRecordInfo.u32LastDisplayTime);
            }
#endif
            ...
            for(j = u32PidStartNum; j < pChnAttr[i].IndexHandle->pstPidCtrl->u32UsedNum; j++)
            {
                ...
                HI_PROC_Printf(pstBuf, "\t       Video Type :%s\n",pStreamType[pstPidInfo->u32VideoType]);
...
{
#ifdef PVR_PID_CHANGE_SUPPORT    
    if (HI_INVALID_HANDLE == pChnAttr->DemuxRecHandle)
    {
        pChnAttr->enState = HI_UNF_PVR_REC_STATE_PID_CHANGE;
        HI_WARN_PVR("PID was changing.... \n");
...
static HI_S32 PVRRecSaveToCacheSecure(PVR_REC_CHN_S* pRecChn)
{
    ...
    do
    {
        s32Ret = PVR_TS_CACHE_BUFFER_WriteToBuffer(&pRecChn->IndexHandle->stTsCacheBufInfo, stReeData.pDataAddr, stReeData.u32Len);
        if (HI_SUCCESS != s32Ret)
        {
            PVR_USLEEP(10 * 1000);
            u32SendCnt++;
...
static HI_S32 PVRRecSaveToCacheNormal(PVR_REC_CHN_S* pRecChn, HI_MPI_DMX_REC_DATA_INDEX_S* pstDataIdx)
{
    ...
    for (i = 0; i < pstDataIdx->u32RecDataCnt; i++)
    {
        ...
        do
        {
            s32Ret = PVR_TS_CACHE_BUFFER_WriteToBuffer(&pRecChn->IndexHandle->stTsCacheBufInfo, pstData->pDataAddr, pstData->u32Len);
            if (HI_SUCCESS != s32Ret)
            {
                PVR_USLEEP(10 * 1000);
                u32SendCnt++;
        ...
        if (ret == HI_SUCCESS)
        {
            ...
            if (HI_UNF_PVR_SECURE_MODE_TEE == pRecChn->stUserCfg.enSecureMode)
            {
                ...
                HI_PVR_TeeAdjustTsData(pstDataIdx, pRecChn->u32TeeChnId,
                                       pRecChn->IndexHandle->u64DeltaGlobalOffset, &pRecChn->stReeBuff, &pRecChn->stTeeBuff);
...
static HI_VOID PVRRecFlushAllToFile(PVR_REC_CHN_S* pstChn)
{
    ...
    (HI_VOID)PVR_TS_CACHE_BUFFER_Reset(&(pstChn->IndexHandle->stTsCacheBufInfo), pstChn->IndexHandle->stTsCacheBufInfo.u64HeadGlobalOffset);
        ...
        if (ret == HI_SUCCESS)
        {
            ...
            if (HI_UNF_PVR_SECURE_MODE_TEE == pRecChn->stUserCfg.enSecureMode)
            {
                ...
                HI_PVR_TeeAdjustTsData(pstDataIdx,
                                       ...
                                       &pRecChn->stReeBuff, &pRecChn->stTeeBuff);
                HI_PVR_TeeGetTeeState(pRecChn->u32TeeChnId, &pRecChn->stTeeState);
                pRecChn->stReeBuff.u32Used = 0;
                PVRRecProcExtendCallBack(pRecChn, &pRecChn->stReeBuff);
            }
...
STATIC INLINE HI_S32 PVRReadAlignDataFromFile(PVR_REC_CHN_S *pChnAttr, HI_U64 u64Offset, HI_U64 u64GlobalOffset)
{
    ...
    if(-1 == readLen)
    {
        HI_ERR_PVR("read failed: want: %lu, Actually: %d  fd: %d\n", u32ReadSize, readLen, sysFd);
        HI_ERR_PVR("u64Offset: %lld\n",u64Offset);
        goto EXIT;
    }
    else
    {
        ...
        s32Ret = PVR_TS_CACHE_BUFFER_Reset(&(pChnAttr->IndexHandle->stTsCacheBufInfo), u64GlobalOffset - u32ReadSize);
        if(HI_SUCCESS != s32Ret)
        {
        ...
        s32Ret = PVR_TS_CACHE_BUFFER_WriteToBuffer(&pChnAttr->IndexHandle->stTsCacheBufInfo, pDataAddr, u32ReadSize);
        if(HI_SUCCESS != s32Ret)
        {
...
HI_S32 HI_PVR_RecCreateChn(HI_U32* pu32ChnID, const HI_UNF_PVR_REC_ATTR_S* pstRecAttr)
{
    ...
    if(HI_TRUE == pstRecAttr->bAppendRec)
    {
...
//解析上次续录的信息并更新到pstPidCtrl中，并判断是否满足续录
    ...
    if((HI_NULL != pChnAttr->IndexHandle) && (HI_TRUE == pChnAttr->stLastRecInfo.bAppend))
    {
        ...
        {
            HI_ERR_PVR("old Index Version not support pid change! \n");
            ret = HI_FAILURE;
            goto CIPHER_EXIT;
        }

        if(HI_SUCCESS != PVR_Index_ParseAllPidList(pChnAttr->IndexHandle, pstRecAttr->szFileName))
        {
        ...
        ret = PVR_Index_RecReviseThePidCtrlInfoWhenAppendRecord(pChnAttr->IndexHandle, pChnAttr->stLastRecInfo.u32EndFrame, &pChnAttr->stLastRecInfo);
        if(HI_SUCCESS != ret)
        {
            goto CIPHER_EXIT;
        }
        pstLastRecPidInfo = &(pChnAttr->IndexHandle->pstPidCtrl->stPidInfo[pChnAttr->IndexHandle->pstPidCtrl->u32UsedNum - 1]);
        if((pstLastRecPidInfo->u32IndexType != pstRecAttr->enIndexType) ||
            (pstLastRecPidInfo->u32VideoType != pstRecAttr->enIndexVidType) ||
                (pstLastRecPidInfo->u32IndexPid != pstRecAttr->u32IndexPid))
        {
            ...
            HI_ERR_PVR("(IndexType, VideoType, IndexPid): config(%d, %d, %d)   Last(%d, %d, %d)\n",
                            pstRecAttr->enIndexType, pstRecAttr->enIndexVidType, pstRecAttr->u32IndexPid,
...
HI_S32 HI_PVR_RecStartChn(HI_U32 u32ChnID)
{
    ...
    else
    {
        ...
        if (HI_TRUE == pRecChn->stLastRecInfo.bAppend)
        {
            ...
            pRecChn->u64CurWritePos = pRecChn->stLastRecInfo.u64GlobalOffset;
            pRecChn->u64CurFileSize = pRecChn->stLastRecInfo.u64GlobalOffset;
#ifdef HI_PVR_EXTRA_BUF_SUPPORT
         /**//*当开启二级缓存时(O_DIRECT)需要判断续录点是否需要对齐,如果需要则将对齐后剩余的数据写入二级缓存中*/
            HI_S32 residualSize = pRecChn->stLastRecInfo.u64GlobalOffset % PVR_DIO_ALIGN_SIZE;
            if(residualSize != 0)
            {
                ret = PVRReadAlignDataFromFile(pRecChn, pRecChn->stLastRecInfo.u64GlobalOffset, pRecChn->stLastRecInfo.u64GlobalOffset);
                if(HI_SUCCESS != ret)
                {
...
static HI_VOID PVRRecCheckAndWaitIdle(PVR_REC_CHN_S* pstRecChn, HI_U32 u32TimeOutMs)
{
    ...
    if(HI_NULL == pstRecChn)
    {
        HI_ERR_PVR("null point for recChn\n");
        return;
    }

    u32WaitCnt = u32TimeOutMs/10;
    if (0 > (HI_S32)u32TimeOutMs)
    {
        u32WaitCnt = -1;
    }
...
static HI_S32 PVRRecCheckPidChange(PVR_REC_CHN_S *pstChnAttr, HI_UNF_PVR_PID_CHANGE_INFO_S *pstPidInfo)
{
    ...
    /*入参指针类型非空检查*/
    if (HI_NULL == pstPidInfo || HI_NULL == pstChnAttr)
    {
        HI_ERR_PVR("null pointer for input!\n");
...
HI_S32 HI_PVR_RecChangePid(HI_U32 u32ChnId, HI_UNF_PVR_PID_CHANGE_INFO_S *pstPidInfo)
{
    ...
    PVR_INDEX_HANDLE hIndex = HI_NULL;
    ...
    PVR_RECORD_LAST_INFO_S *pstLastRecordInfo = HI_NULL;
#ifdef HI_PVR_EXTRA_BUF_SUPPORT
    ...
    hIndex = pstChnAttr->IndexHandle;
    pstChnAttr->DemuxRecHandle = HI_INVALID_HANDLE;
    ...
    {
        HI_ERR_PVR("wait timeout, acquire error!\n");
        pstChnAttr->DemuxRecHandle = hDemuxHandle;
    ...
    //对齐offset
    pstCurPidInfo = &pstChnAttr->IndexHandle->pstPidCtrl->stPidInfo[pstChnAttr->IndexHandle->u32CurRecIdx];
    s32Ret = PVR_Index_RecAlignTheLastPidInfoWhenChangePid(hIndex, pstChnAttr->stUserCfg.szFileName, pstCurPidInfo);
    if(HI_SUCCESS != s32Ret)
    {
    ...
    stOldPidInfo.enIndexType = pstCurPidInfo->u32IndexType;
    stOldPidInfo.u32IndexPid = pstCurPidInfo->u32IndexPid;
    stOldPidInfo.enVidType = pstCurPidInfo->u32VideoType;
    stOldPidInfo.u32PidNum = pstCurPidInfo->u32PidNum;
    memcpy(stOldPidInfo.u32PidList, pstCurPidInfo->u32PidList, HI_UNF_PVR_REC_MAX_PID_NUM * sizeof(HI_U32));

    pstChnAttr->u64CurWritePos = pstCurPidInfo->u64FileOffset;//考虑到回绕，此处应该用fileOffset

#ifdef HI_PVR_EXTRA_BUF_SUPPORT
    /*当开启二级缓存时(O_DIRECT)需要判断变更续录点是否需要对齐,如果需要则将对齐后剩余的数据写入二级缓存中*/
    u32ResidualSize = pstCurPidInfo->u64FileOffset % PVR_DIO_ALIGN_SIZE;
    if(u32ResidualSize != 0)
    {
        s32Ret = PVRReadAlignDataFromFile(pstChnAttr, pstCurPidInfo->u64FileOffset, pstCurPidInfo->u64GlobalOffset);
        if(HI_SUCCESS != s32Ret)
        {
            HI_ERR_PVR("align 512 failed! remainder: %d \n",u32ResidualSize);
            PVR_UNLOCK(&(pstChnAttr->stMutex));
    ...
    //更新LastRecordInfo
    pstChnAttr->IndexHandle->pstLastRecordInfo = &(pstChnAttr->stLastRecInfo);
    pstLastRecordInfo = pstChnAttr->IndexHandle->pstLastRecordInfo;
    pstLastRecordInfo->bAppend = HI_FALSE;
    pstLastRecordInfo->u32EndFrame = pstCurPidInfo->u32EndFrame;
    pstLastRecordInfo->u32LastDisplayTime = pstCurPidInfo->u32EndDispTime;
    pstLastRecordInfo->u64GlobalOffset = pstCurPidInfo->u64GlobalOffset;

    memset(&hIndex->stTimeStamp, 0x00, sizeof(hIndex->stTimeStamp));
    /*save the new pid info to files*/
    PVR_INDEX_AddPidChange(hIndex, pstPidInfo);
...
HI_BOOL HI_PVR_AdvcaRecCheckAppend(HI_U32 u32ChnId, HI_U64 *p64LastRecGlobalOffset, HI_U32 *p32LastRecDisPlayTime)
{
    ...
    CHECK_REC_CHNID(u32ChnId);
    pstChnAttr = PVR_GET_RECPTR_BY_CHNID(u32ChnId);
    CHECK_REC_CHN_INIT(pstChnAttr->enState);
...
HI_U64 HI_PVR_RecCorrectFileOffset(HI_U32 u32ChnId, HI_U64 u64GlobalOffset)
{
