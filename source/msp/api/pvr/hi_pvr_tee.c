...
HI_S32 HI_PVR_TeeAdjustTsData(HI_MPI_DMX_REC_DATA_INDEX_S* pstDmxRecData,
                              HI_U32 u32TeeChnId, HI_U64 u64DropLength,
                              PVR_REE_BUFFER_INFO_S* pstReeBuff, PVR_REE_BUFFER_INFO_S* pstTeeBuff)
{
    ...
    pstTeeBuff->u32SmmuAddr = 0;
    pstTeeBuff->u32Used = 0;
    pstIndexInfo->u32OptimizeFlag = 1;

    memset(&stAdjustOperation, 0x00, sizeof(stAdjustOperation));

    stAdjustOperation.started = 1;
    stAdjustOperation.params[0].value.a = u32TeeChnId;
    stAdjustOperation.params[0].value.b = pstReeBuff->u32SmmuAddr;
    ...
    pstTeeBuff->u32SmmuAddr = pstIndexInfo->stRecData[0].u32PhyAddr;
    pstTeeBuff->u32Used = pstIndexInfo->stRecData[0].u32Len;
...
HI_S32 HI_PVR_TeePlayProcTsData(HI_U32 u32PhyAddr, HI_U32 u32HeadOffset, HI_U32 u32HeadToAdd, HI_U32 u32EndOffset, HI_U32 u32EndToAdd)
{
    ...
    memset(&stStateOperation, 0x00, sizeof(stStateOperation));
    stStateOperation.started = 1;
    stStateOperation.params[0].value.a = u32PhyAddr;
    stStateOperation.params[0].value.b = u32HeadOffset;
    stStateOperation.params[1].value.a = u32EndOffset;
    stStateOperation.params[1].value.b = u32HeadToAdd;
    stStateOperation.params[2].value.a = u32EndToAdd;
    stStateOperation.paramTypes = TEEC_PARAM_TYPES(TEEC_VALUE_INPUT,
                                  TEEC_VALUE_INPUT,
                                  TEEC_VALUE_INPUT,
                                  TEEC_NONE);

    stResult = TEEC_InvokeCommand(&s_stTeeCtrlInfo.stSession,
                                  TEE_PVR_PLAY_PROCESS_DATA,
                                  &stStateOperation,
                                  &u32Origin);
    if (TEEC_SUCCESS != stResult)
    {
