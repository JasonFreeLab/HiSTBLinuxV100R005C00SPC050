...
/*用于解决异常拔掉U盘后，再次录制时会让U盘变成只读权限的问题*/
HI_S32 PVR_FTRUNCATE64(PVR_FILE64 file, HI_U64 u64TsFileSize)
{
    PVR_FILE_S *pPvrFile;
    int pvrFd = 0;
    HI_U32 nodeIdx = 0;
    PVR_FILE tsFd = PVR_FILE_INVALID_FILE;
    HI_CHAR  fNameOpen[PVR_MAX_FILENAME_LEN] = {0};
    ...
    if(0 != nodeIdx)
    {
        ...
        PVRFileGetRealFName(pPvrFile->szFileName, fNameOpen, (HI_S32)nodeIdx);
        tsFd = PVR_OPEN(fNameOpen, pPvrFile->openMode);
        ...
        PVR_CLOSE(pPvrFile->systemFd);
        pPvrFile->systemFd = tsFd;
    }

    if(0 != ftruncate(pPvrFile->systemFd, u64TsFileSize))
    {
...
/**CNcomment: 该接口用于开启extra buffer时，写文件要求512对齐，处理非对齐的数据时使用，方法:用不含O_DIRECT选项打开的文件
                        句柄去写非对齐的数据，并同步相关信息到通道中打开的File信息中*/
