...
#define  AVPLAY_INVALID_HDCP_FMT        0xffff
...
{
    HI_S32                  Ret;
    PRE_SYNC_INFO_S         stPreSyncInfo = {0};
    static HI_BOOL          bLastPreSyncTargetInit = HI_FALSE;

    Ret = HI_MPI_SYNC_GetPreSyncStatus(pAvplay->hSync, &pAvplay->bPreSyncFinish);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_AVPLAY("HI_MPI_SYNC_GetPreSyncStatus failed Ret=%#x \n", Ret);
    }

    Ret = HI_MPI_SYNC_GetPreSyncInfo(pAvplay->hSync, &stPreSyncInfo);
    if (HI_SUCCESS == Ret)
    {
        if ((HI_TRUE == stPreSyncInfo.bPreSyncTargetInit) && (bLastPreSyncTargetInit != stPreSyncInfo.bPreSyncTargetInit))
        {
            Ret = HI_MPI_AO_Track_SetTargetPts(pAvplay->hRenderTrack, stPreSyncInfo.u32TargetPts);
            if (HI_SUCCESS != Ret)
            {
                HI_ERR_AVPLAY("HI_MPI_AO_Track_SetTargetPts failed Ret=%#x \n", Ret);
            }

            HI_WARN_AVPLAY("set target  Pts to AO Render Track succeed,pts=%d\n", stPreSyncInfo.u32TargetPts);
        }

        bLastPreSyncTargetInit = stPreSyncInfo.bPreSyncTargetInit;
    }
    else
    {
        HI_ERR_AVPLAY("HI_MPI_SYNC_GetPreSyncInfo failed Ret=%#x \n", Ret);
    }
...
HI_VOID AVPLAY_ProcRenderTrackPlay(AVPLAY_S *pAvplay)
{
    ...
    if (SYNC_AUD_SPEED_ADJUST_NORMAL == pAvplay->AudOpt.SpeedAdjust)
    {
        (HI_VOID)HI_MPI_AO_Track_SetSpeedAdjust(pAvplay->hRenderTrack, 0, HI_MPI_AO_SND_SPEEDADJUST_MUTE);
        (HI_VOID)HI_MPI_AO_Track_SetSpeedAdjust(pAvplay->hRenderTrack, 0, HI_MPI_AO_SND_SPEEDADJUST_SRC);
    }
    else if (SYNC_AUD_SPEED_ADJUST_UP == pAvplay->AudOpt.SpeedAdjust)
    {
        (HI_VOID)HI_MPI_AO_Track_SetSpeedAdjust(pAvplay->hRenderTrack, 0, HI_MPI_AO_SND_SPEEDADJUST_SRC);
    ...
    {
        (HI_VOID)HI_MPI_AO_Track_SetSpeedAdjust(pAvplay->hRenderTrack, -10, HI_MPI_AO_SND_SPEEDADJUST_SRC);
    }
    else if (SYNC_AUD_SPEED_ADJUST_MUTE_REPEAT == pAvplay->AudOpt.SpeedAdjust)
    {
        (HI_VOID)HI_MPI_AO_Track_SetSpeedAdjust(pAvplay->hRenderTrack, -100, HI_MPI_AO_SND_SPEEDADJUST_MUTE);
    }
...
HI_VOID AVPLAY_ProcVidPlay(AVPLAY_S *pAvplay)
{
    ...
    if (HI_SUCCESS == Ret)
    {
        ...
        AVPLAY_NotifyVidFrmDisplayInfo(pAvplay, hWindow, pAvplay->CurFrmPack.stFrame[i].stFrameVideo.u32Pts);
...
HI_S32 AVPLAY_SetHDR(AVPLAY_S *pAvplay, HI_UNF_AVPLAY_HDR_ATTR_S *pstAttr)
{
    ...
    Ret = HI_SYS_GetChipAttr(&ChipAttr);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_AVPLAY("HI_SYS_GetChipAttr fail 0x%x\n",Ret);
        return HI_FAILURE;
    }

    if (ChipAttr.bDolbyvisionSupport == HI_FALSE)
    {
        HI_ERR_AVPLAY("Dolbyvision is not Support\n");
...
HI_VOID AVPLAY_ProcVidJumpPlay(AVPLAY_S *pAvplay)
{
    ...
    else
    {
        ...
        AVPLAY_Notify(pAvplay, HI_UNF_AVPLAY_EVENT_VID_START_POS, (HI_VOID *)&CurVidPts);
    }
...
HI_VOID AVPLAY_ProcVidEventFrameInfo(AVPLAY_S *pAvplay)
{
    ...
    {
        if (   (pCurVdecFrame->stFrameVideo.DispCtrlWidth == AVPLAY_INVALID_HDCP_FMT)
            && (pCurVdecFrame->stFrameVideo.DispCtrlHeight == AVPLAY_INVALID_HDCP_FMT))
        {
        ...
        else if (   (pstFrmPriv->stVideoOriginalInfo.u32Height == pCurVdecFrame->stFrameVideo.DispCtrlHeight)
                 && (pstFrmPriv->stVideoOriginalInfo.u32Width == pCurVdecFrame->stFrameVideo.DispCtrlWidth))
        {
    ...
    if ((pAvplay->VidInfo.bProgressive == HI_FALSE)&&(VidFrm.bProgressive == HI_TRUE))
    {
        ...
        VidFrm.bProgressive = pAvplay->VidInfo.bProgressive;
...
HI_VOID AVPLAY_ProcMultiAudSync(AVPLAY_S *pAvplay, HI_U32 ChnId)
{
    ...
    Ret = HI_MPI_DMX_AcquireEs(pAvplay->hDmxAud[ChnId], &AudDmxEsBuf);
    if (HI_SUCCESS != Ret)
    {
        return;
    }
...
HI_VOID AVPLAY_GetAudBufState(AVPLAY_S *pAvplay, HI_U32 *pAudBufPercent, HI_UNF_AVPLAY_BUF_STATE_E *pCurAudBufState)
{
    ...
    if (AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType)
    {
        ...
        {
            CurAudBufState  = SYNC_BUF_STATE_NORMAL;
        }
    }
...
HI_VOID AVPLAY_ProcDmxToMainTrack(AVPLAY_S *pAvplay)
{
    ...
    if (HI_INVALID_HANDLE == pAvplay->hDmxAud[0])
    {
        return;
    }
    ...
    if (!pAvplay->AvplayProcDataFlag[AVPLAY_PROC_DMX_MAIN_TRACK])
    {
        ...
        Ret = HI_MPI_DMX_AcquireEs(pAvplay->hDmxAud[0], &pAvplay->AvplayDmxEsBuf);
        if (HI_SUCCESS == Ret)
        {
            pAvplay->DebugInfo.AcquiredAudEsNum++;
    ...
    if (pAvplay->AvplayProcDataFlag[AVPLAY_PROC_DMX_MAIN_TRACK])
    {
        ...
        Ret = HI_MPI_AO_Track_SendStream(pAvplay->hRenderTrack, HI_MPI_AO_BUF_ID_ES_MAIN, &AdecEsBuf, pAvplay->AvplayDmxEsBuf.u32PtsMs);
        if (HI_SUCCESS == Ret)
        {
            ...
            pAvplay->AvplayProcAudContinue = HI_TRUE;
            (HI_VOID)HI_MPI_DMX_ReleaseEs(pAvplay->hDmxAud[0], &pAvplay->AvplayDmxEsBuf);
        }
        else
        {
...
HI_VOID AVPLAY_ProcDmxToAssocTrack(AVPLAY_S *pAvplay)
{
    ...
    if (pAvplay->AvplayProcDataFlag[AVPLAY_PROC_DMX_ASSOC_TRACK])
    {
        ...
        AdecEsBuf.pu8Data = pAvplay->AvplayAdDmxEsBuf.pu8Buf;
        AdecEsBuf.u32Size = pAvplay->AvplayAdDmxEsBuf.u32BufLen;
        Ret = HI_MPI_AO_Track_SendStream(pAvplay->hRenderTrack, HI_MPI_AO_BUF_ID_ES_ASSOC, &AdecEsBuf, pAvplay->AvplayAdDmxEsBuf.u32PtsMs);
        if (HI_SUCCESS == Ret)
        {
...
HI_S32 AVPLAY_MallocRenderTrack(HI_HANDLE *phRenderTrack)
{
    HI_S32                  Ret;
    HI_MPI_AO_RENDER_ATTR_S RenderAttr = {0};
    HI_HANDLE               hRenderTrack = HI_INVALID_HANDLE;

    RenderAttr.enType = HI_MPI_RENDER_TRACK_TYPE_MAIN;

    Ret = HI_MPI_AO_Track_CreateRender(HI_UNF_SND_0, &RenderAttr, &hRenderTrack);
    if (Ret != HI_SUCCESS)
    {
        HI_ERR_AVPLAY("HI_MPI_AO_Track_CreateRender failed 0x%x\n", Ret);
        return Ret;
    }

    *phRenderTrack = hRenderTrack;
    return Ret;
}

HI_S32 AVPLAY_FreeRenderTrack(HI_HANDLE *phRenderTrack)
{
    HI_S32 Ret = HI_SUCCESS;

    if (HI_INVALID_HANDLE == *phRenderTrack)
    {
        return HI_FAILURE;
    }

    Ret = HI_MPI_AO_Track_DestoryRender(*phRenderTrack);
    if (Ret != HI_SUCCESS)
    {
        HI_ERR_AVPLAY("HI_MPI_AO_Track_DestoryRender failed 0x%x\n", Ret);
        return Ret;
    }

    *phRenderTrack = HI_INVALID_HANDLE;
...
HI_S32 AVPLAY_MallocAudChn(AVPLAY_S *pAvplay)
{
    ...
    Ret = AVPLAY_MallocRenderTrack(&pAvplay->hRenderTrack);
    if (Ret != HI_SUCCESS)
    {
        HI_ERR_AVPLAY("Avplay malloc render track failed.\n");
    ...
    if (HI_UNF_AVPLAY_STREAM_TYPE_TS == pAvplay->AvplayAttr.stStreamAttr.enStreamType)
    {
        ...
        if (Ret != HI_SUCCESS)
        {
            ...
            (HI_VOID)AVPLAY_FreeRenderTrack(&pAvplay->hRenderTrack);
            (HI_VOID)AVPLAY_FreeAdec(pAvplay);
            return Ret;
        }
    }
...
HI_S32 AVPLAY_FreeAudChn(AVPLAY_S *pAvplay)
{
    ...
    Ret = AVPLAY_FreeRenderTrack(&pAvplay->hRenderTrack);
    if (Ret != HI_SUCCESS)
    {
        HI_ERR_AVPLAY("Avplay free render track failed.\n");
        return Ret;
    }
    ...
    if (HI_UNF_AVPLAY_STREAM_TYPE_TS == pAvplayAttr->stStreamAttr.enStreamType)
    {
        if (pAvplayAttr->u32DemuxId != pAvplay->AvplayAttr.u32DemuxId)
        {
            if (pAvplay->hAdec != HI_INVALID_HANDLE)
            {
                if (AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType)
                {
                    HI_ERR_AVPLAY("aud type is ms12, can not set aud stream mode.\n");
                    return HI_ERR_AVPLAY_INVALID_OPT;
                }
...
HI_S32 AVPLAY_GetAudType(const HI_UNF_ACODEC_ATTR_S *pAdecAttr, AVPLAY_AUDIO_TYPE_E *pAudType)
{
    HI_S32       Ret = HI_SUCCESS;

    if (HI_NULL != pAdecAttr)
    {
        switch(pAdecAttr->enType)
        {
            case HA_AUDIO_ID_MS12_DDP:
            case HA_AUDIO_ID_MS12_AAC:
            case HA_AUDIO_ID_MS12_AC4:
                *pAudType = AVPLAY_AUDIO_TYPE_RENDER;
                break;
            default:
                *pAudType = AVPLAY_AUDIO_TYPE_NORMAL;
                break;
        }
    }
    else
    {
        HI_ERR_AVPLAY("pAdecAttr is null, default is Normal!\n");
        *pAudType = AVPLAY_AUDIO_TYPE_NORMAL;
    }
...
HI_S32 AVPLAY_SetAdecAttr(AVPLAY_S *pAvplay, const HI_UNF_ACODEC_ATTR_S *pAdecAttr)
{
    ...
    Ret = AVPLAY_GetAudType(pAdecAttr, &pAvplay->AvplayAudioType);
    if (HI_SUCCESS != Ret)
    {
...
HI_S32 AVPLAY_SetRenderAdAttr(AVPLAY_S *pAvplay, HI_UNF_AVPLAY_AD_ATTR_S *pstAttr)
{
    HI_S32                  Ret;
    HI_UNF_DMX_CHAN_ATTR_S  DmxChnAttr;
...
HI_S32 AVPLAY_SetAdAttr(AVPLAY_S *pAvplay, HI_UNF_AVPLAY_AD_ATTR_S *pstAttr)
{
    ...
    Ret = AVPLAY_SetRenderAdAttr(pAvplay, pstAttr);
    if (Ret != HI_SUCCESS)
    {
        HI_ERR_AVPLAY("Set Render Ad Attr fail !\n");
        return HI_FAILURE;
    }
...
static HI_S32 AVPLAY_StartRenderAudChn(AVPLAY_S *pAvplay)
{
    ...
    if (HI_UNF_AVPLAY_STREAM_TYPE_TS == pAvplay->AvplayAttr.stStreamAttr.enStreamType)
    {
        Ret = HI_MPI_DMX_OpenChannel(pAvplay->hDmxAud[0]);
        if (HI_SUCCESS != Ret)
        {
...
static HI_S32 AVPLAY_StopRenderAudChn(AVPLAY_S *pAvplay)
{
    ...
    if (HI_UNF_AVPLAY_STREAM_TYPE_TS == pAvplay->AvplayAttr.stStreamAttr.enStreamType)
    {
        Ret = HI_MPI_DMX_CloseChannel(pAvplay->hDmxAud[0]);
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_AVPLAY("HI_MPI_DMX_CloseChannel failed 0x%x\n", Ret);
        }
...
HI_S32 AVPLAY_AttachRender(HI_HANDLE hRenderTrack, HI_HANDLE hMasterTrack)
{
    HI_S32 Ret = HI_SUCCESS;

    Ret = HI_MPI_AO_Track_Attach(hRenderTrack, hMasterTrack);
    if(HI_SUCCESS != Ret)
    {
        HI_ERR_AVPLAY("call HI_MPI_AO_Track_Attach failed, Ret=0x%x.\n", Ret);
        return Ret;
    }
...
HI_S32 AVPLAY_DetachRender(HI_HANDLE hRenderTrack, HI_HANDLE hMasterTrack)
{
    HI_S32 Ret = HI_SUCCESS;

    Ret = HI_MPI_AO_Track_DeAttach(hRenderTrack, hMasterTrack);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_AVPLAY("call HI_MPI_AO_Track_DeAttach failed, Ret=0x%x.\n", Ret);
        return Ret;
    }
...
HI_S32 AVPLAY_StopRenderAudio(AVPLAY_S *pAvplay)
{
    ...
    if (pAvplay->AudEnable)
    {
        Ret = AVPLAY_DetachRender(pAvplay->hRenderTrack, pAvplay->hSyncTrack);
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_AVPLAY("AVPLAY_DetachRender failed.ret=%#x\n", Ret);
        }
...
HI_S32 AVPLAY_StartRenderAudio(AVPLAY_S *pAvplay)
{
    ...
    Ret = AVPLAY_AttachRender(pAvplay->hRenderTrack, pAvplay->hSyncTrack);
    if (Ret != HI_SUCCESS)
    {
        HI_ERR_AVPLAY("call AVPLAY_AttachRender Aud failed.\n");
        AVPLAY_StopAudChn(pAvplay);
        return Ret;
    }
    Ret = HI_MPI_SYNC_Play(pAvplay->hSync);
    if (Ret != HI_SUCCESS)
    {
        ...
        (HI_VOID)AVPLAY_DetachRender(pAvplay->hRenderTrack, pAvplay->hSyncTrack);
        AVPLAY_StopAudChn(pAvplay);
        return Ret;
    }
...
static HI_VOID AVPLAY_VidProcInfo(AVPLAY_S *pAvplay, HI_PROC_SHOW_BUFFER_S *ProcBuf)
{
    ...
    HI_PROC_Printf(ProcBuf,
                    ...
                    "VP9HDREnable          :%-10s   |VP9HDRType            :%-10s\n",
                ...
                #endif
                    pAvplay->hVdec & 0xff,
                    pAvplay->bPaddingHdrInfo ? "TRUE" : "FALSE",
                    Vp9HdrType
                    );
                        ...
                        HI_U32                        u32Pts)
{
    ...
    if (HI_UNF_AVPLAY_BUF_ID_ES_AUD_AD == enBufId)
    {
        ...
        if (AVPLAY_AUDIO_TYPE_NORMAL == pAvplay->AvplayAudioType)
        {
            ...
            if (HI_INVALID_HANDLE == pAvplay->hAdec)
            {
                HI_ERR_AVPLAY("aud chn not open.\n");
                return HI_ERR_AVPLAY_INVALID_OPT;
            }
...
HI_VOID AVPLAY_GammaTypeConvert_VP9(HDR_INFO_GAMMA_INPUT_S *pstGammaInput, HDR_INFO_GAMMA_OUTPUT_S *pstGammaOutput)
{
    switch (pstGammaInput->u8TransferCharacteristics)
    {
      ...
      //      pstGammaOutput->enGammaType = HI_DRV_GAMMA_TYPE_ARIB_HLG; //HI_DRV_GAMMA_TYPE_HLG
            pstGammaOutput->bBackwardCompatible = HI_FALSE;
            break;
        default:                        //SDR
            pstGammaOutput->enGammaType = HI_DRV_GAMMA_TYPE_1886;
...
HI_S32 HI_MPI_AVPLAY_Create(const HI_UNF_AVPLAY_ATTR_S *pstAvAttr, HI_HANDLE *phAvplay)
{
    ...
    */
    ...
    /*Initalize VP9/AVS2.0 HDR10 function*/
    pAvplay->bPaddingHdrInfo = HI_FALSE;
...
{
    HI_S32    Ret;
    HI_BOOL   bRenderFormat;

    bRenderFormat = HI_MPI_AO_Track_IsRenderFormat(enFormat);
    if (HI_TRUE == bRenderFormat)
    {
        Ret = HI_MPI_AO_Track_FoundSupportDecoder(enFormat, penDstCodecID);
        if (Ret != HI_SUCCESS)
        {
            HI_INFO_AVPLAY("call HI_MPI_AO_Track_FoundSupportDecoder failed 0x%x \n", Ret);
        }
        return Ret;
    }
...
#endif
        if (HI_UNF_AVPLAY_GET_VDEC_HANDLE_CMD == ((HI_CODEC_VIDEO_CMD_S*)pPara)->u32CmdID)
        {
            *(HI_U32 *)(((HI_CODEC_VIDEO_CMD_S*)pPara)->pPara) = pAvplay->hVdec;
            AVPLAY_INST_UNLOCK(Id);
            return HI_SUCCESS;
        }
...
HI_S32 HI_MPI_AVPLAY_IsAudioFormatSupport(const HA_FORMAT_E enFormat, HI_BOOL* pbSupport)
{
    HI_S32  Ret;
    HI_BOOL bRenderFormat;
    HI_U32  DstCodecID;
    ...
    if (HI_TRUE == bRenderFormat)
    {
        return HI_MPI_AO_Track_IsFormatSupport(enFormat, pbSupport);
    }

    Ret = HI_MPI_ADEC_FoundSupportDeoder(enFormat, &DstCodecID);
    if (HI_SUCCESS == Ret)
    {
        *pbSupport = HI_TRUE;
    }
