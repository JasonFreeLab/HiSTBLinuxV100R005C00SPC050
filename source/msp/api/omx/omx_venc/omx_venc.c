...
static OMX_ERRORTYPE omx_check_param_videoHevc(OMX_VIDEO_PARAM_HEVCTYPE* pHevcType)
{
    ...
    bError |= (OMX_S32)(pHevcType->eProfile   != OMX_VIDEO_HEVCProfileMain);
    bError |= (OMX_S32)((pHevcType->eLevel != OMX_VIDEO_HEVCMainTierLevel1)
...
OMX_ERRORTYPE omxvenc_extractGrallocData(void* data, venc_user_info* buffer, OMX_COMPONENT_PRIVATE* pcom_priv)
{
    TDE_HANDLE handle;
    HI_S32 s32Ret = 0;
        ...
        OMX_IN OMX_U32 bytes)
{
    ...
    *omx_bufhdr = pomx_buf;
    ...
    //DEBUG_PRINT("[AB] port[%d] alloc buffer_size = %d,bytes = %d,Phyaddr = 0x%x, VirAddr = %p!(%dx%d)\n",port, pomx_buf->nAllocLen,bytes,pvenc_buf->user_buf.bufferaddr_Phy,pvenc_buf->bufferaddr,port_priv->port_def.format.video.nStride,pcom_priv->m_port[0].port_def.format.video.nFrameHeight);
...
#endif
    //DEBUG_PRINT("[FB] port[%d] free buffer_size = %d,Phyaddr = 0x%x, VirAddr = %p!\n",port, puser_buf->user_buf.buffer_size,puser_buf->user_buf.bufferaddr_Phy,puser_buf->bufferaddr);
    ...
    OMX_IN OMX_BUFFERHEADERTYPE* pomx_buf)
{
        ...
        || (port_priv->m_port_reconfig == OMX_TRUE))
    {
        ...
        return OMX_ErrorNone;                                  //add by sdk
    }
    ...
    if (pcom_priv->m_store_metadata_buf == HI_TRUE)
    {
        if (pomx_buf->nFilledLen != 8)
        {
            ...
            {
                DEBUG_PRINT_ERROR("MetaData buffer is wrong size! "
...
static OMX_ERRORTYPE set_parameter_VideoHevc(OMX_IN OMX_PTR param_data, OMX_COMPONENT_PRIVATE *pcomp_priv)
{
    ...
    DEBUG_PRINT("set_param:OMX_IndexParamVideoAvc,PortIndex = %ld,state=%ld ret = %d\n",
                (OMX_S32)pHevcType->nPortIndex, (OMX_S32)pcomp_priv->m_state, ret);
    return ret;
}
