...
#else
        HI_MMZ_Unmap(pstHwctlStruct->u32TmpPhyaddr);
        HI_MMZ_Delete(pstHwctlStruct->u32TmpPhyaddr);
#endif
#else
...
#ifdef CONFIG_GFX_MMU_SUPPORT
#define HIPNG_ACCELERATE_NOSUPPORT(png_ptr) \
    (png_ptr->interlaced || (png_ptr->transformations & PNG_PACK) \
...
#else
    s32Ret = HI_MMZ_GetPhyaddr(image[0], (HI_CHAR *)pAddr, &u32Size);
#endif
#else
...
HI_U8 hipng_select_decmode(png_structp png_ptr, png_bytepp image, png_uint_32 num_rows)
{
    ...
    if (HIPNG_ACCELERATE_NOSUPPORT(png_ptr))
    {
