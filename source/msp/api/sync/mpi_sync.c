...
HI_S32 HI_MPI_SYNC_GetPreSyncStatus(HI_HANDLE hSync, HI_BOOL *bPreSyncFinish)
{
    ...
    if (!bPreSyncFinish)
    {
        HI_ERR_SYNC("para bPreSyncFinish is null.\n");
        return HI_ERR_SYNC_NULL_PTR;
    }
    ...
    (HI_VOID)SYNC_Mutex_Lock(pSync->pSyncMutex);
    *bPreSyncFinish = (pSync->PreSyncFinish);
    (HI_VOID)SYNC_Mutex_UnLock(pSync->pSyncMutex);
    return HI_SUCCESS;
}
...
HI_S32 HI_MPI_SYNC_GetPreSyncInfo(HI_HANDLE hSync, PRE_SYNC_INFO_S *pstPreSyncInfo)
{
    ...
    (HI_VOID)SYNC_Mutex_Lock(pSync->pSyncMutex);
    pstPreSyncInfo->bPreSyncTargetInit = pSync->PreSyncTargetInit;
    pstPreSyncInfo->u32TargetPts = pSync->PreSyncTargetTime;
    pstPreSyncInfo->enPreSyncTarget = pSync->PreSyncTarget;
    (HI_VOID)SYNC_Mutex_UnLock(pSync->pSyncMutex);
