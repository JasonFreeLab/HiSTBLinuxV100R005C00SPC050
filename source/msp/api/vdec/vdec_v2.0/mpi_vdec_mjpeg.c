...
#if 0
HI_VOID MJPEG_SAVE_InputFile(HI_VOID* pInputStrem, HI_U32 StreamSize)
{
    static HI_S32 Cnt = 0;
    HI_CHAR FileName[256] = {'\0'};
    FILE *pFile = NULL;
    snprintf(FileName,256,"%s_%d.jpg","/mnt/sdcard/Pictures/input_mjpeg_stream",Cnt++);
    FileName[sizeof(FileName) - 1] = '\0';
    pFile = fopen(FileName,"wb+");
    if (NULL == pFile)
    {
       ALOGE("++++ open %s failure\n",FileName);
       return;
    }
    fwrite(pInputStrem,StreamSize,1,pFile);
    fclose(pFile);
    ALOGE("\n=====================================================================\n");
    ALOGE("save %s success\n",FileName);
    ALOGE("=====================================================================\n");
    return;
}
HI_VOID MJPEG_SAVE_OutPutYuvSp(HI_CHAR* pYStream,HI_S32 YWidth,HI_S32 YHeight,HI_S32 YStride,
                                           ...
                                           HI_JPEG_FMT_E eFmt)
{
     ...
     snprintf(pFileName,256,"%s_%d.yuv","/mnt/sdcard/Pictures/output_dec_stream",Count);
     pFileName[sizeof(pFileName) - 1] = '\0';
     snprintf(pHdrName,256,"%s_%d.hdr","/mnt/sdcard/Pictures/output_dec_stream",Count);
     ...
     if (0 != UVWidth){
         ...
         for (s32Cnt = 0; s32Cnt < UVHeight; s32Cnt++)
         {
             ...
             fwrite(pTmpCbCr, UVWidth, 1, pOutFile );
             pTmp += UVStride;
         }
         pTmp = pUVAddr;
         for (s32Cnt = 0; s32Cnt < UVHeight; s32Cnt++)
     ...
     switch (eFmt)
     {
         case JPEG_FMT_YUV400:
             fprintf(pHdrFile, "format = \"Y\" ;\n");
             break;
         case JPEG_FMT_YUV420:
             fprintf(pHdrFile, "format = \"YUV420\" ;\n");
             break;
         case JPEG_FMT_YUV422_12:
             fprintf(pHdrFile, "format = \"YUV422\" ;\n");
             break;
         case JPEG_FMT_YUV422_21:
             fprintf(pHdrFile, "format = \"YUV422\" ;\n");
             break;
         case JPEG_FMT_YUV444:
             fprintf(pHdrFile, "format = \"YUV444\" ;\n");
             break;
         default:
             ALOGE("format is unknow ;\n");
             break;
     }
     fprintf(pHdrFile, "offset = 0 ;\n");
     fprintf(pHdrFile, "width = %d ;\n",   YWidth);
     fprintf(pHdrFile, "height = %d ;\n",  YHeight);
     fprintf(pHdrFile, "framerate = 50 ;\n");
     fprintf(pHdrFile, "loop = 1 ;\n");
     fclose(pHdrFile);
     ALOGE("\n=====================================================================\n");
     ALOGE("save %s success\n",pFileName);
     ALOGE("save %s success\n",pHdrName);
...
HI_S32 MJPEG_DecodeFrame(HI_HANDLE hInst, HI_CODEC_STREAM_S * pstIn, HI_CODEC_FRAME_S * pstOut)
{
...
#ifdef JPEG_DEBUG_ENABLE
    MJPEG_SAVE_InputFile((HI_VOID*)(pstIn->pu8Addr), pstIn->u32Size);
#endif
    /** begin to decode **/
    ...
    HI_JPEG_SetStreamBufInfo(&s_stMjpegParam[hInst].stInfo,pstIn->u32PhyAddr,0,NULL,0);
	...
	#ifdef JPEG_DEBUG_ENABLE
    MJPEG_SAVE_OutPutYuvSp((HI_CHAR*)stSurfaceDesc.pOutVir[0], \
                            stJpegInfo.u32Width[0],            \
                            stJpegInfo.u32Height[0],           \
                            stJpegInfo.u32OutStride[0],        \
                            (HI_CHAR*)stSurfaceDesc.pOutVir[1],\
                            stJpegInfo.u32Width[1],            \
                            stJpegInfo.u32Height[1],           \
                            stJpegInfo.u32OutStride[1],        \
                            stJpegInfo.enFmt);
	ALOGE("mjpeg dec success w %d h %d\n",pstOut->unInfo.stVideo.u32Width,pstOut->unInfo.stVideo.u32Height);
    ALOGE("=====================================================================\n");
	#endif
