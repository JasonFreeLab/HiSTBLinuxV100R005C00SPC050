...
static HI_S32 TUNER_CheckSignalChange_Connect_S(HI_U32 u32tunerId)
{
    ...
    stTunerPara.u32ScrambleValue = s_strCurTunerConnectPara[i].unConnectPara.stSat.u32ScrambleValue;
    /**add end***/
...
static HI_S32 TUNER_Connect_T_T2(HI_U32 u32tunerId, const HI_UNF_TUNER_CONNECT_PARA_S *pstConnectPara, HI_U32 u32TimeOut)
{
    ...
    stTunerPara.unTer.enDVBT2.enChannelAttr = pstTer->enChannelMode;
...
static HI_S32 TUNER_GetBER_C(HI_U32 u32tunerId, const TUNER_DATABUF_S *pstTunerDataBuf, HI_U32 *pu32BER)
{
    ...
    switch (s_strCurTunerAttr[u32tunerId].enDemodDevType)
    {
        ...
        case HI_UNF_DEMOD_DEV_TYPE_3138:
            dBer = (HI_DOUBLE)((HI_DOUBLE)pstTunerDataBuf->u32DataBuf[0]/16384.0) ;
...
static HI_S32 TUNER_CaculateSnr_Hi3138(TUNER_QAM_TYPE_E enQamType, HI_DOUBLE dTmp_HiDVBC, HI_DOUBLE *pdSnrEva)
{
    switch (enQamType)
    {
        case QAM_TYPE_16:
            *pdSnrEva = 10.0 * log10(161 / dTmp_HiDVBC);
            break;

        case QAM_TYPE_32:
            *pdSnrEva = 10.0 * log10(180 / dTmp_HiDVBC);
            break;

        case QAM_TYPE_64:
            *pdSnrEva = 10.0 * log10(168 / dTmp_HiDVBC);
            break;

        case QAM_TYPE_128:
            *pdSnrEva = 10.0 * log10(184 / dTmp_HiDVBC);
            break;

        case QAM_TYPE_256:
            *pdSnrEva = 10.0 * log10(170 / dTmp_HiDVBC);
            break;

        default:
            HI_ERR_TUNER("Unkown QamType:%d\n", enQamType);
            return HI_FAILURE;
    }
...
static HI_S32 TUNER_GetSNR_C(HI_U32 u32tunerId, const TUNER_DATA_S *pu32StTunerData, HI_DOUBLE *pdoubleDSnr)
{
    ...
    switch (s_strCurTunerAttr[u32tunerId].enDemodDevType)
    {
        ...
        case HI_UNF_DEMOD_DEV_TYPE_3138:
            ...
            *pdoubleDSnr = 0;
            (HI_VOID)TUNER_CaculateSnr_Hi3138(s_stCurrentSignal[u32tunerId].enQamType, dTmp_HiDVBC, pdoubleDSnr);
            break;
...
static HI_VOID TUNER_GetSignalStrength_C_MXL603(HI_U32 u32tunerId, TUNER_DATABUF_S *pstTunerDataBuf, HI_U32 *pu32SignalStrength)
{
    ...
    {
        *pu32SignalStrength = pstTunerDataBuf->u32DataBuf[1];
...
static HI_S32 TUNER_GetSignalQuality_C_BerSnr(HI_U32 u32TunerId, HI_DOUBLE *pdBer, HI_DOUBLE *pdSnrEva)
{
    ...
    switch (s_strCurTunerAttr[u32TunerId].enDemodDevType)
    {
        ...
        case HI_UNF_DEMOD_DEV_TYPE_3138:
            ...
            dBer = dBer*10*10*4/204/8;  /*convert PER to BER*/
        ...
        case HI_UNF_DEMOD_DEV_TYPE_3138:
            ...
            *pu32Freq = (HI_U32)( dTmpFreq ) + pstTunerDataBuf->u32DataBuf[0];
            HI_INFO_TUNER("HI_UNF_TUNER_GetRealFreqSymb,stTunerDataBuf.u32DataBuf[0]=%d,s_stCurrentSignal[u32TunerId].u32Frequency=%d\n",
                                    pstTunerDataBuf->u32DataBuf[0],s_stCurrentSignal[u32TunerId].u32Frequency);
