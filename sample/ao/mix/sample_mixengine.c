...
HI_S32 main(int argc, char* argv[])
{
    ...
    if (argc < 5)
    {
        printf("Usage:    %s inputfile0 trackweight0 samplerate0 inchannels0 inputfile1 trackweight1 samplerate1 inchannels1 ...\n", argv[0]);
        printf("examples:\n");
    ...
    while (1)
    {
        printf("\n please input the q to quit!  v to toggle PrecisionVolume\n");
        ...
        if ('v' == InputCmd[0] || 'V' == InputCmd[0])
        {
            HI_UNF_SND_PRECIGAIN_ATTR_S stPreciGain  = {-20, -125};
            HI_UNF_SND_PRECIGAIN_ATTR_S stPreciGain1 = {10, 125};

            static int PreciVol_toggle = 0;
            PreciVol_toggle++;

            if (PreciVol_toggle & 1)
            {
                HI_UNF_SND_SetPrecisionVolume(HI_UNF_SND_0, HI_UNF_SND_OUTPUTPORT_HDMI0, &stPreciGain);
                printf("PrecisionVolume -125 on!\n");
            }
            else
            {
                HI_UNF_SND_SetPrecisionVolume(HI_UNF_SND_0, HI_UNF_SND_OUTPUTPORT_HDMI0, &stPreciGain1);
                printf("PrecisionVolume +125 on!\n");
            }
        }
    }
