...
HI_S32 main(HI_S32 argc, HI_CHAR *argv[])
{
    ...
    if (argc < 3)
    {
        sample_printf(" usage: %s afile1 atype1\n", argv[0]);
        sample_printf("        %s afile1 atype1 afile2 atype2\n", argv[0]);
        sample_printf("        %s afile1 atype1 afile2 atype2 afile3 atype3\n", argv[0]);
        sample_printf(" atype: aac/mp3/dts/dra/mlp/pcm/ddp(ac3/eac3)\n");
        sample_printf(" examples: \n");
        sample_printf("        %s afile1 mp3 afile2 aac afile3 ddp\n", argv[0]);
        return -1;
    }
    else if(argc < 5)
    {
