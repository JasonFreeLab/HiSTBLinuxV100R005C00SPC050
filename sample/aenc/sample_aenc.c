...
static HI_S32 G722AencCreate(HI_HANDLE* phAenc)
{
    HI_S32 s32Ret;
    HI_UNF_AENC_ATTR_S stAencAttr;
    ...
    s32Ret = HI_UNF_AENC_RegisterEncoder("libHA.AUDIO.G722.codec.so");
    if (HI_SUCCESS != s32Ret)
    {
        sample_printf("HI_UNF_AENC_RegisterEncoder failed(0x%x)\n", s32Ret);
        return s32Ret;
    ...
    stAencAttr.enAencType = HA_AUDIO_ID_G722;

    HA_G722_GetEncDefaultOpenParam(&stAencAttr.sOpenParam);
...
static HI_S32 AencCreate(HA_CODEC_ID_E enAencType)
{
    switch (enAencType)
    {
        ...
        case HA_AUDIO_ID_G722:
            return G722AencCreate(&g_hAenc);
...
HI_S32 main(int argc, char* argv[])
{
    ...
    if (6 != argc)
    {
        printf("Usage:   ./sample_aenc infileName inChannel inSamplerate outfileName outfileType\n"
               ...
               "         outfileType:aac g711 g722 amrwb amrnb\n"
              );
