...
******************************************************************************
File Name     : sample_ecdh.c
Version       : Initial Draft
Author        : Hisilicon
Created       : 2017
Last Modified :
Description   : sample for ecdh
Function List :
History       :
******************************************************************************/
#include <stdlib.h>
...
int main(int argc, char* argv[])
{
    ...
    pu8Buf = (HI_U8*)malloc(MAX_KEY_SIZE * 14);
    if(pu8Buf == HI_NULL)
    {
    ...
    pu8PrivKeyA = stEccParam.pu8n + MAX_KEY_SIZE;
    pu8PrivKeyB = pu8PrivKeyA + MAX_KEY_SIZE;
    pu8PubKeyAX = pu8PrivKeyB + MAX_KEY_SIZE;
    pu8PubKeyAY = pu8PubKeyAX + MAX_KEY_SIZE;
    pu8PubKeyBX = pu8PubKeyAY + MAX_KEY_SIZE;
    pu8PubKeyBY = pu8PubKeyBX + MAX_KEY_SIZE;
    pu8SharedSecretA = pu8PubKeyBY + MAX_KEY_SIZE;
 ...
 /*     printBuffer("P", stEccParam.pu8p, ecdh_sizes[i]);
        ...
        printBuffer("B", stEccParam.pu8b, ecdh_sizes[i]);
        ...
        ret = HI_UNF_CIPHER_EccGenKey(&stEccParam, HI_NULL, pu8PrivKeyA, pu8PubKeyAX, pu8PubKeyAY);
        if ( HI_SUCCESS != ret )
        {
            HI_ERR_CIPHER("HI_UNF_CIPHER_DhGenerateKey A failed, ret = 0x%x\n", -ret);
            return HI_FAILURE;
        }
    /*  printBuffer("PrivKeyA", pu8PrivKeyA, ecdh_sizes[i]);
        printBuffer("PubKeyAX", pu8PubKeyAX, ecdh_sizes[i]);
        printBuffer("PubKeyAY", pu8PubKeyAY, ecdh_sizes[i]);*/

        ret = HI_UNF_CIPHER_EccGenKey(&stEccParam, HI_NULL, pu8PrivKeyB, pu8PubKeyBX, pu8PubKeyBY);
        if ( HI_SUCCESS != ret )
        {
            HI_ERR_CIPHER("HI_UNF_CIPHER_DhGenerateKey B failed\n");
            return HI_FAILURE;
        }
   /*   printBuffer("PrivKeyB", pu8PrivKeyB, ecdh_sizes[i]);
        printBuffer("PubKeyBX", pu8PubKeyBX, ecdh_sizes[i]);
        printBuffer("PubKeyBY", pu8PubKeyBY, ecdh_sizes[i]);*/

        ret = HI_UNF_CIPHER_EcdhComputeKey(&stEccParam, pu8PrivKeyA, pu8PubKeyBX, pu8PubKeyBY, pu8SharedSecretA);
        if ( HI_SUCCESS != ret )
        {
            HI_ERR_CIPHER("HI_UNF_CIPHER_DhGenerateKey A failed, ret = 0x%x\n", -ret);
            return HI_FAILURE;
        }
 //     printBuffer("SharedSecretA", pu8SharedSecretA, ecdh_sizes[i]);

        ret = HI_UNF_CIPHER_EcdhComputeKey(&stEccParam, pu8PrivKeyB, pu8PubKeyAX, pu8PubKeyAY, pu8SharedSecretB);
        if ( HI_SUCCESS != ret )
        {
            HI_ERR_CIPHER("HI_UNF_CIPHER_DhGenerateKey B failed, ret = 0x%x\n", -ret);
            return HI_FAILURE;
        }
 //     printBuffer("SharedSecretB", pu8SharedSecretB, ecdh_sizes[i]);

        if(memcmp(pu8SharedSecretA, pu8SharedSecretB, ecdh_sizes[i]) != 0)
        {
            HI_ERR_CIPHER("SharedSecretA != SharedSecretB failed\n");
            printBuffer("A", pu8SharedSecretA, ecdh_sizes[i]);
            printBuffer("B", pu8SharedSecretB, ecdh_sizes[i]);
            return HI_FAILURE;
        }
    }
