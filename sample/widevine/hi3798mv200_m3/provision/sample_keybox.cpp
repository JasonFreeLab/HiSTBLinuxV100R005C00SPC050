...
int main(int argc, char **argv)
{
    ...
    //1. get DRMAddr in deviceinfo
    ret = HI_UNF_PDM_GetDeviceInfo("DRMAddr", DRMAddr, sizeof(DRMAddr));
    if (ret != HI_SUCCESS)
    {
        printf("Failed to get DRMAddr!\n");
        return -1;
    }

    offset = strtoul(DRMAddr, NULL, 16);
    printf("offset = 0x%x\n", offset);
    ...
    ret = OEMCrypto_WrapKeybox(gEncryptedKey, KEYBOX_LENGTH,
