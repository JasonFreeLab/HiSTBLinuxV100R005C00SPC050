...
HI_S32 WidevineProcessImpl::StartWidevineProcess(IN string key_id, IN string url, OUT HI_U8* content_key, OUT HI_U32& content_key_length)
{
    ...
    HI_S32 ret;
    HI_WIDEVINE_CONFIG_S stConfig;

    ret = Hi_Widevine_GetConfig(&stConfig);
    if (HI_SUCCESS != ret)
    {
        WVLOGE("Get widevine config file fail!\n");
        return HI_FAILURE;
    }

    if (FINNAL_DATA_STORE_MODE_TO_REE == stConfig.enFinnalDataStoreMode)
    {
        snprintf(keybox_file.szstring, 256, "%s/%s", "/data/drm/widevine", "keybox_enc.bin_stb_root_key_enc");

        WV_PROMPT("Read %s\n", keybox_file.szstring);
        ret = Widevine_ReadDevCert(&keybox_file);
        if (ret < 0)
        {
           return HI_FAILURE;
        }
