    ...
    for (i = 0; i<HI_UNF_CEC_LOGICALADD_BUTT; i++)
    {
        ...
        switch(pstCecCmd->enCmd)
        {
            ...
            case HDMI_TEST_CEC_SET_VOLUME_UP:
                stCmd.u8Opcode = 0x44;
                stCmd.unOperand.stUIOpcode = 0x41;
                break;
            case HDMI_TEST_CEC_SET_VOLUME_DOWN:
                stCmd.u8Opcode = 0x44;
                stCmd.unOperand.stUIOpcode = 0x42;
                break;
            case HDMI_TEST_CEC_SET_VOLUME_MUTE:
                stCmd.u8Opcode = 0x44;
                stCmd.unOperand.stUIOpcode = 0x43;
                break;
            case HDMI_TEST_CEC_SYSTEM_AUDIO_MODE_REQUEST:
                stCmd.u8Opcode = 0x70;
                stCmd.unOperand.stRawData.u8Length = 0x02;
                stCmd.unOperand.stRawData.u8Data[0] = ((stCecStatus.u8PhysicalAddr[0] << 4) & 0xf0) | (stCecStatus.u8PhysicalAddr[1] & 0x0f);
                stCmd.unOperand.stRawData.u8Data[1] = ((stCecStatus.u8PhysicalAddr[2] << 4) & 0xf0) | (stCecStatus.u8PhysicalAddr[3] & 0x0f);
                break;
