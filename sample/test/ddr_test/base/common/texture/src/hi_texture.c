...
typedef struct TEXTURE_TGA_HEADER_S
{
...
}TEXTURE_TGA_HEADER_S;

typedef struct TEXTURE_PKM_HEADER_S
{
...
} TEXTURE_PKM_HEADER_S;

void *HI_TEXTURE_LoadTGA(FILE *    File,
                                    GLenum *  Format,
                                    GLsizei * Width,
                                    GLsizei * Height)
{
    struct TEXTURE_TGA_HEADER_S tga;
    size_t bytes;
    unsigned char * bits;
...
void *HI_TEXTURE_LoadPKM(FILE *    File,
                                    GLenum *  Format,
                                    GLsizei * Width,
                                    GLsizei * Height,
                                    GLsizei * Bytes)
{
    struct TEXTURE_PKM_HEADER_S pkm;
    size_t bytes;
    unsigned char * bits;
    unsigned short type;
...
HI_S32 HI_TEXTURE_Load2DTexture(const HI_CHAR *pFileName, HI_S32 s32DataType)
{
    GLuint texture = 0;
    FILE * f;
    ...
    do
    {
        ...
        case HI_TGA:     /* TGA file. */
            bits = HI_TEXTURE_LoadTGA(f, &format, &width, &height);
        ...
        case HI_PKM:     /* PKM file. */
            bits = HI_TEXTURE_LoadPKM(f, &format, &width, &height, &bytes);
